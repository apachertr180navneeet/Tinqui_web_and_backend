<template>
    <div
        ref="sidebarMenuC"
        class="sider_bar_menus h-screen bg-primary-50 dark:border-gray-600 dark:bg-secondary-900 text-secondary-800 dark:text-secondary-100 transition-all duration-30 p-4 fixed xl:relative overflow-y-scroll overflow-x-hidden scroll-smooth"
        :class="{
            'w-76': sidebarFull,
            'w-76 xl:w-20': !sidebarFull,
            'top-0 start-0': sidebarNavOpen,
            'top-0 -left-76 xl:left-0': !sidebarNavOpen,
        }"
    >
        <!-- sidebar title -->

        <div
            class="flex flex-row ms-1 mt-4 justify-center xl:ms-0"
            :class="!sidebarFull ? '' : 'ps-2 pe-2'"
        >
            <Link :href="route('admin.index')" class="flex flex-row">
                <div
                    v-if="$page.props.backendLogo"
                    class="rounded-lg pe-1"
                    :class="!sidebarFull ? 'w-8 h-8' : 'h-12 w-12'"
                >
                    <img
                        v-lazy="{
                            src:
        ...
        <div class="mt-5 mb-5" :class="sidebarFull ? 'flex' : 'flex xl:hidden'">
            <ps-input-with-right-icon
                ...
                theme="bg-white dark:bg-secondaryDark-black border-none rounded-lg placeholder-secondary-800 dark:placeholder-secondary-200"
                class="w-full rounded-full flex"
            ...
            >
                <template #icon>
                    <ps-icon
                        ...
                        theme="text-secondary-800 dark:text-secondary-200"
                    />
                </template>
            </ps-input-with-right-icon>
        </div>
        <div
            class="rounded-lg p-1 mt-6 mb-8 w-full flex items-center text-secondary-800 bg-primary-50 hover:bg-primary-200 dark:hover:bg-secondary-700 dark:text-secondary-300 dark:bg-secondary-900"
        ...
        <div class="grid grid-cols-1 divide-y-4">
            <div class="">
                ...
                <div
                    v-for="menugroup in $page.props.menuGroups"
                    :key="menugroup.id"
                >
                    <ps-label
                        :class="{ flex: sidebarFull, hidden: !sidebarFull }"
                        class="mt-4 mb-2"
                        v-if="
                            searchterm == null &&
                            menugroup.is_invisible_group_name != 1
                        "
                    >
                    ...
                    <ps-label
                        :class="{ flex: sidebarFull, hidden: !sidebarFull }"
                        class="mt-4 mb-2"
                        v-else-if="menugroup.is_invisible_group_name != 1"
                    >
                        <span
                            v-if="
                                $t(menugroup.group_lang_key)
                        ...
                        <span
                            v-else-if="
                                menugroup.sub_menu_group.filter((sub) =>
                                    $t(sub.sub_menu_lang_key)
                                        .toLowerCase()
                                        .trim()
                                        .includes(
                        ...
                        <span
                            v-else-if="
                                menugroup.sub_menu_group.filter(
                                    (sub) =>
                                        sub.is_dropdown == 1 &&
                                        sub.module.filter((module) =>
                                            $t(module.module_lang_key)
                                                .toLowerCase()
                                                .trim()
                                                .includes(
                                                    searchterm
                    ...
                    <div
                        v-for="group in menugroup.sub_menu_group"
                        :key="group.id"
                        class="mt-2"
                    >
                        <sidebar-menu-item
                            v-model:dropDown="dropDownOpen"
                            :group="group"
            ...
            <div class="flex items-center justify-center my-2 py-2">
                ...
                >
                    {{ $t("core__be_version") }}
                    {{ $page.props.backendSetting.backend_version_no }}
                </ps-label>
            </div>
        </div>
    </div>
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            if (localStorage.sidebarScroll != null) {
                setTimeout(() => {
                    sidebarMenuC.value.scrollTo(
                        0,
                        parseInt(localStorage.sidebarScroll)
                    );
                }, 1000);
            }
        });
