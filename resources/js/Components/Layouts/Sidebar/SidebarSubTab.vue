<template>
    <ps-label>
        <div
            ...
            :class="{
                'justify-start': sidebarFull,
                'sm:justify-center': !sidebarFull,
                [activeTheme]: sidebarActive == name,
                [defaultTheme]: sidebarActive != name,
                [rounded]: true,
                [padding]: true,
            }"
        >
            <div class="relative items-center ps-10">
                ...
                <span
                    :class="!sidebarFull ? 'sm:hidden' : ''"
                    class="me-2 text-base"
                >
                    {{ name }}
                </span>
            </div>
            ...
            >
                <ps-label
                    v-if="noti != ''"
                    class="ms-2 me-2 text-xxs rounded-full text-center"
                    :class="{
                        'text-primary-500': sidebarActive == name,
                        'bg-primary-500  text-white': sidebarActive != name,
                        'w-6 h-6 pt-1.5': parseInt(noti) > 99,
                        'w-5 h-5 pt-1': parseInt(noti) > 9,
                        'w-4 h-4 pt-0.5': parseInt(noti) < 9,
                    }"
                >
...
export default {
    ...
    props: {
        ...
        defaultTheme: {
            type: String,
            default:
                "text-secondary-800 bg-primary-50 hover:bg-primary-200 dark:hover:bg-primary-200 dark:hover:text-secondary-800 dark:text-secondary-100 dark:bg-secondary-900 ",
        },
        activeTheme: {
            type: String,
