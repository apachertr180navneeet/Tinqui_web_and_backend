<template>
    ...
    <!-- <small
        v-if="!project.ps_license_code"
        class="z-50 p-1 w-full text-white-50 bg-red-100 h-8 fixed top-0 text-center"
    >
    ...
    <div class="flex flex-row" :dir="getDir()">
        <!-- right -->
        <div
            class="flex-grow w-full flex flex-col dark:bg-secondaryDark-black dark:text-textLight"
        >
            <!-- content -->
            <div
                @click="clickOutsideSidebar"
                :style="[
                    !project.ps_license_code
                        ? 'margin-top: 24px !important;'
                        : '',
                ]"
                :class="{
                    'xl:ms-76 ms-0': sidebarFull,
                    'ms-0 xl:ms-20': !sidebarFull,
                }"
                class="h-screen pt-18"
            >
                <div
                    class="right_side_content h-full px-4 transition-all duration-600 overflow-x-hidden overflow-y-auto scroll-smooth"
                >
                            ...
                            >
                                ...
                                <div class="w-full">
                                    <div class="flex justify-between w-full">
                                        <ps-label
                                            textColor="leading-6 text-base font-medium text-primary-500"
                                            >{{
                                                $t(
                                                    "core_be__version_update_noti_title"
                                                )
                                            }}</ps-label
                                        >
                                        <ps-label
                                    ...
                                    <ps-label
                                        textColor="text-sm text-secondary-500 mt-3"
                                        >{{
                                            $t(
                                                "core_be__version_update_noti_desc"
                                            )
                                        }}</ps-label
                                    >
                                ...
                                <div class="w-full">
                                    <div class="flex justify-between w-full">
                                        <ps-label
                                            ...
                                            >{{
                                                $t(
                                                    "core_be__project_change_noti_title"
                                                )
                                            }}</ps-label
                                        >
                                        <ps-label
                                    ...
                                    <ps-label
                                        ...
                                        >{{
                                            $t(
                                                "core_be__project_change_noti_desc"
                                            )
                                        }}</ps-label
                                    >
                                    <ps-button
                                ...
                                <div class="w-full">
                                    <div class="flex justify-between w-full">
                                        <ps-label
                                            ...
                                            >{{
                                                $t(
                                                    "core_be__losing_connection_noti_title"
                                                )
                                            }}</ps-label
                                        >
                                        <ps-label
                                    ...
                                    <ps-label
                                        ...
                                        >{{
                                            $t(
                                                "core_be__losing_connection_noti_desc"
                                            )
                                        }}</ps-label
                                    >
                                    <ps-button class="mt-6" rounded="rounded">
                                        <a
                                            href="https://tinqui.ch"
                                            target="_blank"
                                            >{{
                                                $t(
                                                    "core_be__open_token_key_documentation"
                                                )
                                            }}</a
                                        >
                                    </ps-button>
        ...
        <!-- left -->
        <div
            class="min-h-screen antialiased flex fixed ltr:left-0 rtl:right-0"
            :style="[!project.ps_license_code ? 'top: 24px !important;' : '']"
        >
            <sidebar-menu />
        </div>
    ...
    <ps-license-activate-modal
        ...
        :project="project"
        :errors="errors"
        :systemCode2="systemCode2"
        ref="ps_license_activate_modal"
    />
</template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        if (props.builderAppInfo != null && props.builderAppInfo != "") {
            const builderVersioncode = ref(
                props.builderAppInfo?.latestVersion?.version_code
            );
            const currentVersionUpdate = ref(props.checkVersionUpdate);

            if (currentVersionUpdate.value != null) {
                const sourceCode = ref(
                    currentVersionUpdate.value.source_code_version_code ==
                        builderVersioncode.value
                        ? true
                        : false
                );
                const backendLanguage = ref(
                    currentVersionUpdate.value.backend_language_version_code ==
                        builderVersioncode.value
                        ? true
                        : false
                );
                const frontendLanguage = ref(
                    currentVersionUpdate.value.frontend_language_version_code ==
                        builderVersioncode.value
                        ? true
                        : false
                );
                const mobileLanguage = ref(
                    currentVersionUpdate.value.mobile_language_version_code ==
                        builderVersioncode.value
                        ? true
                        : false
                );
                const syncAble = ref(
                    props.builderAppInfo.syncAble == 1 ? true : false
                );
                if (
                    sourceCode.value &&
                    backendLanguage.value &&
            ...
            isProjectChanged.value =
                props.builderAppInfo.isProjectChanged == 1 ? true : false;
            isConnected.value =
                props.builderAppInfo.isConnected == 0 ? true : false;
        }
        ...
        onMounted(() => {
            ...
            if (
                props.builderAppInfo != null &&
                props.builderAppInfo.isValid == 0
            ) {
                ps_license_activate_modal.value.openModal(
                    trans("pls_activate_your_app"),
            ...
            if (currentRoute == "admin") {
                ...
                for (let i = 0; i < menugroup.length; i++) {
                    for (
                        let j = 0;
                        j < menugroup[i].sub_menu_group.length;
                        j++
                    ) {
                        if (menugroup[i].sub_menu_group[j].module.length > 0) {
                            for (
                                let k = 0;
                                k <
                                menugroup[i].sub_menu_group[j].module.length;
                                k++
                            ) {
                                if (
                                    menugroup[i].sub_menu_group[j].module[k]
                                        .module_name == currentRoute
                                ) {
                                    const moduleLangName =
                                        menugroup[i].sub_menu_group[j].module[k]
                                            .module_lang_key;
                                    setTimeout(() => {
                                        store.dispatch(
                                            "handleSidebarActive",
                                            trans(moduleLangName)
                                        );
                                    }, 100);
                                }
                            }
                            ...
                            if (
                                menugroup[i].sub_menu_group[j].sub_menu_name ==
                                currentRoute
                            ) {
                                const moduleLangName =
                                    menugroup[i].sub_menu_group[j]
                                        .sub_menu_lang_key;
                                setTimeout(() => {
                                    store.dispatch(
                                        "handleSidebarActive",
