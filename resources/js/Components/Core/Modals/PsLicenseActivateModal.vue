<template>
    <ps-modal
        ...
        line="hidden"
        :isClickOut="false"
        theme=" px-4 py-4 rounded-lg shadow-xl"
        class="z-20"
    >
        <template #title>
            ...
            <div v-if="isCountDownShow" class="flex flex-col">
                <div
                    class="bg-primary-50 text-xl p-3 text-dark rounded w-full text-center mb-8"
                >
                    {{ $t("your_free_trial_duration_left") }}
                    <span
                        class="text-green-500 animate-pulse"
                        id="systemCode2"
                        >{{ showcountDownDate }}</span
                    >
                </div>
            </div>
            <ps-label class="font-medium text-xl lg:text-2xl px-4">
                {{ $t(title) }}
            </ps-label>
        </template>
        <template #body>
            <div class="w-full mt-6 px-4">
                ...
                <div class="mb-6">
                    <div class="mb-4">
                        <ps-label class="text-base mb-1">
                            ...
                            {{
                                $t(
                                    "installer_messages.environment.wizard.form.base_domain_label"
                                )
                            }}
                        </ps-label>
                        <ps-input-with-right-icon
                            :disabled="isDisableBackendUrl"
                            v-model:value="form.backend_url"
                            :placeholder="$t('enter_backend_url')"
                        >
                            <template #icon>
                                <ps-loading
                                    v-if="loadingBackendUrl"
                                    theme="border-2 border-t-2 border-text-8 border-t-primary-500"
                                    loadingSize="h-5 w-5"
                                />
                                <ps-label
                                    v-if="successBackendUrl"
                                    class="transition-all duration-300"
                                    textColor="text-dark dark:text-secondaryDark-black"
                                    >{{ $t("core__be_btn_saved") }}</ps-label
                                >
                                <ps-button
                                    @click="handleUpdateBackendUrl()"
                                    v-if="
                                        isEditBackendUrl &&
                                        !loadingBackendUrl &&
                                        !successBackendUrl
                        ...
                        <ps-label-caption
                            ...
                            class="mt-2 block"
                            >{{ errors.backend_url }}</ps-label-caption
                        >
                    </div>
                    ...
                    <div class="">
                        ...
                        <ps-input-with-right-icon
                            :disabled="isDisablePurchasedCode"
                            v-model:value="form.purchased_code"
                            :placeholder="$t('enter_purchase_code')"
                        >
                            <template #icon>
                                <ps-loading
                                    v-if="loadingPurchasedCode"
                                    theme="border-2 border-t-2 border-text-8 border-t-primary-500"
                                    loadingSize="h-5 w-5"
                                />
                                <ps-label
                                    v-if="successPurchasedCode"
                                    class="transition-all duration-300"
                                    textColor="text-dark dark:text-secondaryDark-black"
                                    >{{ $t("core__be_btn_saved") }}</ps-label
                                >
                                <ps-button
                                    @click="handleUpdatePurchaseCode()"
                                    v-if="
                                        isEditPurchasedCode &&
                        ...
                        <ps-label-caption
                            textColor="text-red-500 "
                            class="mt-2 block"
                            >{{ errors.purchased_code }}</ps-label-caption
                        >
                        <ps-label-caption
                            v-if="purchased_code"
                            textColor="text-red-500 "
                            class="mt-2 block"
                            >{{ purchased_code }}</ps-label-caption
                        >
                    </div>
                </div>
                   ...
                   <div v-if="!selectedFile && !selectedZipFileName" class="">
                    ...
                    <div class="mb-4">
                        ...
                        <ps-input-with-right-icon
                            ...
                            :placeholder="
                                $t('setup_connection_with_builder__enter_token')
                            "
                            ref="token"
                        >
                            <template #icon>
                ...
                <div
                    v-if="!connect && !reconnect"
                    class="mt-2 mb-6 pb-4 px-4 pt-2 bg-blue-50 flex flex-row"
                >
                    ...
                    <div>
                        <ps-label textColor="text-secondary-900"
                            ><span class="text-base">{{
                                $t("project_create_at_ps_builder__title")
                            }}</span></ps-label
                        >
                        <ps-label class="mt-1 mb-2">{{
                            $t("project_create_at_ps_builder__description")
                        }}</ps-label>
                        <ps-label textColor="text-blue-500" class="underline"
                            ><a
                                target="_blank"
                                href="https://tinqui.ch/project/create"
                                >{{
                                    $t("project_create_at_ps_builder__link")
                                }}</a
                            ></ps-label
                        >
                    </div>
                ...
                <!-- alert start -->
                <div
                    class="p-4 bg-red-100 border border-red-200 rounded-md"
                    v-if="reconnect"
                >
                        ...
                        <div class="w-0 flex-1 flex">
                            ...
                            <div>
                                <h4 class="text-md leading-6 font-medium">
                                    {{
                                        $t(
                                            "setup_connection_with_builder__error_title"
                                        )
                                    }}
                                </h4>
                                <p class="text-sm">
                                    {{
                                        $t(
                                            "setup_connection_with_builder__error_dialog"
                                        )
                                    }}
                                </p>
                                <div class="flex mt-3">
                ...
                <div
                    class="p-4 bg-green-100 border border-red-200 rounded-md"
                    v-if="connect"
                >
                    <div class="flex justify-between flex-wrap">
                        <div class="w-0 flex-1 flex">
                            ...
                            <div>
                                <h4 class="text-md leading-6 font-medium">
                                    {{
                                        $t(
                                            "setup_connection_with_builder__success_title"
                                        )
                                    }}
                                </h4>
                                <p class="text-sm">
                                    {{
                                        $t(
                                            "setup_connection_with_builder__success_dialog"
                                        )
                                    }}
                                </p>
                            </div>
                ...
                <div class="mt-3">
                    <ps-button
                        ...
                        class="mb-0.5"
                        padding="px-3 py-2"
                        v-if="!activate && !reconnect"
                    >
                        <!-- <ps-icon name="plus" class="mx-0.5 font-semibold" /> -->
                        <ps-label textColor="text-white">{{
                            $t("setup_connection_with_builder__connect")
                        }}</ps-label>
                    </ps-button>
                    ...
                    >
                        <!-- <ps-icon name="plus" class="mx-0.5 font-semibold" /> -->
                        <ps-label textColor="text-white">{{
                            $t("setup_connection_with_builder__reconnect")
                        }}</ps-label>
                    </ps-button>
                    ...
                    <ps-button
                        ...
                        class="mb-0.5"
                        padding="px-3 py-2"
                        v-if="activate && connect"
                    >
                        <!-- <ps-icon name="plus" class="mx-0.5 font-semibold" /> -->
                        <ps-label textColor="text-white">{{
                            $t("setup_connection_with_builder__activate")
                        }}</ps-label>
                    </ps-button>
                </div>
                ...
                <div
                    v-if="logMessages"
                    class="mb-6 border rounded border-red-500 w-80 p-3"
                >
                    <div v-for="logMessage in logMessages" class="">
                        <div
                            class="flex mb-2"
                            v-if="logMessage.status === 'danger'"
                        >
                            <div class="">
                                <ps-icon
                                    name="cross"
                                    w="20"
                                    h="20"
                                    class="me-4 p-0.5 inline-block align-middle font-semibold bg-red-500 border rounded-full text-white"
                                />
                            </div>
                        ...
                        <div
                            class="flex mb-2"
                            v-if="logMessage.status === 'success'"
                        >
                            <div class="">
                                <ps-icon
                                    name="check"
                                    w="20"
                                    h="20"
                                    class="me-4 p-0.5 inline-block align-middle font-semibold bg-green-500 border rounded-full text-white"
                                />
                            </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="mb-2">
                        <a
                            target="_blank"
                            href="https://www.docs.tinqui.ch/mpc/faqs/web/how-to-solve-connectivity-error-with-Tinqui-Builder"
                            class="underline text-indigo-500"
                        >
                    ...
                    <div class="mb-2">
                        <a target="_blank" href="https://tinqui.ch">
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function importClicked() {
            if (
                !isEditBackendUrl.value &&
                !loadingBackendUrl.value &&
                !successBackendUrl.value &&
                !isEditPurchasedCode.value &&
                !loadingPurchasedCode.value &&
                !successPurchasedCode.value
            ) {
            ...
            if (tokenForm.token == null || tokenForm.token == "") {
                router.post(route("api_token.default_creating_token"), form1, {
                    onSuccess: (res) => {
                        ...
                        tokenForm.token = !res.props.defaultBuilderToken
                            ? ""
                            : res.props.defaultBuilderToken;
        ...
        function installClicked() {
            if (tokenForm.token != "") {
                router.post(
                    route(
                        "admin.dashboard.activateLicenseWithBuilderConnection"
                    ),
                    tokenForm,
                    {
                        onBefore: () => {
        ...
        function checkConnection() {
            if (tokenForm.token != "") {
                // console.log(tokenForm.token);
                router.post(
                    route("admin.dashboard.checkConnection"),
                    tokenForm,
                    {
                        onBefore: () => {
                            ps_loading_circle_dialog.value.openModal(
                                trans("creating_auto_sync"),
                                trans("wait_msg")
                            );
                        },
                        onSuccess: () => {
