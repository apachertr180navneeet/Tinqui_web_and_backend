...
if (!function_exists('duplicate')) {
    ...
    function duplicate($data, $copies, $img_copy = false)
    {
        ...
        if ($img_copy == true) {
            ...
            $storage_upload_path = '/storage/MPC/uploads/';
            $storage_thumb1x_path = '/storage/MPC/thumbnail/';
            $storage_thumb2x_path = '/storage/MPC/thumbnail2x/';
            $storage_thumb3x_path = '/storage/MPC/thumbnail3x/';
...
if (!function_exists('getVendorIdFromSession')) {
    ...
    function getVendorIdFromSession()
    {
        ...
        $currentSessionData = DB::table('custom_sessions')->where('id', $currentSessionId)->first();

        //check if have same ip address and user agent for same user id
        if($currentSessionData == null && Auth::user()){
            $sameIpAndAgent = DB::table('custom_sessions')
            ...
            if ($sameIpAndAgent) {
                ...
                $currentSessionData = DB::table('custom_sessions')
        ...
        if($currentSessionData == null && Auth::user()){
            ...
            // Insert data into the custom_sessions table
...
if (!function_exists('generateFEangStrJson')) {
    function generateFEangStrJson($fileName, $lang_str)
    {
        ...
        File::put(base_path('Modules/Template/FETemplate/Resources/frontend_languages/' . $fileName), $file);
    }
}
...
if (!function_exists('send_android_fcm_bid')) { 
    function send_android_fcm_bid($registatoin_ids, $data, $platform_names)
    {
        // dd("gheere");
        $backend_setting = BackendSetting::first();
        ...
        $bid = BidDetails::where('id',$data['bid_id'])->first();
        //\Log::info($bid);
        if (isset($bid->product_id)) {
        //\Log::info($bid->product_id);
            $id = $bid->product_id;
        ...
        foreach ($platform_names as $platform_name) {
            ...
            //     $click_action = $prj_name .  'chat?buyer_user_id=' . $data['buyer_user_id'] . '&seller_user_id=' . $data['seller_user_id'] . '&item_id=' . $data['item_id'] . '&chat_flag=' . $data['chat_flag'];
            // } else
            if (strtolower($platform_name) == "android" || strtolower($platform_name) == "ios") {
                $click_action = ps_constant::flutterNotificationClick;
            } else if (strtolower($platform_name) == "frontend")
                $click_action = $prj_name . "";
            else {
        ...
        if ($flag == 'bid') {
            ...
            $seller_id = $bid->sender_id;
            $item_id = $bid->product_id;
            $bid_id = $data['bid_id'];
            ...
            $noti_arr = array(
                ...
                'bid_id' => $bid_id,
                'sender_name' => 'Tinqui',
                'sender_profile_photo' => "65926178b3f1d_.png",
                'action' => "abc",
                'click_action' => $click_action
            );
...
if (!function_exists("checkPurchasedCode")) {
    function checkPurchasedCode($response, $routeName = null)
    {
        ...
        if (empty($response->item)) {
            return redirect()->back()->with("purchased_code", "Purchase Code is invalid")->withInput();
        }
    }
}
...
if (!function_exists('deleteOldSessions')) {
    function deleteOldSessions()
    {
        ...
        DB::table('custom_sessions')->where('last_activity', '<', $expirationTimestamp)->delete();
    }
}
...
if (!function_exists('updateSessionLastActivity')) {
    function updateSessionLastActivity()
    {
        ...
        DB::table('custom_sessions')->where('id','=',$currentSessionId)->update(
            [
