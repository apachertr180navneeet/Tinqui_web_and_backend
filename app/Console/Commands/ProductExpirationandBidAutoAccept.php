...
class ProductExpirationandBidAutoAccept extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'product:accept';
     ...
     * @var string
     */
    protected $description = 'It will check for the products creation date and the expiration days and mark the item as expire as well as check bids on item and make the highest bid as accepted bid and transfer money';
    ...
    public function handle()
    {
         ...
         $activeItems = Item::where('is_expired', 0)->whereNotNull('auction_period')->get();
        ...
        $currentunix_timestamp = $currentDateTime->timestamp * 1000;
            if(count($activeItems)>0)
            {
                foreach($activeItems as $activeItem)
                {
                    ...
                    // $item_active_hours = $activeItem->auction_period; // Assuming auction_period is in hours
                    // $expiry_date = $creation_date_time->copy()->addHours($item_active_hours);

                    $item_active_days = $activeItem->auction_period;
                    $expiry_date = $creation_date_time->copy()->addDays($item_active_days);
