...
class PackageReportExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function collection()
    {
        ...
        leftJoin('payment_infos','package_bought_transactions.package_id','=','payment_infos.id')
        ->leftjoin('users','package_bought_transactions.user_id','=','users.id')
        ->select('package_bought_transactions.*','users.name as user_name','payment_infos.value')
        ->with($relation)
        ->latest()
        ->get();
    }
