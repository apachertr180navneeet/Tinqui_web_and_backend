...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('vendor_languages_strings', function (Blueprint $table) {
            $table->id();
        ...
        $vendor_lang_module = DB::table('modules')->where('lang_key','=','vendor_language_module')->first();
        $vendor_lang_str_module = DB::table('modules')->where('lang_key','=','vendor_language_string_module')->first();

        if(empty($vendor_lang_module)){
            $vendor_lang_module_id = DB::table('modules')->insertGetId([
                'id' => 75,
        ...
        if(empty($vendor_lang_menus)){
            ...
            DB::table('modules')->where('id','=',$vendor_lang_module_id)->update(['menu_id' => $menu->id]);
        }
    ...
    {
        Schema::dropIfExists('vendor_languages_strings');
    }
};
