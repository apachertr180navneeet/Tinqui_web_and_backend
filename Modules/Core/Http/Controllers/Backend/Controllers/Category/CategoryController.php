...
//         return $dataArr;
    ...
    public function store(Request $request)
    {
        ...
        if(in_array('name',$coreFieldsIds)){
            $validationArr['name'] = 'required|min:3|unique:categories,name';
        }
        ...
        if(in_array('commission',$coreFieldsIds)){
             $validationArr['commission'] = [
            'required',
            'numeric',
            'min:1',
            'regex:/^(?:[1-9]|[1-9][0-9]|99)(?:\.\d{1,2})?$/'
        ];
        }

        $messages = [
        'commission.numeric' => 'Le champ Commission doit être une valeur numérique.',
        'commission.regex' => "La commission doit être un nombre compris entre 1 et 99, avec jusqu'à deux décimales.",
    ]; 

        $validator = Validator::make($request->all(),$validationArr, $messages);
    ...
    public function update(Request $request, $id)
    {
        ...
        if(in_array('name',$coreFieldsIds)){
            $validationArr['name'] = 'required|min:3|unique:categories,name,'.request()->route('category');
        }
