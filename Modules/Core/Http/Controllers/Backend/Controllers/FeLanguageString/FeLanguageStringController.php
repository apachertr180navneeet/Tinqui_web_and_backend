...
namespace Modules\Core\Http\Controllers\Backend\Controllers\FeLanguageString;
...
use Modules\Core\Http\Services\FeLanguageStringService;

class FeLanguageStringController extends Controller
{
    const parentPath = "frontend_languages_strings/";
    const indexPath = self::parentPath."Index";
    const createPath = self::parentPath."Create";
    const editPath = self::parentPath."Edit";
    const indexRoute = "fe_language_string.index";
    const createRoute = "fe_language_string.create";
    const editRoute = "fe_language_string.edit";

    protected $languageStringService, $successFlag, $dangerFlag, $csvFile, $warningFlag, $code, $coreFieldFilterSettingService;

    public function __construct(FeLanguageStringService $languageStringService, CoreFieldFilterSettingService $coreFieldFilterSettingService)
    {
    ...
    public function store(Request $request)
    {
        ...
        if(in_array('key',$coreFieldsIds)){
            $validationArr['key'] = 'required|unique:fe_languages_strings,key';
        }

        if(in_array('value',$coreFieldsIds)){
            $validationArr['value'] = 'required|unique:fe_languages_strings,value';
        }
...
//    }
    ...
    public function update($language_id, $id, Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect()->route('fe_language_string.edit',['language' => $language_id, 'fe_language_string' => $id])->with('language_strings_relation_errors',$errors)
                ->withErrors($validator)
                ->withInput();
        } else {
            if (collect($errors)->isNotEmpty()){
