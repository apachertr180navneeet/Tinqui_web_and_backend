...
class MobileLanguageController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(in_array('symbol',$coreFieldsIds)){
            $validationArr['symbol'] = 'required|unique:mobile_languages,symbol';
        }

        if(in_array('name',$coreFieldsIds)){
            $validationArr['name'] = 'required|unique:mobile_languages,name';
        }

        if(in_array('language_code',$coreFieldsIds)){
            $validationArr['languageCode'] = 'required|unique:mobile_languages,language_code';
        }

        if(in_array('country_code',$coreFieldsIds)){
            $validationArr['countryCode'] = 'required|unique:mobile_languages,country_code';
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(in_array('symbol',$coreFieldsIds)){
            $validationArr['symbol'] = 'required|unique:mobile_languages,symbol,'.$id;
        }

        if(in_array('name',$coreFieldsIds)){
            $validationArr['name'] = 'required|unique:mobile_languages,name,'.$id;
        }

        if(in_array('language_code',$coreFieldsIds)){
            $validationArr['languageCode'] = 'required|unique:mobile_languages,language_code,'.$id;
        }

        if(in_array('country_code',$coreFieldsIds)){
            $validationArr['countryCode'] = 'required|unique:mobile_languages,country_code,'.$id;
        }
