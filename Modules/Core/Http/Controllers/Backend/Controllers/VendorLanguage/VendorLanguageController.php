...
namespace Modules\Core\Http\Controllers\Backend\Controllers\VendorLanguage;
...
use Modules\Core\Http\Services\VendorLanguageService;

class VendorLanguageController extends Controller
{
    const parentPath = "vendor_languages/";
    const indexPath = self::parentPath . "Index";
    const createPath = self::parentPath . "Create";
    const editPath = self::parentPath . "Edit";
    const indexRoute = "vendor_language.index";
    const createRoute = "vendor_language.create";
    const editRoute = "vendor_language.edit";

    protected $languageService, $successFlag, $dangerFlag, $csvFile, $warningFlag, $code, $coreFieldFilterSettingService;

    public function __construct(VendorLanguageService $languageService, CoreFieldFilterSettingService $coreFieldFilterSettingService)
    {
    ...
    public function store(Request $request)
    {
        ...
        if (in_array('symbol', $coreFieldsIds)) {
            $validationArr['symbol'] = 'required|unique:languages,symbol';
        }

        if (in_array('name', $coreFieldsIds)) {
            $validationArr['name'] = 'required|unique:languages,name,';
        }
...
//    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (in_array('symbol', $coreFieldsIds)) {
            $validationArr['symbol'] = 'required|unique:languages,symbol,' . $id;
        }

        if (in_array('name', $coreFieldsIds)) {
            $validationArr['name'] = 'required|unique:languages,name,' . $id;
        }
