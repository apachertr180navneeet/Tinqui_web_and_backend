...
use Modules\Core\Transformers\Api\App\V1_0\CategorySubcategory\CategorySubcategoryApiResource;
...
class CategoryApiController extends Controller  
{
    ...
    public function getwithSubCategories(Request $request)
    {
        $categories = $this->categoryService->getFromApi($request);
        if(isset($categories['error'])) {
            if(isset($categories['status'])){
                return responseMsgApi($categories['error'], $categories['status']);
            }else{
            ...
            $data = CategorySubcategoryApiResource::collection($categories);
