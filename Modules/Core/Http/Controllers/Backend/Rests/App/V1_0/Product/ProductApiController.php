...
class ProductApiController extends Controller
{
    ...
    public function create(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'language_symbol' => 'required|exists:languages,symbol',
        ]);
    ...
    { 
            //  echo 'here store items in db';die;
        // dd($request->all());
        ...
        if (in_array('title', $coreFieldsIds)) {
            $validationArr['title'] = 'required|min:3|unique:items,title,' . $request->id;
        }
        if (in_array('description', $coreFieldsIds)) {
            $validationArr['description'] = 'required|min:10';
        }
        if (in_array('category_id', $coreFieldsIds)) {
            $validationArr['category_id'] = 'required|exists:categories,id';
        }
        if (in_array('subcategory_id', $coreFieldsIds)) {
            $validationArr['subcategory_id'] = 'required|exists:subcategories,id';
        }
        if (in_array('location_city_id', $coreFieldsIds)) {
            $validationArr['location_city_id'] = 'required|exists:location_cities,id';
        }
        if (in_array('location_township_id', $coreFieldsIds)) {
            $validationArr['location_township_id'] = 'required|exists:location_townships,id';
        }
        if (in_array('currency_id', $coreFieldsIds)) {
            $validationArr['currency_id'] = 'required|exists:currencies,id';
        }
        if (in_array('vendor_id', $coreFieldsIds)) {
            $validationArr['vendor_id'] = 'required|exists:vendors,id';
        }
        ...
        if (empty($id)) {
            ...
            // if(isset($item->jitsi_token) && !empty($item->jitsi_token))
            // {
            //     $jitsi_token = $item->jitsi_token;
            // }

            $item = new ProductApiResource($this->itemService->getItem($item->id, $itemApiRelation));
            /***********JITSI TOKEN ******************/
            // if(isset($jitsi_token))
            // {
            //     $item->jitsi_token = $jitsi_token;
            // }
    ...
    public function getLiveProductById(Request $request)
    {
       
         $response = $this->itemService->getLiveByIdFromApi($request);
        if (isset($response['error'])) {
            if (isset($response['status'])) {
                return responseMsgApi($response['error'], $response['status']);
            } else {
    ...
    public function update_auction_status(Request $request)
    {
         $validator = Validator::make($request->all(), [ 
            'item_id' => 'required|exists:items,id',
        ]);
        ...
        if ($validator->fails()) {
         ...
         $result = $this->itemService->updateItemAuctionStatus($request->item_id);

         if($result)
         {
            return responseMsgApi(__('core__api_auction_status_updated',[],$request->language_symbol), $this->okStatusCode, $this->successStatus);
         }
    }


    public function get_user_bid_items(Request $request)
    {
         $validator = Validator::make($request->all(), [ 
        ...
        if ($validator->fails()) {
         ...
         $items = $this->itemService->getBidItemsOfUser($request,$request->login_user_id);
            $data = ProductApiResource::collection($items);
            $hasError = $this->itemService->noDataError($request->offset, $data);
    ...
    public function get_user_purchased_items(Request $request)
    {
         $validator = Validator::make($request->all(), [ 
            'login_user_id' => 'required|exists:users,id',
        ]);
        ...
        if ($validator->fails()) {
         ...
         $items = $this->itemService->getPurchasedItemsOfUser($request,$request->login_user_id);
            $data = ProductApiResource::collection($items);
            $hasError = $this->itemService->noDataError($request->offset, $data);
