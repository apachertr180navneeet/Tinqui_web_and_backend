...
use App\Models\WalletLog;  use App\Models\BidDetails;
use Carbon\Carbon;
...
class UserService extends PsService
{
    ...
    public function getSqlForCustomField()
    {
        ...
        foreach (array_unique($customizeuideatil_array) as $key => $customizeuideatil) {
            $sql .= "max(case when user_infos.core_keys_id = '$customizeuideatil' then customize_ui_details.name end) as '$key',";
        }
        foreach ($customizeUis as $key => $customizeUi) {
            if ($key + 1 == count($customizeUis)) {
                $sql .= "max(case when user_infos.core_keys_id = '$customizeUi->core_keys_id' then user_infos.value end) as '$customizeUi->core_keys_id'";
            } else {
                $sql .= "max(case when user_infos.core_keys_id = '$customizeUi->core_keys_id' then user_infos.value end) as '$customizeUi->core_keys_id' ,";
            }
        }
    ...
    public function getUserWallet($id = null)
    {

        // $user_wallet = UserWallet::where('user_id', $id)
        //         ->select('users_wallet.*', 'wallet_log.*')
        //         ->leftJoin('wallet_log', 'wallet_log.users_wallet_id', '=', 'users_wallet.id')
        //         ->get()
        //         ->groupBy('users_wallet.id');

        $user_wallet_details = UserWallet::where('user_id', $id)
            ->get();
        if(count($user_wallet_details)>0) {
             foreach ($user_wallet_details as $user_wallet) {
            $wallet_logs = WalletLog::where('users_wallet_id', $user_wallet->id)->orderBy('id','DESC')->get();
            $user_wallet->wallet_logs = $wallet_logs;

             $bid_details = BidDetails::where('seller_id', $id)->orderBy('id','DESC')->get();
            $user_wallet->bid_details = $bid_details;
    ...
    public function getBuyers($relation = null, $status = null, $isBanned = null, $conds = null, $limit = null, $offset = null, $condsIn = null, $noPagination = null, $pagPerPage = null, $sort = null)
    {
        ...
        $users = User::join('user_boughts', 'user_boughts.buyer_user_id', '=', 'users.id')
            ...
            ->leftJoin($this->usrInfoTableName, $this->usrTableName . '.' . $this->userIdCol, '=', $this->usrInfoTableName . '.' . $this->usrInfoUserIdCol)
            ->leftJoin('customize_ui_details', $this->usrInfoTableName . '.value', '=', 'customize_ui_details.id')
            ->groupBy($this->usrTableName . '.' . $this->userIdCol)
            ->when($relation, function ($q, $relation) {
                $q->with($relation);
            })
        ...
        //        ->latest('user_boughts.added_date', 'desc');
    ...
    public function getSellers($relation = null, $status = null, $isBanned = null, $conds = null, $limit = null, $offset = null, $condsIn = null, $noPagination = null, $pagPerPage = null, $sort = null)
    {
        ...
        $users = User::join('items', 'items.added_user_id', '=', 'users.id')
            ->select('users.*', DB::raw("count(items.added_user_id) as purchased_item_count"))
    ...
    public function registerFromApi($request)
    {
        ...
        //create wallet entry for the user along with stripe account//
         $stripe = new \Stripe\StripeClient(trim($this->paymentSettingService->getPaymentInfo(null, null, $this->stripeSecretKey)->value));
            ...
            else
            {
                ...
                StripeConnectDetails::create(['user_id'=>$userId,'stripe_connect_account_id'=>$accountId]);
                 $data = [];
                $data['user_id'] = $userId;
                $data['currency'] = 'Fr.';
                $data['wallet_balance'] = 0.00;
                $data['recharge_date'] = '11754782574';
        ...
        if ($email_verification_enabled != 1) {
            ...
            if (!sendMail($to, $to_name, $title, $subject, $body)) {
                return responseMsgApi(__('core__api_user_register_success_but_email_not_send_new', [], $request->language_symbol), $this->internalServerErrorStatusCode);
            }
    ...
    public function getTopRatedSellerFromApi($request){
        $limit = $request->limit;
        $offset = $request->offset;
        $users = User::join('user_boughts', 'user_boughts.seller_user_id', '=', 'users.id')
            ->select('users.*')
    ...
    /**********************feb21 get wallet **********************/
       public function getWalletFromApi($request)
    {
        /// check permission start
        $deviceToken = $request->header($this->deviceTokenParaApi);
        $userId = $request->login_user_id;
        ...
        if (empty($userAccessApiToken)) {
            ...
            if (!$userwallet) {
                ...
                return responseDataApi($userwallet);
            }
        }
    }
    /**************************ends here ************************ */
