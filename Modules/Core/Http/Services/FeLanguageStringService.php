...
use Modules\Core\Transformers\Backend\Model\FeLanguageString\FeLanguageStringWithKeyResource;


class FeLanguageStringService extends PsService
{
    ...
    public function __construct(LanguageService $languageService)
    {
        ...
        $this->code = Constants::feLanguageString;
    ...
    public function store($request)
    {
        DB::beginTransaction();
        try {
            $language_string = new FeLanguageString();
            ...
            foreach ($backend_languages as $backend_language){
                if ($backend_language->id !== $request->language_id){
                    $backend_language_string = new FeLanguageString();
                ...
                $lang_str = FeLanguageString::where($this->langStringLanguageIdCol, $backend_language->id)->get();
    ...
    public function update($languageStringId, $request)
    {
       DB::beginTransaction();
        try {
            $language_string = FeLanguageString::findOrFail($languageStringId);
            $language_string->key = $request->key;
            $language_string->value = $request->value;
            $language_string->language_id = $request->language_id;
            $language_string->added_user_id = Auth::user()->id;
            ...
            $lang_str = FeLanguageString::where($this->langStringLanguageIdCol, $request->language_id)->get();
            generateFeLangStrJson($fileName, $lang_str);
            DB::commit();
        }catch (\Throwable $e){
            DB::rollBack();
    ...
    public function getLanguageStrings($id, $relations = null,$pagPerPage = null,$conds = null){
        ...
        $languageStrings = FeLanguageString::where($this->langStringLanguageIdCol, $language->id)
	...
	public function index($id,$request){
        ...
        // check permission
        $checkPermission = $this->checkPermission($this->viewAnyAbility, FeLanguageString::class, "admin.index");



        $language = $this->languageService->getLanguage($id);
        $relations = ['owner', 'editor', 'language'];
        $language_strings = FeLanguageStringWithKeyResource::collection($this->getLanguageStrings($id, $relations,$row,$conds));
    ...
    public function create($id){
        ...
        $checkPermission = $this->checkPermission($this->createAbility,FeLanguageString::class, "admin.index");
...
//        }
    ...
    public function getlanguageStringsByKey($request){
        ...
        foreach ($languages as $language){
            $languageString = FeLanguageString::where($this->langStringLanguageIdCol, $language->id)->with(['language'])->where($this->langStringKeyCol, $conds['key'])->first();
            if(!$languageString){
                ...
                $languageString = FeLanguageString::where($this->langStringIdCol, $language_string->id)->with(['language'])->first();
            }
    ...
    public function updateLanguageStrings($request){
        ...
        try {
            foreach ($values as $langString){
                ...
                $lang_str = FeLanguageString::where($this->langStringLanguageIdCol, $langString['language_id'])->get();
                generateFeLangStrJson($fileName, $lang_str);
    ...
    public function importAllLanguageStrings($data){
        ...
        deleteBuilderFeLanguageStrings();
    ...
    public function exportJson($languageId){
        ...
        $langStrs = FeLanguageString::where($this->langStringLanguageIdCol, $language->id)->get();
        ...
        $file = 'fe_' . $language->symbol . '_tran.json';
    ...
    public function exportCSV($languageId)
    {
        ...
        $file = 'fe_' . $language->symbol . '_tran.csv';
    ...
    public function importLanguageString($languageId, $rows){
        ...
        //filter exist keys
        $langStrs = FeLanguageString::where($this->langStringLanguageIdCol, $languageId)->whereIn($this->langStringKeyCol, $currentKeys)->get();
