...
class  ItemService extends PsService
{
    ...
    {
        $this->BASE_URL ="https://livestream.tinqui.ch:5443/openmeetings/"; 
        $this->storage_key_path = 'storage/MPC/JitsiPrivateKey/';
        ...
        $this->itmFavouriteCountCol = Item::favouriteCount;
         ...
         $this->itmAuctionCol = Item::is_auction_item;
    ...
    public function storeCoreFieldValues($request, $status = null)
    {
        ...
        try {
            ...
            if (isset($request->added_user_id) && !empty($request->added_user_id)) {
                ...
                $added_user = $request->added_user_id;
            } else {
                if (Auth::check()) {
            ...
            /*********************auction item April 02***********/
            if (isset($request->is_auction_item) && !empty($request->is_auction_item)) {
                $item->is_auction_item = $request->is_auction_item;

                /*****************create openmeet login for the user ****************/
                if($added_user)
                {
                        // $config = new Configuration();
                        // $config->setHost($this->BASE_URL . 'services');
                        // $userApiInstance = new UserServiceApi(null, $config);
                        // $serviceResultLoginWrapper = $userApiInstance->login("AdminUser", "PassNew@123");
                        // if ($serviceResultLoginWrapper->getServiceResult()->getType() != "SUCCESS") {
                        
                        // return responseMsgApi($serviceResultLoginWrapper->getServiceResult()->getMessage(), $this->internalServerErrorStatusCode);
                        // }
             ...
             if (isset($request->is_warranty_item) && !empty($request->is_warranty_item)) {
                $item->is_warranty_item = $request->is_warranty_item;

            }
             if (isset($request->preferred_payment) && !empty($request->preferred_payment)) {
                $item->preferred_payment = $request->preferred_payment;

            }
           
            if (isset($request->auction_period) && !empty($request->auction_period)) {
               $item->auction_period = $request->auction_period;
                // $item->auction_period = 1;
            }
            if (isset($request->product_creation_date) && !empty($request->product_creation_date)) {
                $item->product_creation_date = $request->product_creation_date;

            }
           // echo 'here item save'; print_r($item);die;
            $item->save();
    ...
    private function createJitsiToken($user_id)
    {
      ...
      $privateKey = file_get_contents(env('APP_URL').$this->storage_key_path.'jitsi_private_key.pk');
          //   $privateKey =file_get_contents("http://127.0.0.1/storage/MPC/JitsiPrivateKey/jitsi_private_key.pk");
        if($user->user_cover_photo)
        {
            $pic = $user->user_cover_photo;
        }
        else{
        ...
        $payload = [
            "context" => [
                "user" => [
                    "avatar"=>"https://tinqui.ch/storage/uploads/".$pic,
                    "name" => $user->name, 
                    "email"=>$user->email,
                    "id"=>$user->id,
                    "moderator"=> "true"
    ...
    private function createOpenMeetLink($user_id,$product_id)
    {
                $config = new Configuration();
                $config->setHost($this->BASE_URL . 'services');
                $userApiInstance = new UserServiceApi(null, $config);
        ...
        //     $apiInstance = new RoomServiceApi(
        ...
        //     $body = new RoomBody(); // \Swagger\Client\Model\RoomBody | 
       ...
       // $uniqueRoomId = $this->generateCustomUuid();
        ...
        $serviceResultHashWrapper = $userApiInstance->getRoomHash($sid,
            ...
            new RoomOptionsDTO(
                    array(
                        ...
                        'name' => 'Live Auction',
                        'type' => 'conference',
                        'comment' => 'Tinqui Product Live Auction',
                    )
                )
        );
        ...
        if ($serviceResultHashWrapper->getServiceResult()->getType() != "SUCCESS") {
           // return responseMsgApi($serviceResultHashWrapper->getServiceResult()->getMessage(), $this->internalServerErrorStatusCode);
        }
    ...
    {
        // //April 05
        //  if (isset($conds['auctionItem']) && $conds['auctionItem'] == 1){
             
        //     $query->where('is_auction_item', 1);
        ...
        if (isset($conds['auctionItem']) && $conds['auctionItem']) {
            $query->where($this->itmTableName . '.' . $this->itmAuctionCol, $conds['auctionItem']);
        }
        ...
        if (isset($conds['added_date_range']) && $conds['added_date_range']) {
            ...
            $query->whereBetween('items.added_date', $added_date_filter);
        }
        ...
        if (isset($conds['updated_date_range']) && $conds['updated_date_range']) {
            ...
            $query->whereBetween('items.updated_date', $updated_date_filter);
        }
    ...
    public function getSuccessfulDealItems($relation, $noPagination = null, $pagPerPage = null, $limit = null, $offset = null, $conds = null, $condsIn = null, $condsNotIn = null, $sort = null)
    {
        ...
        $items = Item::select($this->itmTableName . '.*', "$this->userBoughtTableName.$this->userBoughtBuyerUserIdCol", "$this->userBoughtTableName.$this->userBoughtSellerUserIdCol")
            ->leftJoin($this->itmInfoTableName, $this->itmTableName . '.' . $this->itmIdCol, '=', $this->itmInfoTableName . '.' . $this->itmInfoItemIdCol)
            ->leftJoin($this->customUiDetailTableName, "$this->itmInfoTableName.$this->itmInfoValueCol", '=', "$this->customUiDetailTableName.$this->customUiDetailIdCol")
            ->join('user_boughts', 'user_boughts.item_id', '=', $this->itmTableName . '.' . $this->itmIdCol)
            ->groupBy($this->itmTableName . '.' . $this->itmIdCol, "$this->userBoughtTableName.$this->userBoughtBuyerUserIdCol", "$this->userBoughtTableName.$this->userBoughtSellerUserIdCol")
            ->when(isset($conds['order_by']) && $conds['order_by'], function ($q) use ($sort) {
                if ($sort == 'category_id@@name') {
    ...
    public function getSlowMovingItems($relation, $noPagination = null, $pagPerPage = null, $limit = null, $offset = null, $conds = null, $condsIn = null, $condsNotIn = null, $sort = null, $extra = null)
    {
        ...
        $items = Item::select($this->itmTableName . '.*')
            ->when(isset($conds['order_by']) && $conds['order_by'], function ($q) use ($sort) {
                if ($sort == 'category_id@@name') {
                    $q->join($this->catTableName, 'categories.id', '=', $this->itmTableName . '.' . 'category_id');
                    $q->select('categories.name as cat_name', 'items.*');
                }
                if ($sort == 'subcategory_id@@name') {
                    $q->join($this->subCatTableName, 'subcategories.id', '=', $this->itmTableName . '.' . 'subcategory_id');
                    $q->select('subcategories.name as sub_cat_name', 'items.*');
                }
                if ($sort == 'location_city_id@@name') {
                    $q->join($this->locationCityTableName, 'location_cities.id', '=', $this->itmTableName . '.' . 'location_city_id');
                    $q->select('location_cities.name as city_name', 'items.*');
                }
                if ($sort == 'location_township_id@@name') {
                    $q->join($this->locationCityTableName, 'location_townships.id', '=', $this->itmTableName . '.' . 'location_township_id');
                    $q->select('location_townships.name as township_name', 'items.*');
                }
                if ($sort == 'currency_id@@currency_short_form') {
                    $q->join($this->currencyTableName, 'currencies.id', '=', $this->itmTableName . '.' . 'currency_id');
                    $q->select('currencies.currency_short_form as curr_short_form', 'items.*');
                }
                if ($sort == 'added_user_id@@name') {
                    $q->join($this->userTableName, 'users.id', '=', $this->itmTableName . '.' . 'added_user_id');
                    $q->select('users.name as owner_name', 'items.*');
                }
            })
            ->when($sql, function ($query, $sql) {
                $query->selectRaw($sql);
            ...
            ->when(empty($sort), function ($query, $conds) {
                $query->orderBy('items.status', 'desc')->orderBy('items.title', 'asc');
            })
            ->latest($this->itmTableName . '.' . 'added_date');
    ...
    public function getSoldOutItems($relation, $noPagination = null, $pagPerPage = null, $limit = null, $offset = null, $conds = null, $condsIn = null, $condsNotIn = null, $sort = null)
    {
        ...
        //     if($sort == 'subcategory_id@@name')
        //     {
        //         $q->join('subcategories','subcategories.id','=','subcategory_id');
        //         $q->select('subcategories.name as sub_cat_name','items.*');
        //     }
        //     if($sort == 'location_city_id@@name')
        //     {
        //         $q->join('location_cities','location_cities.id','=','location_city_id');
        //         $q->select('location_cities.name as city_name','items.*');
        //     }
        //     if($sort == 'location_township_id@@name')
        //     {
        //         $q->join('location_townships','location_townships.id','=','location_township_id');
        //         $q->select('location_townships.name as township_name','items.*');
        //     }
        //     if($sort == 'currency_id@@currency_short_form')
        //     {
        //         $q->join('currencies','currencies.id','=','currency_id');
        //         $q->select('currencies.currency_short_form as curr_short_form','items.*');
        //     }
        //     if($sort == 'added_user_id@@name')
        //     {
        ...
        // })
        // ->leftJoin($this->itmInfoTableName, $this->itmTableName.'.'.$this->itmIdCol,'=',$this->itmInfoTableName.'.'.$this->itmInfoItemIdCol)
        // ->leftJoin($this->customUiDetailTableName,"$this->$this->itmInfoTableName.$this->$this->itmInfoValueCol",'=',"cus$this->$this->customUiDetailTableName>$this->customUiDetailIdCol")
        // ->join('user_boughts','user_boughts.item_id','=',$this->itmTableName.'.'.$this->itmIdCol)
        // ->groupBy($this->itmTableName.'.'.$this->itmIdCol)
        // ->when($relation, function ($q, $relation) {
        //     $q->with($relation);
        ...
        $items = Item::select($this->itmTableName . '.*')
            ->when(isset($conds['order_by']) && $conds['order_by'], function ($q) use ($sort) {
                if ($sort == 'category_id@@name') {
                    $q->join($this->catTableName, 'categories.id', '=', 'category_id');
                    $q->select('categories.name as cat_name', 'items.*');
                }
                if ($sort == 'subcategory_id@@name') {
                    $q->join($this->subCatTableName, 'subcategories.id', '=', 'subcategory_id');
                    $q->select('subcategories.name as sub_cat_name', 'items.*');
                }
                if ($sort == 'location_city_id@@name') {
                    $q->join($this->locationCityTableName, 'location_cities.id', '=', 'location_city_id');
                    $q->select('location_cities.name as city_name', 'items.*');
                }
                if ($sort == 'location_township_id@@name') {
                    $q->join($this->locationTownshipTableName, 'location_townships.id', '=', 'location_township_id');
                    $q->select('location_townships.name as township_name', 'items.*');
                }
                if ($sort == 'currency_id@@currency_short_form') {
                    $q->join($this->currencyTableName, 'currencies.id', '=', 'currency_id');
                    $q->select('currencies.currency_short_form as curr_short_form', 'items.*');
                }
                if ($sort == 'added_user_id@@name') {
    ...
    public function getNormalItems($relation, $limit = null, $offset = null, $conds = null, $condsNotIn = null)
    {
        ...
        $normalItems = Item::select($this->itmTableName . '.*')
            ...
            //     $leftJoin->on('paid_item_histories.item_id', '=', 'items.id')
    ...
    public function getItemProduct($id, $dataWithRelation = null, $userId)
    {
        $item = Item::when($dataWithRelation, function ($query, $dataWithRelation) {
            $query->with($dataWithRelation);
        })->where('id', $id)->first();

        // if(isset($item) && $item->is_auction_item == 1)
        // {
        //     $user_id = $userId;
        //     $jitsiToken = $this->createOpenMeetLink($user_id,$id);
        //    // print_r($jitsiToken);
        //     $item->jitsi_token = $jitsiToken;
        // }
...
// dd($columnAndColumnFilter[ps_constant::handlingColumn], $columnAndColumnFilter[ps_constant::handlingFilter]);
    ...
    public function updateItemAuctionStatus($item_id)
    {
        $updated =  Item::where('id',$item_id)->update(['auction_status'=>1,"is_expired"=>1]);
        return $updated;
    }
    ...
    public function soldOutItemReportShow($id, $relations)
    {
        ...
        $cityRelation = ['township'];
        $itemRelation = [
            'vendor', 'category', 'subcategory', 'user_boughts', 'city', 'township', 'currency', 'owner',
    ...
    public function indexFromApi($request)
    {
        ...
        $items = $this->getItems($itemApiRelation, true, null, $limit, $offset);
            ...
            {
                $product_id =$item->id;
                        $highestBidPrice = BidDetails::where('product_id', $product_id)
                                    ->orderBy('bid_price', 'desc')->pluck('bid_price')
                                    ->first();
                        $highest_bid_price = $highestBidPrice ?? $item->price; 
                        $item->highest_bid_price = $highest_bid_price;
                /***************add item commission as per the category **********/
                    $category_commission_of_product = Category::join('items','items.category_id','=','categories.id')->where('items.id',$product_id)->first('categories.commission'); 
                   $commission =  $category_commission_of_product['commission'];
    ...
    public function getByIdFromApi($request)
    {
     ...
     public function getLiveByIdFromApi($request)
    {
        // $today_date = Carbon::now();
        // echo json_encode($today_date);exit;
        // print_r($request);die;
        ...
        $item = $this->getItemProduct($itemId, $itemApiRelation,$userId);
        if (!$item) {
            return  ['error' =>  __('core__api_record_not_found', [], $request->language_symbol), 'status' =>  $this->noContentStatusCode];
        } else {
    ...
    { 
        // echo 'inside storefromapi';
        // print_r($request->is_auction_item);die;
        // check permission start
        $deviceToken = $request->header($this->deviceTokenParaApi);
        ...
        // if($request->is_auction_item == 1)
        // {
        //     $user_id = $request->login_user_id;
        //     $jitsiToken = $this->createOpenMeetLink($user_id,$item->id);
        //    // print_r($jitsiToken);
        //     $item->jitsi_token = $jitsiToken;
        // }
    ...
    public function searchFromApi($request)
    {
        ...
        $conds['auctionItem'] = $request->auctionItem;
    ...
    /*************April 09 *********/
    public function getBidItemsOfUser($request,$id)
    {
        $user_id = $id;
        $bidDetails = BidDetails::where('buyer_id',$user_id)->orderBy('id','desc')->get();
        
        $itemApiRelation = ['vendor', 'category', 'subcategory', 'city', 'township', 'currency', 'owner', 'itemRelation' => ['customizeUi'], 'cover', 'video', 'icon'];

        $items_array = [];
        if(count($bidDetails)>0) {
            foreach($bidDetails as $bidDetail)
            {
                $itemId = $bidDetail->product_id;
                $item = $this->getItem($itemId, $itemApiRelation);
                if (!$item) {
                    return  ['error' =>  __('core__api_record_not_found', [], $request->language_symbol), 'status' =>  $this->noContentStatusCode];
                } else {
    ...
    public function getPurchasedItemsOfUser($request,$id)
    {
        $user_id = $id;
        $bidDetails = BidDetails::where('buyer_id',$user_id)->where('bid_status','completed')->orderBy('id','desc')->get();
