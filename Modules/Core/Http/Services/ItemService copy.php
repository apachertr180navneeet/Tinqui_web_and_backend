...
class  ItemService extends PsService
{
    ...
    private function createJitsiToken($user_id)
    {
       ...
       $apiKey = "vpaas-magic-cookie-bf2e6df6ca2949bfa006a61ddfbc0247/0ae139";
        ...
        $payload = [
            "context" => [
                "user" => [
                    "avatar"=>env('APP_URL')."storage/MPC/uploads/".$user->user_cover_photo,
                    "name" => $user->name, 
                    "email"=>$user->email,
                    "id"=>$user->id,
                    "moderator"=> "true"
    ...
    public function searching($query, $conds)
    {
        ...
        //     $auction = $conds['auctionItem'];
        //     $query->where(function ($query) use ($auction) {
        //         $query->where($this->itmTableName . '.' . 'is_auction_item','=', $auction);
        //     });
        // }
    ...
    public function getItemsforAuction($relation, $noPagination = null, $pagPerPage = null, $limit = null, $offset = null, $conds = null, $condsIn = null, $condsNotIn = null, $sort = null, $extra = null)
    {
    ...
    public function getAuctionItems($relation, $limit = null, $offset = null, $conds = null, $condsNotIn = null)
    {
        $row = Constants::dataTableDefaultRow;

        $auctionItems = ProductWithKeyResource::collection($this->getItemsforAuction($relation, false, $row, null, null, $conds));
        return $auctionItems;
    }
...
// dd($columnAndColumnFilter[ps_constant::handlingColumn], $columnAndColumnFilter[ps_constant::handlingFilter]);
        ...
        if($request->is_auction_item == 1)
            {
                ...
                $jitsiToken = $this->createJitsiToken($user_id);
    ...
    public function searchFromApi($request)
    {
        ...
        if ($ad_post_type == $this->onlyPaidItemAdType) {
            ...
            // if($conds['auctionItem']==1)
            // {
            //     $items = $this->getAuctionItems($itemApiRelation, $limit, $offset, $conds, $condsNotIn);
            // }
            // else
            // {
    ...
    /*****************get item Jitsi Token *****************************/
        foreach($items as $item)
        {
             if($item->is_auction_item == 1)
            {
                $user_id = $request->login_user_id;;
                $JitsiToken = $this->createJitsiToken($user_id);
                $item->jitsi_token = $jitsiToken;
