...
class ImageService extends PsService
{
    ...
    public function clearUnuseImage()
    {
        ...
        $files = Storage::files('/storage/MPC/uploads/');
        $filenames = [];
        foreach ($files as $file) {
            array_push($filenames, explode('/', $file)[3]);
        }

        // $coreImages = DB::table('core_images')->pluck('img_path')->toArray();
        $userImages = DB::table('users')->pluck('user_cover_photo')->toArray();

        $allCoreImages = DB::table('core_images')->get();

        $coreImages =  $allCoreImages->pluck('img_path')->toArray();

        $backendIds = DB::table('backend_settings')->pluck('id')->toArray();
        $blogIds = DB::table('blogs')->pluck('id')->toArray();
        $categoryIds = DB::table('categories')->pluck('id')->toArray();
        $chatIds = DB::table('chat_histories')->pluck('id')->toArray();
        $landingIds = DB::table('landing_pages')->pluck('id')->toArray();
        $itemIds = DB::table('items')->pluck('id')->toArray();
        $notiIds = DB::table('push_notification_messages')->pluck('id')->toArray();
        $subcatgoryIds = DB::table('subcategories')->pluck('id')->toArray();
        $frontendIds = DB::table('frontend_settings')->pluck('id')->toArray();
        // $userImages = DB::table('users')->pluck('user_cover_photo')->toArray();
