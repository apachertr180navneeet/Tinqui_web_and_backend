...
class SubcategoryService extends PsService
{
    ...
    public function getSubcategory($id = null, $name = null, $relation = null){
     ...
     public function getSubCategoriesbyCat($cat_id = null, $limit = null, $offset = null, $status = null, $pagPerPage = null, $noPagination = false, $conds = null)
        {
            $sort = '';

            // Assume $conds is a parameter to pass sorting information
            if (isset($conds['order_by'])) {
                $sort = $conds['order_by'];
            }
            ...
            $subcategories = Subcategory::select($this->subcattableName.'.*')
                ->when($cat_id, function ($query, $cat_id) {
                    $query->whereHas('category', function ($q) use ($cat_id) {
                        $q->where($this->subcattableName .'.'.$this->CategoryIdCol, $cat_id);
                    });
                })
                ->when($limit, function ($query, $limit) {
                    $query->limit($limit);
