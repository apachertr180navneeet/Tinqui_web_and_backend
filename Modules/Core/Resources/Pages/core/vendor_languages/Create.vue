    ...
    <ps-layout>
        ...
        <ps-card class="w-full h-auto">
            <div class="rounded-xl">
                <!-- card header start -->
                <div
                    class="rounded-t-xl bg-primary-50 dark:bg-primary-900 py-2.5 ps-4"
                >
                    <ps-label-header6
                        textColor="text-secondary-800 dark:text-secondary-100"
                        >{{ $t("core__be_language_info") }}</ps-label-header6
                    >
                </div>
                <!-- card header end -->
                        ...
                        <div class="grid w-full sm:w-1/2 gap-6">
                            <div>
                                ...
                                <ps-input
                                    ...
                                    :placeholder="$t('core__be_symbol_label')"
                                    @keyup="
                                        validateEmptyInput(
                                            'symbol',
                                            form.symbol
                                ...
                                <ps-label-caption
                                    ...
                                    class="mt-2 block"
                                    >{{ errors.symbol }}</ps-label-caption
                                >
                            </div>
                            <div>
                                <ps-label
                                    >{{ $t("core__be_language_name_label")
                                    }}<span
                                        class="text-red-800 font-medium ms-1"
                                        >*</span
                                    ></ps-label
                                ...
                                <ps-input
                                    ...
                                    "
                                    @keyup="
                                        validateEmptyInput('name', form.name)
                                    "
                                    @blur="
                                        validateEmptyInput('name', form.name)
                                    "
...
export default defineComponent({
    ...
    setup(props) {
        ...
        function handleSubmit() {
            this.$inertia.post(route("vendor_language.store"), form, {
                forceFormData: true,
                onBefore: () => {
                    loading.value = true;
                },
                ...
                onError: () => {
                    // alert("here")
                    loading.value = false;
                    symbol.value.isError = !Boolean(props.errors.symbol)
                        ? false
                        : true;
                    name.value.isError = !Boolean(props.errors.name)
                        ? false
                        : true;
                },
            });
    ...
    computed: {
        breadcrumb() {
            return [
                ...
                {
                    label: trans("core__be_add_vendor_language"),
    ...
    methods: {
        handleCancel() {
            this.$inertia.get(route("vendor_language.index"));
        },
    },
});
</script>
