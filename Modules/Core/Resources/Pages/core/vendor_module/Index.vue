<template>
    <Head :title="$t('vendor_module_registering_module')" />
    <ps-layout>
        <div class="">
            <!-- breadcrumb start -->
            ...
            <!-- data table start -->
             <ps-table2 :row="row" :search="search" :object="modules" :colFilterOptions="colFilterOptions"
                ...
                @handleRow="handleRow" :searchable="showFilter">
                    <template #button>
                       <ps-button v-if="can.createVendorModule" @click="handleCreate()" rounded="rounded-lg" type="button" class="flex flex-wrap items-center"> <ps-icon name="plus" class="me-1 font-semibold" /> {{$t('core_add_vendor_module')}}</ps-button>
                    </template>
    ...
    setup(props) {
        ...
        function confirmDeleteClicked(id) {
            ps_danger_dialog.value.openModal(
                ...
                () => {
                    this.$inertia.delete(route('vendor_module_registering.destroy',id),{
                            onSuccess: () => {
                            visible.value = true;
                            setTimeout(() => {
                                visible.value = false;
        ...
        function handleEdit(id){
            // console.log(id);
            this.$inertia.get(route('vendor_module_registering.edit',id));
        }
        function handleCreate() {
            this.$inertia.get(route("vendor_module_registering.create"));
        }
        function handlePublish(id,hasPermission){
            if(hasPermission){
                this.$inertia.put(route('vendor_module_registering.statusChange',id));
            }
        }
        ...
        function handleSearchingSorting(page = null, row = null) {
            router.get(route('vendor_module_registering.index'),
                {
    ...
    computed: {
        breadcrumb() {
            return [
                ...
                {
                    label: trans('vendor_module_registering_module'),
      ...
      methods: {
        FilterOptionshandle(value) {
            ...
            router.post(route('vendor_module_registering.screenDisplayUiSetting.store'),
                {
                    value,
