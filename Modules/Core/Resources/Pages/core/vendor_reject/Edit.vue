<template>
    <Head :title="$t('reject_vendor_module')" />
    <ps-layout>
        <!-- breadcrumb start -->
        ...
        <div class="">
            <div class="bg-primary-50 px-4 py-2 rounded-t dark:bg-primary-900">
                <ps-label-header-6>{{ $t('core_be__reject_vendor_details') }}</ps-label-header-6>
            </div>
            <div class="grid grid-cols-2 gap-10 p-4">
                <div class="">
                    ...
                    <ps-button @click="toIndexPage()" class="float-end mt-4">{{ $t('core_be__back') }}</ps-button>
                </div>
            </div>
        </div>
...
import PsIcon from "@/Components/Core/Icons/PsIcon.vue";
    ...
    export default defineComponent({
        ...
        setup(props){
            ...
            {
                this.$inertia.get(route('reject_vendor.index'));
            }
        ...
        computed: {
            breadcrumb() {
                return [
                    ...
                    {
                        label: trans('reject_vendor_module'),
                        url: route('reject_vendor.index')
                    },
                    {
                        label: trans('core_be__reject_vendor_details'),
