            ...
            <div class="rounded-xl">
                ...
                >
                    <ps-label-header-6
                        ...
                        >{{ $t("core__be_category_info") }}</ps-label-header-6
                    >
                </div>
                <!-- card header end -->
                        ...
                        <div class="grid w-full sm:w-1/2 gap-6">
                            <div>
                                ...
                                <ps-input
                                    ...
                                    :placeholder="
                                        $t('core__category_placeholder')
                                    "
                                    @keyup="
                                        validateNameInput('name', form.name)
                                    "
                            ...
                            <div>
                                <ps-label class="text-base mb-2"
                                    >{{ $t("core__category_photo_label")
                                    }}<span
                                        class="text-red-800 font-medium ms-1"
                                        >*</span
                                    >
                                </ps-label>
                                <ps-label-title-3 v-if="!category.cover">
                                    {{
                                        $t("core__be_recommended_size_400_200")
                                    }}
                                </ps-label-title-3>
                                <div
                                    v-if="category.cover"
                                ...
                                <ps-image-upload
                                    v-else
                                    uploadType="image"
                                    v-model:imageFile="form.cover"
                                />
                                <ps-label-caption
                                    textColor="text-red-500 "
                                    class="mt-2 block"
                                    >{{ errors.cover }}</ps-label-caption
                                >
                            </div>

                            <div>
                                <ps-label class="text-base mb-2"
                                    >{{ $t("core__category_icon_label")
                                    }}<span
                                        class="text-red-800 font-medium ms-1"
                                        >*</span
                                    >
                                </ps-label>
                                <ps-label-title-3 v-if="!category.icon">
                                    {{
                                ...
                                >
                                    ...
                                    <ps-button
                                        type="button"
                                        @click="
                                            replaceIconClicked(category.icon.id)
                                        "
                                        rounded="rounded-full"
                                        shadow="drop-shadow-2xl"
                                        class="-ms-10 mb-2"
                                ...
                                <ps-image-upload
                                    v-else
                                    uploadType="icon"
                                    v-model:imageFile="form.icon"
                                />
                                <ps-label-caption
                                    textColor="text-red-500 "
                                    class="m-2"
                                    >{{ errors.icon }}</ps-label-caption
                                >
                            </div>

                            <div>
                                <ps-label class="text-base mb-2"
                                    >{{ $t("core__category_commission_label")
                                    }}<span
                                        class="text-red-800 font-medium ms-1"
                                        >*</span
                                    >
                                ...
                                <ps-input
                                    ...
                                    :placeholder="
                                        $t(
                                            'core__category_commission_placeholder'
                                        )
                                    "
                                    :maxlength="5"
                                    @keyup="
                                ...
                                <ps-label-caption
                                    textColor="text-red-500 "
                                    class="mt-2 block"
                                    >{{ errors.commission }}</ps-label-caption
                                >
                            </div>

                            <div>
                                <ps-label class="text-base mb-2">{{
                                    $t("core__status_label")
                                }}</ps-label>
                                <ps-checkbox-value
                                    v-model:value="form.status"
                                    class="font-normal"
                                    :title="$t('core__publish_label')"
                                />
                            </div>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const { isEmpty, minLength, isCommissionValid } = useValidators();
        ...
        const validateCommissionInput = (fieldName, fieldValue) => {
            props.errors[fieldName] = !fieldValue
                ? isEmpty(fieldName, fieldValue)
                : isCommissionValid(fieldName, fieldValue);
            console.log(commission.value);
            if (fieldName == "commission") {
                commission.value.isError =
                    props.errors.commission == "" ? false : true;
            }
        };

        let form = useForm({
            name: props.category.name,
            commission: props.category.commission,
        ...
        function replaceImageClicked(id) {
            ...
            ps_action_modal.value.openModal(
                trans("core__be_replace_cat_photo"),
        ...
        function replaceIconClicked(id) {
            ...
            ps_action_modal.value.openModal(
                trans("core__be_replace_cat_icon"),
