<template>
    ...
    <ps-layout>
            ...
            <div class="w-full">
                <splide class="px-4" :options="options" :has-track="false">
                            ...
                            <ps-text-button
                                :class="
                                    categoryId == category.id
                                        ? 'bg-primary-100 rounded-t-md'
                                        : ''
                                "
                                @click="clickCategory(category.id)"
                                class="w-full p-4 cursor-pointer h-full flex flex-row items-center justify-center"
                            >
                                <ps-label> {{ $t(category.name) }}</ps-label>
                                <ps-label
                                    v-if="category?.count != '0'"
                                    class="ms-1 text-xs px-1 py-0.5 rounded-xl bg-blue-500"
                                    textColor="text-white"
                                    >{{ category.count }}</ps-label
                                >
                            </ps-text-button>
                        </splide-slide>
                    </splide-track>
                    <div class="splide__arrows splide__arrows--ltr">
                        ...
                        >
                            <ps-icon
                                textColor="dark_text-feSecondary-200"
        ...
        <ps-table2
            ...
            @FilterOptionshandle="FilterOptionshandle"
        ...
        >
            ...
            <template #searchRight>
                <ps-text-button
                    ...
                    class="flex text-sm items-center text-red-400 dark:text-red-400"
                    padding="py-1 px-4"
                >
                    <ps-icon
                ...
                <ps-icon-button
                    :colors="
                        !showFilter
                            ? ''
                            : 'bg-primary-500 text-white dark:text-secondary-800'
                    "
                    focus=""
                    padding="py-1 px-4"
                    ...
                    >{{ $t("core__be_filter") }}</ps-icon-button
                >
            </template>
            ...
            <template #Filter>
                <!-- category filter -->
                <ps-dropdown
                    @on-click="loadCategory"
                    align=""
                    class="h-10"
                    v-if="
                        !colFilterOptions.filter(
                            (option) => option.key == 'category_id@@name'
                        )[0]?.hidden
                    "
                >
                    <template #select>
                        <ps-dropdown-select
                            :placeholder="$t('core__be_category')"
                            :border="
                                selected_cat !== '' && selected_cat !== 'all'
                                    ? 'border border-indigo-500/100'
                                    : 'border border-1 border-secondary-200'
                            "
                            :selectedValue="
                                selected_cat == '' || selected_cat == 'all'
                                    ? ''
                ...
                <!-- subcategory filter -->
                <ps-dropdown
                    @on-click="subCategoryDropdownClick"
                    class="h-10"
                    v-if="
                        $page.props.isSubCategoryOn == '1' &&
                        !colFilterOptions.filter(
                            (option) => option.key == 'subcategory_id@@name'
                        )[0]?.hidden
                    "
                >
                    <template #select>
                        <ps-dropdown-select
                            :placeholder="$t('core__be_subcategory')"
                            :border="
                                selected_sub_cat !== '' &&
                    ...
                    <template #list>
                        <div class="rounded-md shadow-xs w-56">
                            <div class="pt-2 z-30">
                                ...
                                >
                                    <ps-label class="text-gray-200 ms-2">{{
                                        $t("core__be_select_all")
                                    }}</ps-label>
                                </div>
                                ...
                                >
                                    <ps-label
                                        class="ms-2"
                                        :class="
                                            subcategory.id == selected_sub_cat
                                                ? ' font-bold'
                                                : ''
                                        "
                                    >
                    ...
                    <template #loadmore>
                        <div
                            @click="subCategoryDropdownClick(true)"
                            v-if="subCategory_loadmore_visible"
                            class="w-56 flex py-2 px-2 hover:bg-primary-000 dark:hover:bg-primary-900 cursor-pointer items-center"
                        >
                            ...
                            <div
                                class="flex justify-between rtl:space-x-reverse space-x-4"
                            >
                                <ps-label class="ms-2">
                                    {{
                                        is_loading
                ...
                <ps-dropdown
                    @on-click="cityDropdownClick"
                    align=""
                    class="h-10"
                    v-if="
                        !colFilterOptions.filter(
                            (option) => option.key == 'location_city_id@@name'
                        )[0]?.hidden
                    "
                >
                    <template #select>
                        <ps-dropdown-select
                            :placeholder="$t('core__be_city')"
                            :border="
                                selected_city !== '' && selected_city !== 'all'
                                    ? 'border border-indigo-500/100'
                                    : 'border border-1 border-secondary-200'
                            "
                            :selectedValue="
                    ...
                    <template #loadmore>
                        <div
                            @click="cityDropdownClick(true)"
                            v-if="city_loadmore_visible"
                            class="w-56 flex py-2 px-2 hover:bg-primary-000 dark:hover:bg-primary-900 cursor-pointer items-center"
                        >
                            <!-- <ps-label v-if="is_loading"  class="ms-2 italic underline">
                                {{$t('core__be_loading')}}
                ...
                <ps-dropdown
                    @on-click="townshipDropdownClick"
                    class="h-10"
                    v-if="
                        !colFilterOptions.filter(
                            (option) =>
                                option.key == 'location_township_id@@name'
                        )[0]?.hidden
                    "
                >
                    <template #select>
                        <ps-dropdown-select
                            :placeholder="$t('core__be_township')"
                            :border="
                                selected_township !== '' &&
                                selected_township !== 'all'
                                    ? 'border border-indigo-500/100'
                                    : 'border border-1 border-secondary-200'
                            "
                            :selectedValue="
                            ...
                            <div class="pt-2 z-30">
                                ...
                                <div>
                                    <div
                                        ...
                                        class="w-56 flex py-2 px-2 hover:bg-primary-000 dark:hover:bg-primary-900 cursor-pointer items-center"
                                        @click="
                                            handleTownshipfilter(township.id)
                                        "
                                    >
                                        <ps-label
                                            class="ms-2"
                                            :class="
                                                township.id == selected_township
                                                    ? ' font-bold'
                                                    : ''
                                            "
                                        >
                    ...
                    <template #loadmore>
                        <div
                            @click="townshipDropdownClick(true)"
                            v-if="townships_loadmore_visible"
                            class="w-56 flex py-2 px-2 hover:bg-primary-000 dark:hover:bg-primary-900 cursor-pointer items-center"
                        >
                            <!-- <ps-label v-if="is_loading"  class="ms-2 italic underline">
                                {{$t('core__be_loading')}}
                ...
                <!-- owner filter -->
                <ps-dropdown
                    @on-click="ownersDropdownClick"
                    align=""
                    class="h-10"
                    v-if="
                        colFilterOptions.filter(
                            (option) => option.key == 'added_user_id'
                        )[0]
                            ? !colFilterOptions.filter(
                                  (option) => option.key == 'added_user_id'
                              )[0]?.hidden
                ...
                >
                    <template #select>
                        <ps-dropdown-select
                            :placeholder="$t('core__be_posted_by')"
                            :border="
                                selected_owner !== '' &&
                                selected_owner !== 'all'
                                    ? 'border border-indigo-500/100'
                            ...
                            "
                            :selectedValue="
                                selected_owner == '' || selected_owner == 'all'
                                    ? ''
                                    : selectedOwner.name
                            "
                        />
                    ...
                    <template #loadmore>
                        <div
                            @click="ownersDropdownClick(true)"
                            v-if="owners_loadmore_visible"
                            class="w-56 flex py-2 px-2 hover:bg-primary-000 dark:hover:bg-primary-900 cursor-pointer items-center"
                        >
                            <!-- <ps-label v-if="is_loading"  class="ms-2 italic underline">
                                {{$t('core__be_loading')}}
                ...
                <date-picker
                    v-if="
                        ...
                        colFilterOptions.filter(
                            ...
                            : false
                    "
                    :placeholder="$t('core__be_posted_date')"
                    @datepick="handleAddedDatefilter"
                    class="rounded shadow-sm pt-0.5 focus:outline-none focus:ring-1 focus:ring-primary-500"
                    :class="
                        selected_added_date == null || selected_added_date == ''
                            ? 'w-full'
                            : 'w-full'
                    "
                    v-model:value="selected_added_date"
                    :range="true"
                    :inline="false"
                    :autoApply="false"
                />
                ...
                <date-picker
                    v-if="
                        reRenderDate &&
                        colFilterOptions.filter(
                            (option) => option.key == 'updated_date'
                        )[0]
                            ? !colFilterOptions.filter(
                                  (option) => option.key == 'updated_date'
                              )[0]?.hidden
                            : false
                    "
                    :placeholder="$t('core__be_updated_date')"
                    @datepick="handleUpdatedDatefilter"
                    class="rounded shadow-sm pt-0.5 focus:outline-none focus:ring-1 focus:ring-primary-500"
                    :class="
                        selected_updated_date == null ||
                        selected_updated_date == ''
                            ? 'w-full'
                            : 'w-full'
                    "
                    v-model:value="selected_updated_date"
                    :range="true"
                    :inline="false"
                    :autoApply="false"
                />
            ...
            <template #tableActionRow="rowProps">
                <div class="flex flex-row" v-if="rowProps.field == 'action'">
                    <ps-button
                        :disabled="!rowProps.row.vendorAuthorizations.update"
                        @click="handleEdit(rowProps.row.id)"
                        class="me-2"
                        rounded="rounded-lg"
                        colors="bg-primary-400 text-white"
                        padding="p-1.5"
                    ...
                    <ps-button
                        :disabled="!rowProps.row.vendorAuthorizations.delete"
                        @click="confirmDeleteClicked(rowProps.row.id)"
                        rounded="rounded-lg"
            ...
            <template #tableRow="rowProps">
                <span v-if="rowProps.field == itmPurchasedOption + '@@name'">
                    <ps-badge
                        v-if="rowProps.row[itmPurchasedOption + '@@name']"
                        >{{
                            rowProps.row[itmPurchasedOption + "@@name"]
                        }}</ps-badge
                    >
                </span>
                ...
                <span v-if="rowProps.field == itmItemType + '@@name'">
                    <ps-badge
                        ...
                        v-if="rowProps.row[itmItemType + '@@name']"
                ...
                <span v-if="rowProps.field == itmConditionOfItem + '@@name'">
                    <ps-badge
                        theme="text-red-500 bg-red-100"
                        v-if="rowProps.row[itmConditionOfItem + '@@name']"
                        >{{
                            rowProps.row[itmConditionOfItem + "@@name"]
                        }}</ps-badge
                    >
                </span>
                ...
                <div v-if="rowProps.field == 'status' && reRenderToogle">
                    <ps-toggle
                        ...
                        @click="
                            handlePublish(
                                rowProps.row.id,
                                rowProps.row.vendorAuthorizations.update
                            )
                        "
                    ></ps-toggle>
                ...
                <ps-label v-if="rowProps.field == 'is_sold_out'" class="flex">
                    <ps-label
                        v-if="rowProps.row.is_sold_out == 1"
                        class="mb-2 px-1 py-0.5 text-xs font-semibold rounded whitespace-nowrap"
                ...
                <ps-label v-if="rowProps.field == 'original_price'">
                    {{
                        checkPriceFormat(
                            rowProps.row["currency_id@@currency_symbol"],
                            rowProps.row.original_price
                        )
                    }}
                </ps-label>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const showFilter =
            ...
            props.selectedCity ||
            props.selectedTownship ||
            props.selectedPrice ||
            props.selectedAvailable ||
            props.selectedOwner ||
            props.selectedAddedDate ||
            props.selectedUpdatedDate
                ? ref(true)
                : ref(false);
        ...
        let selected_cat = props.selectedCategory
            ? ref(props.selectedCategory.id)
            : ref("");
        let selected_sub_cat = props.selectedSubcategory
            ? ref(props.selectedSubcategory.id)
            : ref("");
        let selected_city = props.selectedCity
            ? ref(props.selectedCity.id)
            : ref("");
        let selected_township = props.selectedTownship
            ? ref(props.selectedTownship.id)
            : ref("");
        let selected_price = props.selectedPrice
            ? ref(props.selectedPrice)
            : ref("");
        let selected_owner = props.selectedOwner
            ? ref(props.selectedOwner.id)
            : ref("");
        let selected_added_date = props.selectedAddedDate
            ? ref(props.selectedAddedDate)
            : ref("");
        let selected_updated_date = props.selectedUpdatedDate
            ? ref(props.selectedUpdatedDate)
            : ref("");
        let selected_available = props.selectedAvailable
            ? ref(props.selectedAvailable)
            : ref("");
        let selected_customfield = props.selectedCustomfield
            ? ref(props.selectedCustomfield)
            : ref("");
        ...
        if (props.isSubCategoryOn == "1") {
            columns = [
                ...
                {
                    action: "Action",
                    field: "location_township_id@@name",
                    label: "Township",
                    sort: 1,
                    type: "Integer",
                },
        ...
        async function getCategoriesData(offset) {
            ...
            getCategories(offset, catSearch.value, props.authUser.id).then(
                (response) => {
        ...
        function getCitiesData(offset) {
            ...
            is_loading.value = true;
            getCities(offset, citySearch.value, props.authUser.id).then(
                (response) => {
                    if (!response.data.length) {
        ...
        function getOwnersData(offset) {
            ...
            is_loading.value = true;
            getUsers(offset, ownersSearch.value, props.authUser.id).then(
                (response) => {
                    if (!response.data.length) {
        ...
        function customFieldDropdownClick(core_key_id, loadMore = null) {
            ...
            let offset =
                core_key_id in customFields.value
                    ? customFields.value[core_key_id].length
                    : 0;
            ...
            if (offset == 0 || loadMore == true) {
                category_loadmore_visible.value = true;
                getCustomFields(offset, catSearch.value, 1, core_key_id).then(
                    (response) => {
    ...
    created(props) {
        ...
        this.colFilterOptions = this.hideShowFieldForFilterArr.map(
            (columnFilterOption) => {
                return {
    ...
    methods: {
        ...
        FilterOptionshandle(value) {
            router.put(
                route("vendor_item.screenDisplayUiSetting.store"),
                {
                    value,
