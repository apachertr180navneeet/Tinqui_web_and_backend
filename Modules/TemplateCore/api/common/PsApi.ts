...
export default class PsApi {
    // check Null
    static async checkIsEmpty<T extends PsObject<T>, R>(
        data: any
    ...
    // getData
    static async getData<T extends PsObject<T>, R>(
        obj: T,
        url: String
    ): Promise<PsResource<R>> {
                ...
                if (this.isSuccessful(response.status)) {
                    if (Array.isArray(response.data)) {
                        ...
                        psResource.init(
                            ...
                            response.status,
                            "",
                            obj.fromMap(response.data) as any
                        );
                    }
                } else {
                    if (response) {
    ...
    // post Data
    static async postData<T extends PsObject<T>, R>(
        obj: T,
        url: String,
        postData: any
    ): Promise<PsResource<R>> {
    ...
    static async wrapDataFromProps<T extends PsObject<T>, R>(
        obj: T,
        data: any
    ): Promise<PsResource<R>> {
        const psResource: PsResource<R> = new PsResource();
    ...
    // post JsonData
    static async postJsonData<T extends PsObject<T>, R>(
        obj: T,
        url: String,
        postData: any
    ): Promise<PsResource<R>> {
