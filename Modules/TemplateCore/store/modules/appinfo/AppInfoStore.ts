...
export const usePsAppInfoStoreState = defineStore("AppInfoStore", () => {
    ...
    async function loadAppInfo(holder: AppInfoParameterHolder) {
        ...
        const response = await PsApiService.postPsAppInfo<PsAppInfo>(
            new PsAppInfo(),
            userId
        );
    ...
    function isNoPriceSettingOn() {
        return (
            appInfo?.data?.psAppSetting?.SelectedPriceType == PsConst.NO_PRICE
        );
    }
    ...
    function isSubmitButtonShow(
        roleId: number,
        isVerifybluemark: number
    ): boolean {
    ...
    function isFilterLocationOn() {
        return (
            appInfo.data?.mobileSetting?.no_filter_with_location_on_map ==
            PsConst.ONE
        );
    }
    ...
    function isVendorSettingOn() {
        return appInfo.data?.vendorSetting == 1 ? true : false;
    }
