...
import SalePurchaseHistory from "../../../object/SalePurchaseHistory";

export const useProductStore = makeSeparatedStore((key: string) =>
    defineStore(`productStore/${key}`, () => {
        const itemList = reactive<PsResource<AuctionItem[]>>(new PsResource());
        const phoneList = reactive<PsResource<Phone[]>>(new PsResource());
        const item = reactive<PsResource<AuctionItem>>(new PsResource());
        ...
        let id: string = "";
        let paramHolder = reactive<ProductParameterHolder>(
        ...
        function updateProductList(responseData: PsResource<AuctionItem[]>) {
            if (
                itemList != null &&
                itemList.data != null &&
        ...
        async function loadItemList(
            ...
            const responseData =
                await PsApiService.getItemListByUserId<AuctionItem>(
                    new AuctionItem(),
                    limit.value,
                    offset,
                    loginUserId,
            ...
            // console.log("loaditem list ------------>>>",responseData)
        ...
        async function loadAuctionList(
            loginUserId: string,
            holder: AuctionItemParameterHolder
        ) {
            loading.value = true;
            const responseData =
                await PsApiService.getAuctionedProductList<AuctionItem>(
                    new AuctionItem(),
                    loginUserId,
                    holder.toMap()
                );
            ...
            loading.value = false;
            let array = [].concat(responseData.data.slice(0, 20));
            return array;
        }
        ...
        async function loadItem(itemId: string, loginUserId: string) {
            ...
            const responseData = await PsApiService.getItemDetail<AuctionItem>(
                new AuctionItem(),
                itemId,
                loginUserId
            );

            // console.log("item details ------>>>>.>>>>.>.>.", responseData, responseData.data)
        ...
        async function loadAuctionItem(itemId: string, loginUserId: string) {
            loading.value = true;

            const responseData =
                await PsApiService.GetLiveProductDetail<AuctionItem>(
                    new AuctionItem(),
                    itemId,
                    loginUserId
                );
        ...
        async function updateAuctionStatus(
            ...
            const responseData = await PsApiService.updateAuctionItemStatus<{}>(
                {},
                loginUserId,
                { item_id: itemId }
            );
        ...
        async function relatedItem(
            ...
            const responseData =
                await PsApiService.getRelatedProductList<AuctionItem>(
                    new AuctionItem(),
                    loginUserId,
                    productId,
                    categoryId,
        ...
        async function submitItemEntry(
            holder: ItemEntryParameterHolder,
            loginUserId: string
        ): Promise<PsResource<AuctionItem>> {
            loading.value = true;
            const responseData = await PsApiService.postItemEntry<AuctionItem>(
                new AuctionItem(),
                holder.toMap(),
                loginUserId
            );
        ...
        async function postItemImageUpload(
            loginUserId: string,
            imgParentId: string,
            imgType: string,
            ordering: Number,
            ...
            imageFile: any,
            imgId: string
        ): Promise<PsResource<DefaultPhoto>> {
            loading.value = true;

            const returnData =
                await PsApiService.postItemImageUpload<DefaultPhoto>(
                    new DefaultPhoto(),
                    imgParentId,
                    imgType,
                    ordering,
        ...
        async function postItemImageUploadDropzone(
            loginUserId: string,
            images: any
        ): Promise<PsResource<DefaultPhoto>> {
            loading.value = true;
            ...
            const returnData =
                await PsApiService.ps_image_dropzone_upload_url<DefaultPhoto>(
                    new DefaultPhoto(),
                    loginUserId,
                    images
                );
        ...
        async function postReorderItemImage(
            loginUserId: string,
            reorderImages: any[]
        ): Promise<PsResource<ApiStatus>> {
            loading.value = true;

            const status = await PsApiService.postReorderItemImage<ApiStatus>(
                new ApiStatus(),
                loginUserId,
                reorderImages
            );
        ...
        async function postItemVideoUpload(
            img_id: string,
            imgParentId: string,
            videoFile: File,
            loginUserId: String
            ...
            const returnData =
                await PsApiService.postItemVideoUpload<DefaultPhoto>(
                    new DefaultPhoto(),
                    img_id,
                    imgParentId,
                    videoFile,
        ...
        async function postItemVideoThumbUpload(
            imgParentId: string,
            videoThumbFile: File,
            loginUserId: String
        ): Promise<PsResource<DefaultPhoto>> {
            ...
            const returnData =
                await PsApiService.postItemVideoThumbUpload<DefaultPhoto>(
                    new DefaultPhoto(),
                    imgParentId,
                    videoThumbFile,
                    loginUserId
        ...
        function getURLforListByParam(paramHolder: ProductParameterHolder) {
            ...
            if (url == "" && paramHolder.itemLocationTownship != "") {
                url =
                    ...
                    paramHolder.itemLocationTownship;
        ...
        async function userDeleteItem(
            ...
            const returnedData = await PsApiService.deleteItem<ApiStatus>(
                new ApiStatus(),
                loginUserId,
                holder.toMap()
            );
        ...
        async function refleshProductList(
            loginUserId: string,
            holder: ProductParameterHolder
        ) {
            loading.value = true;
            const tempOffset = 0;
            const templimit = itemList?.data?.length ?? 20;
            const responseData =
                await PsApiService.getItemListByUserId<AuctionItem>(
                    new AuctionItem(),
                    templimit,
        ...
        async function postCreditCard(
            ...
            loading.value = true;
            const status = await PsApiService.postSellerCreditCard<ApiStatus>(
                new ApiStatus(),
                holder
            ); //holder?.toMap());
            loading.value = false;
        ...
        async function updateCreditCard(
            ...
            loading.value = true;
            const status = await PsApiService.postCreditCardUpdate<ApiStatus>(
                new ApiStatus(),
                holder
            ); //holder.toMap());
            loading.value = false;
        ...
        function promoteStatus() {
            return (
                item?.data &&
                item?.data.isOwner == PsConst.ONE &&
                item?.data.status == PsConst.ONE &&
                (item?.data.paidStatus == PsConst.ADSNOTAVAILABLE ||
                    item?.data?.paidStatus == PsConst.ADSFINISHED)
            );
        }

        function showPromoteButton(
            isPromoteSuccessful: boolean,
            isAllPaymentDisable: any
        ) {
            return (
                promoteStatus() &&
                isPromoteSuccessful == false &&
                !isAllPaymentDisable
            );
        }
        ...
        function loadProductRelation(coreKeyIds: String[]): any[] {
            ...
            const filteredProductRelation = item?.data?.productRelation?.filter(
                (e) =>
                    coreKeyIds.includes(e.coreKeysId) &&
                    e.selectedValue[0]?.value !== ""
            ) as any[];
        ...
        function isSoldOut(loginUserId: any) {
            return (
                item?.data &&
                item.data.isSoldOut == PsConst.ZERO &&
                loginUserId == item?.data?.user?.userId
            );
        }
        ...
        async function getSalesPurchaseHistory(loginUserId: string) {
            const result =
                await PsApiService.getSalePurchaseHistory<SalePurchaseHistory>(
                    new SalePurchaseHistory(),
                    loginUserId
                );
