...
export const useItemPromotionStoreState = makeSeparatedStore((key: string) =>
    defineStore(`itemPromotionStore/${key}`, () => {
        const reportedReportedItemList = reactive<PsResource<ItemPaidHistory>>(
            new PsResource()
        );
        const paiditem = reactive<PsResource<ItemPaidHistory>>(
            new PsResource()
        );
        ...
        async function postItemPromotion(
            holder: ItemPaidHistoryParameterHolder,
            loginUserId
        ): Promise<PsResource<ItemPaidHistory>> {
            loading.value = true;

            paiditem.data =
                await PsApiService.postItemPromotion<ItemPaidHistory>(
                    new ItemPaidHistory(),
                    loginUserId,
                    holder.toMap()
                );
