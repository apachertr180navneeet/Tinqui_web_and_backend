...
import UserStripeConnect from "@templateCore/object/UserStripeConnect";
...
import walletrechargeParameterHolder from "@templateCore/object/holder/walletrechargeParameterHolder";
...
export const useUserStore = makeSeparatedStore((key: string) =>
    defineStore(`userStore/${key}`, () => {
        ...
        const defaultProfileImage = 'https://tinqui.ch/public/storage/MPC/uploads/default_profile.png'
        ...
        let id: string = "";
        let paramHolder = reactive<UserParameterHolder>(
            new UserParameterHolder()
        );
        let userparamHolder = reactive<UserListParameterHolder>(
            new UserListParameterHolder()
        );
        ...
        async function loadUser(loginUserId: string) {
            ...
            const result = await PsApiService.getUser<User>(
                new User(),
                loginUserId
            );
        ...
        /***********************feb 21********************** */
        async function loadUserWallet(loginUserId: string) {
            loading.value = true;

            const result = await PsApiService.getUserWallet<UserWallet>(
                new UserWallet(),
                loginUserId
            );
        ...
        /************************Stripe COnnect ***********************/
        async function loadUserStripeConnect(loginUserId: string) {
            loading.value = true;

            const result =
                await PsApiService.getUserStripeConnect<UserStripeConnect>(
                    new UserStripeConnect(),
                    loginUserId
                );
        ...
        async function loadUserStripeConnectPayments(loginUserId: string) {
            loading.value = true;

            const result =
                await PsApiService.getUserStripeConnectPayments<UserStripeConnect>(
                    new UserStripeConnect(),
                    loginUserId
                );
        ...
        async function getOnboardUrl(loginUserId: string) {
            loading.value = true;

            const result =
                await PsApiService.getUserStripeOnboardUrl<MolliePaymentUrl>(
        ...
        /*************withdraw requests *****************/
        async function loadUserWalletWithdrawals(loginUserId: string) {
            loading.value = true;

            const result =
                await PsApiService.getUserWalletWithdrawals<WalletWithdrawHistory>(
                    new WalletWithdrawHistory(),
                    loginUserId
                );
        ...
        async function loadOtherUser(
            loginUserId: string,
            holder: UserParameterHolder
        ) {
            loading.value = true;

            const result = await PsApiService.getOtherUser<User>(
                new User(),
                loginUserId,
                holder.toMap()
            );
        ...
        async function postProfileUpdate(holder: any, loginUserId: string) {
            loading.value = true;
            const returnData = await PsApiService.postProfileUpdate<User>(
                new User(),
                holder.toMap(),
                loginUserId
            );
        ...
        async function postImageUpload(
            ...
            await PsApiService.postUserImageUpload<DefaultPhoto>(
                new DefaultPhoto(),
                userId,
                platformName,
                imageFile,
        ...
        async function resetUserSearchList(
            loginUserId: string,
            holder: UserListParameterHolder
        ) {
            offset = 0;
            loading.value = true;

            const responseData = await PsApiService.getUserSearchList<User>(
                new User(),
                limit.value,
                offset,
                loginUserId,
        ...
        async function refleshUserSearchList(
            loginUserId: string,
            holder: UserListParameterHolder
        ) {
            loading.value = true;
        ...
        async function postUserFollow(holder: UserFollowHolder) {
            ...
            const result = await PsApiService.postUserFollow<User>(
                new User(),
                holder.toMap(),
                holder.userId
            );
        ...
        async function blockUser(
            holder: UserBlockParameterHolder,
            loginUserId: String
        ) {
            this.loading.value = true;

            const status = await PsApiService.blockUser<ApiStatus>(
                new ApiStatus(),
                loginUserId,
                holder.toMap()
            );
        ...
        async function blueMarkUser(
            loginId,
            holder: UserBlueMarkParameterHolder
        ) {
            this.loading.value = true;

            const status = await PsApiService.blueMarkUser<ApiStatus>(
                new ApiStatus(),
                loginId,
                holder.toMap()
            );
        ...
        async function postChangePassword(
            holder: ChangePasswordParameterHolder,
            loginUserId: String
        ) {
            loading.value = true;

            const status = await PsApiService.postChangePassword<ApiStatus>(
                new ApiStatus(),
                holder.toMap(),
                loginUserId
            );
        ...
        async function postDeleteUser(holder: UserDeleteItemParameterHolder) {
            ...
            const status = await PsApiService.postDeleteUser<ApiStatus>(
                new ApiStatus(),
                holder.toMap()
            );
        ...
        return {
            ...
            loadUserWallet,
            loadUserName,
            loadUserWalletWithdrawals,
            loadUserStripeConnect,
            loadUserStripeConnectPayments,
            getOnboardUrl,
            defaultProfileImage
        };
    })
