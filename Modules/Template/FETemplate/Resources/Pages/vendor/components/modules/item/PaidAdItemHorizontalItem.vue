<template>
    ...
    <div class="cursor-pointer sm:w-full md:w-full" v-on:click="onClick != null ? onClick(paiditem) : null">
        ...
        <ps-card class="lg:rounded-lg rounded-lg shadow-sm relative z-0" :enabledHover="true">
            ...
            <!-- Profile -->
            <ps-route-link :to="{ name: 'fe_vendor_info', params: { vendorId: paiditem.item?.vendor?.id }}"  v-if="appInfoStore?.isVendorSettingOn() && paiditem.item?.vendor?.id != ''" >
                <div v-if="showProfile" class="pt-1 pb-2 flex items-center justify-between leading-none px-2  rounded-b-xl lg:rounded-b-2xl">
                    <div class="flex flex-row relative items-center no-underline text-feAchromatic-900">
                        ...
                        v-lazy=" { src: $page.props.thumb1xUrl + '/' + paiditem.item?.vendor?.logo?.imgPath, loading: $page.props.sysImageUrl+'/loading_gif.gif', error: $page.props.sysImageUrl+'/default_vendor_shop.png' }">

                        <!-- Bkue Mark -->
                        <!-- <ps-icon v-if="paiditem.item.user.isVerifybluemark == '1'" class="bg-feInfo-500 rounded-full absolute bottom-1 left-6" name="bluemark" textColor="text-feSecondary-50" w="10" h="10" /> -->

                    </div>
                        <!-- Profile Name and Add Date -->
                        <p class="ms-2 mt-1 text-sm flex-grow">
                            <div class=" flex flex-row w-full">
                                <ps-label class=" text-xs" textColor="text-feSecondary-800 dark:text-feSecondary-300" > {{ paiditem.item?.vendor?.name.length > 24 ? paiditem.item?.vendor?.name.slice(0,24)+' ...' : paiditem.item?.vendor?.name }} </ps-label>
                            ...
                            <ps-label class=" font-light text-xs " textColor="text-feSecondary-500 dark:text-feSecondary-500"> {{ paiditem.item?.vendor ? moment(paiditem.item?.vendor.addedDate).format($page.props.dateFormat) : '' }}  </ps-label>
                        </p>
                </div>
            </ps-route-link>
            <ps-route-link v-else :to="{ name: 'fe_other_profile', params: { userId: paiditem.item?.user?.userId }}" >
