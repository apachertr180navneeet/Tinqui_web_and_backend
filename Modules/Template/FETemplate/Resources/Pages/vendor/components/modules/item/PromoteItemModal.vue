    ...
    <div>
        ...
        >
            <template #title>
                ...
                <div class="ms-8 flex flex-row justify-between">
                    <div class="flex flex-wrap">
                        <ps-label class="font-medium text-xl lg:text-3xl me-2">
                            {{ $t("promote_item_modal__promote") }}
                        </ps-label>
                        <ps-label
                            class="font-light text-sm lg:text-xl lg:mt-2.5 mt-1.5"
                        >
                            {{ $t("promote_item_modal__item_promotion") }}
                        </ps-label>
                    </div>
                    <ps-icon
                        @click="isPromoteSuccessful(false)"
                        name="close"
                        class="text-feSecondary-500 dark:text-feAchromatic-50"
                        w="30"
                        h="30"
                    />
                </div>
            ...
            <template #body>
                ...
                <div
                    class="h-80 overflow-y-scroll flex flex-col lg:mt-4 mt-3 px-8"
                >
                    <!-- Start Left Screen -->
                    <div class="">
                        <div class="flex flex-col w-full">
                            <!-- for Ads Start Time -->
                            <ps-label
                                class="font-medium text-sm lg:text-base mb-2"
                            >
                                {{ $t("promote_item_modal__ads_start_date") }}
                            </ps-label>
                            <div class="flex">
                                <div class="">
                                    <ps-date-picker
                                        ref="startDate"
                                        class="z-50"
                                        :pickedDateProps="pickedDate"
                                    />
                                </div>
                                <ps-time-picker
                                    class="bg-fePrimary-50 dark:bg-feAchromatic-800"
                                    v-model:hour="startTimeH"
                                    v-model:min="startTimeM"
                                    v-model:ampm="startTimeAmpm"
                                />
                            </div>
                            <!-- end Ads Start Time -->

                            <ps-label
                                class="font-medium lg:mt-4 mt-3 text-sm lg:text-base"
                            >
                                {{ $t("promote_item_modal__choose_package") }}
                            </ps-label>
                            <div class="flex flex-col w-full" id="r1">
                                <div
                                    v-for="selectData in promotionSetList"
                                    :key="selectData.id"
                                >
                                    <ps-radio-1
                                        :key="selectData.id"
                                    ...
                                    >
                                        <template #title>
                                            <div v-if="selectData.type == 1">
                                                <ps-label
                                                    class="ms-0.5 font-light text-xs lg:text-sm"
                                                    >{{ selectData.days }}
                                                    {{
                                                        $t(
                                                            "promote_item_modal__days"
                                                        )
                                                    }},
                                                    {{
                                                        $t(
                                                            "promote_item_modal__promotion"
                                                        )
                                                    }}</ps-label
                                                >
                                            </div>
                                            <div
                                                v-else-if="selectData.type == 2"
                                            >
                                                <div class="flex flex-row">
                                                    ...
                                                    <ps-label
                                                        class="mt-3 ms-2 font-light text-xs lg:text-sm"
                                                    >
                                                        {{
                                                            $t(
                                                                "promote_item_modal__days"
                                        ...
                                        <template #price>
                                            <div v-if="selectData.type == 1">
                                                <ps-label
                                                    class="font-light text-xs lg:text-sm"
                                                    >{{
            ...
            <template #footer>
                <div class="px-8 flex gap-2 flex-row flex-wrap justify-between">
                    <ps-button
                        ...
                        v-if="appInfoStore.appInfo.data?.stripeEnable == '1'"
                        class="w-1/4 py-3"
                        @click="paymentClicked('STRIPE')"
                        >{{
                            $t("promote_item_modal__pay_with_stripe")
                        }}</ps-button
                    >
        ...
        <ps-error-dialog ref="ps_success_dialog" />
...
export default defineComponent({
    ...
    setup(_, { emit }) {
        ...
        const selected = { id: "1", days: 1, type: 1 };
        const selectedPromotePlan = reactive(selected);

        async function openModal(itemIdStr, amount) {
        ...
        async function loadUserData() {
            ...
            //     appInfoStore.appInfo.data.mobileSetting
            //         .promote_second_choice_day == "0"
            // ) {
            //     promotionSetList[1].empty = "1";
            // } else {
            //     promotionSetList[1].empty = "0";
            //     promotionSetList[1].days = parseInt(
            //         appInfoStore.appInfo.data.mobileSetting
            //             .promote_second_choice_day
            //     );
            // }
            ...
            //     appInfoStore.appInfo.data.mobileSetting
            //         .promote_third_choice_day == "0"
            // ) {
            //     promotionSetList[2].empty = "1";
            // } else {
            //     promotionSetList[2].empty = "0";
            //     promotionSetList[2].days = parseInt(
            //         appInfoStore.appInfo.data.mobileSetting
            //             .promote_third_choice_day
            //     );
            // }
            ...
            //     appInfoStore.appInfo.data.mobileSetting
            //         .promote_fourth_choice_day == "0"
            // ) {
            //     promotionSetList[3].empty = "1";
            // } else {
            //     promotionSetList[3].empty = "0";
            //     promotionSetList[3].days = parseInt(
            //         appInfoStore.appInfo.data.mobileSetting
            //             .promote_fourth_choice_day
            //     );
            // }
        ...
        function stripeClicked() {
            ...
            if (selectedDate < currentDate) {
                ...
                const startDateTimeStr =
                    PsUtils.timeStampToDateString(selectedDate);
                const startDateTimestampStr =
                    PsUtils.getTimeStampDividedByOneThousand(selectedDate);
                if (selectedPromotePlan.type == 1) {
                    if (Number(itemPrice) < 100) {
                        if (selectedPromotePlan.days == 1) {
                            amountStr = (1.99).toString();
                        } else if (selectedPromotePlan.days == 3) {
                            amountStr = (4.99).toString();
                        } else if (selectedPromotePlan.days == 7) {
                            amountStr = (9.99).toString();
                        }
                    } else {
                        let value = Number(itemPrice) * 0.01;
                        amountStr = (
                            selectedPromotePlan.days * value
                        ).toString();
                    }
                    daysStr = selectedPromotePlan.days.toString();
                } else {
                    if (Number(itemPrice) < 100) {
                        amountStr = (customPromoDate.value * 1.99).toString();
                    } else {
                        let value = Number(itemPrice) * 0.01;
                        amountStr = (customPromoDate.value * value).toString();
                    }
                ...
                stripe_payment_modal.value.openModal(
                    () => {
                        const payment =
                            PsConst.PAYMENT_STRIPE_METHOD.toString();
                        doPayment(
                            payment,
                            promoteitemId,
                            amountStr,
        ...
        async function doPayment(
            ...
            promoteParameterHolder.paymentMethodNounce =
            ...
            const paiditem = await provider.postItemPromotion(
                promoteParameterHolder,
                loginUserId
            );
            psloading.value.closeModal();
            psmodal.value.toggle(false);
            // console.log("paiditem", paiditem);
            if (paiditem.status == PsStatus.ERROR) {
                ps_error_dialog.value.openModal(
                    trans("ps_error_dialog__error"),
                    provider.paiditem.message,
                ...
                ps_success_dialog.value.openModal(
                    ...
                    trans("promote_item_modal__pay_success"),
        ...
        function paypalClicked() {
            ...
            if (selectedDate < currentDate) {
                ...
                if (selectedPromotePlan.type == 1) {
                    amountStr = (
                        selectedPromotePlan.days * pricePerDay.value
                    ).toString();
                    daysStr = selectedPromotePlan.days.toString();
                } else {
                    amountStr = (
                        customPromoDate.value * pricePerDay.value
                    ).toString();
                    daysStr = customPromoDate.value.toString();
                }
                ...
                paypal_payment_modal.value.openModal(
                    () => {
                        const payment =
                            PsConst.PAYMENT_PAYPAL_METHOD.toString();
                        doPayment(
                            payment,
                            promoteitemId,
                            amountStr,
                        ...
                        (emailStr) => {
                            ...
                            const paystack = PaystackPop.setup({
                                ...
                                callback: async function (response) {
                                    ...
                                    const payment =
                                        PsConst.PAYMENT_PAY_STACK_METHOD.toString();
                                    doPayment(
                                        payment,
                                        promoteitemId,
                                        amountStr,
        ...
        function offlineClicked() {
            ...
            if (selectedDate < currentDate) {
                ...
                const startDateTimeStr =
                    PsUtils.timeStampToDateString(selectedDate);
                const startDateTimestampStr =
                    PsUtils.getTimeStampDividedByOneThousand(
                        selectedDate
                    ).toString();
                if (selectedPromotePlan.type == 1) {
                    amountStr = (
                ...
                offline_payment_modal.value.openModal(
                    () => {
                        const payment =
                            PsConst.PAYMENT_OFFLINE_METHOD.toString();
                        doPayment(
                            payment,
                            promoteitemId,
                            amountStr,
        ...
        function amountSelector(days) {
            // console.log(days, typeof(days))
            if (Number(itemPrice) < 100) {
                if (Number(days) == 1) {
                    return formatPrice(1.99);
                } else if (Number(days) == 3) {
                    return formatPrice(4.99);
                } else if (Number(days) == 7) {
                    return formatPrice(9.99);
                } else {
                    return formatPrice(Number(days) * 1.99);
                }
            } else {
                let value = Number(itemPrice) * 0.01;
                return formatPrice(Number(days) * value);
            }
        }
