<template>
    <ps-modal ref="psmodal" line="hidden" maxWidth="405px" bodyHeight="488px" :isClickOut='false'
        theme="p-6 rounded-md">
        <template #title>
        </template>
        <template #body>
            <div class=" w-full p-2">
                <div class="w-full flex flex-col">
                    ...
                    <div class="flex justify-between">
                        ...
                        <ps-icon name="close" class="cursor-pointer text-feAchromatic-400"
                            @click.prevent="cancel"></ps-icon>
                    </div>

                    <div class="mt-5">
                        <ps-label class="font-semibold text-center text-lg">{{ $t("review_entry__your_experience")
                            }}</ps-label>
                        <rating-selected ref="rating" class="mt-3 flex justify-center" :grade="0" :maxStars="5"
                            :hasCounter="true" colors="text-feWarning-500" />
                    </div>

                    <ps-label class="mt-4"> {{ $t("review_entry__title") }} </ps-label>
                    <ps-input class="mt-2" type="text" v-bind:placeholder="$t('review_enter__title')"
                        v-model:value="title"></ps-input>

                    <ps-label class="mt-4"> {{ $t("review_entry__description") }} </ps-label>
                    <ps-textarea class=" mt-2" v-bind:placeholder="$t('review_enter__description')" :rows="5"
                        v-model:value="description"></ps-textarea>
                </div>
            </div>
        </template>
        <template #footer>
            <div class="grid grid-cols-4 gap-6">
                <ps-button v-if="ratingProvider.loading.value == false" @click="clicked" textSize="text-xxs lg:text-sm"
                    class="col-span-2"> {{ $t("review_entry__submit") }}</ps-button>
                <ps-button :disabled="true" v-else textSize="text-xxs lg:text-sm" class="col-span-2"> {{
                    $t("review_entry__loading") }}</ps-button>
                <ps-button @click="cancel()" textSize="text-xxs lg:text-sm" class="col-span-2"
                    colors="bg-feAchromatic-50 dark:bg-feAchromatic-800 dark:text-feAchromatic-200 hover:text-feAchromatic-50"
                    border="border border-feAchromatic-300 dark:border-feAchromatic-500"> {{ $t("review_entry__cancel")
                    }} </ps-button>
            </div>
...
export default {
    ...
    setup() {
        ...
        async function clicked() {
            // console.log("ratingr",rating.value.getRating())
            if (rating.value.getRating() > 0) {
                ...
                ps_warning_dialog.value.openModal(
                    trans('review_entry__your_rating'),
                    trans("minimum_review_requirement")
                )
            }
        }
