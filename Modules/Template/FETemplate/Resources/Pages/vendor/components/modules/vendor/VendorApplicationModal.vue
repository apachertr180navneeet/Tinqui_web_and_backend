<template>
    <ps-modal
        ...
        class="h-full"
        bodyHeight="max-h-81"
        theme="p-6 border rounded"
        bgColor="bg-feAchromatic-50 dark:bg-feAchromatic-800"
    >
        <template #title>
            <div class="w-full flex justify-between h-6">
                <ps-label
                    textColor="text-xl font-semibold text-feSecondary-800 dark:text-feAchromatic-50"
                    >{{ $t("core_fe__vendor_application") }}</ps-label
                >
                <ps-icon
                    class="cursor-pointer dark:text-feSecondary-500"
                    name="close"
                    w="24"
                    h="24"
                    @click="psmodal.toggle(false)"
                />
            </div>
        </template>
        <template #body>
            <div class="mt-6">
                <ps-label
                    textColor="text-base font-medium text-feSecondary-800 dark:text-feAchromatic-200"
                    >{{ $t("core_fe__user_name") }}
                    <span class="text-fePrimary-500">*</span></ps-label
                >
                <ps-input
                    :disabled="true"
                    type="text"
                    v-model:value="userStore.user.data.userName"
                    ref="user_name"
                />
            </div>
            <div class="mt-4">
                ...
                <ps-input
                    type="email"
                    v-model:value="holder.email"
                    ref="email"
                    :placeholder="$t('core_fe__email_placeholder')"
                />
            </div>
            <div class="mt-4">
                <ps-label
                    textColor="text-base font-medium text-feSecondary-800 dark:text-feAchromatic-200"
                    >{{ $t("core_fe__vendor_name") }}
                    <span class="text-fePrimary-500">*</span></ps-label
                >
                <ps-input
                    type="text"
                    v-model:value="holder.vendorName"
                    ref="vendor_name"
                    :placeholder="$t('core_fe__vendor_name_placeholder')"
                />
            </div>
            <div class="mt-4">
                <ps-label
                    textColor="text-base font-medium text-feSecondary-800 dark:text-feAchromatic-200"
                    >{{ $t("core_fe__business_document") }}
                    <span class="text-fePrimary-500">*</span></ps-label
                >
                <ps-file-upload
                    v-model:documentFile="holder.document"
                    :title="document"
                    :placeholder="$t('core_fe__or_drop_files')"
                />
            </div>
            <div class="mt-4">
                <ps-label
                    textColor="text-base font-medium text-feSecondary-800 dark:text-feAchromatic-200"
                    >{{ $t("core_fe__cover_letter") }}
                    <span class="text-fePrimary-500">*</span></ps-label
                >
                <ps-textarea
                    rows="4"
                    v-model:value="holder.coverLetter"
                    :placeholder="$t('core_fe__cover_letter_placeholder')"
                />
            </div>
            <ps-label textColor="my-4 text-sm text-feSecondary-800 flex gap-1">
                <span class="dark:text-feAchromatic-200"
                    >{{ $t("core_fe__by_applying_you_agree_to_the") }}
                </span>
                <span
                    @click="privacyPolicy()"
                    class="text-fePrimary-500 cursor-pointer"
                    >{{ $t("core_fe__privacy_policy") }}</span
                >
            </ps-label>
            <div
                v-if="isError"
                class="p-4 mt-2 rounded bg-fePrimary-50 border border-fePrimary-500"
            >
                <ps-label textColor="text-fePrimary-500 font-medium">{{
                    error
                }}</ps-label>
            </div>
        ...
        <template #footer>
            <div class="w-full flex flex-col gap-4">
                <ps-button @click="submit()">{{
                    $t("core__fe_apply")
                }}</ps-button>
                <ps-button
                    colors="hover:text-feAchromatic-50 dark:text-feAchromatic-200"
                    border="border"
                    @click="closeModal()"
                    >{{ $t("core_fe__cancel") }}</ps-button
                >
            </div>
        </template>
    </ps-modal>
    ...
    setup(props) {
        ...
        async function loadInfo() {
            ...
            if (isUpdate.value) {
                ...
                holder.value.coverLetter =
                    vendorStore.vendor.data.vendorApplication.coverLetter;
                // holder.value.document = vendorStore.vendor.data.vendorApplication.document;
                document.value =
                    vendorStore.vendor.data.vendorApplication.document;
            }
        ...
        async function submit() {
            ...
            let response = await vendorApplicationStore.submitVendorApplication(
                loginUserId,
                holder.value.id,
                holder.value.email,
                holder.value.vendorName,
                holder.value.coverLetter,
