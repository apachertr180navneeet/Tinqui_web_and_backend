        ...
        <ps-card class="flex w-full bg-feAchromatic-50 flex-col lg:rounded-lg shadow-sm "
            ...
            <div class="lg:px-0 lg:py-0 px-1 "
                ...
                <div class="h-44 relative z-0">
                    ...
                    <div v-if="!notShowTitle && !isOwner"
                        class="absolute z-10 bg-feAchromatic-50 dark:bg-feSecondary-800 rounded w-10 h-10 flex justify-center items-center right-2 top-2"
                        @click="favouriteClicked(item.id)">
                        <ps-icon v-if="item.isFavourited == '1'"
                            textColor="text-fePrimary-500 dark:text-feAchromatic-50" class="" name="heart" h="24"
                            w="24" />
                        <ps-icon v-else textColor="text-fePrimary-500 dark:text-feAchromatic-50" class=""
                            name="heart-outline" h="24" w="24" />
                    </div>
                    ...
                    <div v-if="item.isDiscount == '1' && item.status == '1'"
                        ...
                        <div v-if="item.isPaid == '1' && item.paidStatus == PsConst.paidItemProgressStatus && item.status == '1'"
                            class="absolute bg-feSuccess-500 rounded px-2 py-1 left-0 top-8">
                    ...
                    <div v-if="appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType != PsConst.NO_PRICE">
                        ...
                        <div :class="['ps-1 flex justify-start', formatPrice(item ? item.originalPrice : '').length > 10 ? 'flex-col' : 'flex-row  items-center']"
                            ...
                            <ps-label class="line-through me-2 font-semibold text-xs "
                                ...
                                {{ item && appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType ==
                                    PsConst.NORMAL_PRICE ?
                                    item.itemCurrency.currencySymbol : '' }}

                                <span
                                    v-if="item && appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType == PsConst.NORMAL_PRICE">{{
                            ...
                            <ps-label class="font-semibold text-xs lg:text-base" textColor="text-fePrimary-500 "> {{
                                item && appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType ==
                                    PsConst.NORMAL_PRICE ?
                                    item.itemCurrency.currencySymbol : '' }} {{ formatPrice(item ? item.highestBidPrice ?
                                        item.highestBidPrice : item.price : '') }}
                            </ps-label>
                        </div>
                        <div class="ps-1" v-else>
                            <ps-label class="font-semibold text-base" textColor="text-fePrimary-500 "> {{ item &&
                                appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType == PsConst.NORMAL_PRICE ?
                                item.itemCurrency.currencySymbol : '' }} {{ formatPrice(item ? item.highestBidPrice ?
                                    item.highestBidPrice : item.originalPrice : '') }}
                            </ps-label>
                        </div>
                    </div>
                    <div :class="showProfile ? 'pb-0' : 'pb-2'" class="ps-1 flex items-center" v-if="!notShowTitle">
                        <ps-icon textColor="text-feSecondary-400 dark_text-feAchromatic-500" name="location-marker"
                            h="14" w="14" class="text-feSecondary-400 me-3" viewBox="0 0 14 14" />
            ...
            <ps-route-link :to="{ name: 'fe_vendor_info', params: { vendorId: item?.vendor?.id } }"
                v-if="appInfoStore?.isVendorSettingOn() && item?.vendor?.id != ''">
                <div v-if="showProfile"
                    class="pt-1 pb-2 flex items-center justify-between leading-none px-2  rounded-b-xl lg:rounded-b-2xl">
                    ...
                    <div class=" flex flex-row w-full">
                        <ps-label class=" text-xs" textColor="text-feSecondary-800 dark:text-feSecondary-300"> {{
                            item?.vendor?.name.length > 24 ? item?.vendor?.name.slice(0, 24) + ' ...' :
                                item?.vendor?.name
                            }} </ps-label>
                        <ps-label class="p-1 ms-2 text-xxs rounded-sm bg-feWarning-500"
                            textColor="text-feAchromatic-50"> {{ $t('core_fe__vendor_indicator') }}</ps-label>
                    </div>

                    <ps-label class=" font-light text-xs " textColor="text-feSecondary-500 dark:text-feSecondary-500">
                        {{ item?.vendor ? moment(item?.vendor.addedDate).format($page.props.dateFormat) : '' }}
                    </ps-label>
                    </p>
                </div>
            </ps-route-link>
            <ps-route-link v-else :to="{ name: 'fe_other_profile', params: { userId: item?.user?.userId } }">
                <div v-if="showProfile"
                    ...
                    <div class="flex flex-row relative items-center no-underline text-feAchromatic-900">
                        ...
                        <!-- Bkue Mark -->
                        <ps-icon v-if="item.user.isVerifybluemark == '1'"
                            class="bg-feInfo-500 rounded-full absolute bottom-1 left-6" name="bluemark"
                            textColor="text-feSecondary-50" w="10" h="10" />

                    </div>
                    <!-- Profile Name and Add Date -->
                    <p class="ms-2 mt-1 text-sm flex-grow">
                        <ps-label class="truncate text-xs" textColor="text-feSecondary-800 dark:text-feSecondary-300">
                            {{ item.user.userName.length > 12 ? item.user.userName.slice(0, 12) + ' ...' :
                                item.user.userName }} </ps-label>
                        <ps-label class=" font-light text-xs "
                            textColor="text-feSecondary-500 dark:text-feSecondary-500"> {{ item ?
                                moment(item.addedDate).format($page.props.dateFormat) : '' }} </ps-label>
                    </p>
                </div>
            </ps-route-link>
        ...
        function formatPrice(value) {
            ...
            if (value.toString() == '0' && appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType == PsConst.NORMAL_PRICE) {
                ...
                if (appInfoStore.appInfo.data?.psAppSetting?.SelectedPriceType == PsConst.PRICE_RANGE) {
                    ...
                    // console.log("format",appInfoStore?.appInfo?.data?.mobileSetting?.price_format)
