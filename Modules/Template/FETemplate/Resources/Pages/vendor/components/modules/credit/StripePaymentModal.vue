    ...
    <ps-modal ref="psmodal" :isClickOut='false'>
        <template #title>
            ...
            <div class="flex flex-col items-center mt-8">
                <ps-label-header-4 class="font-bold"> {{ $t('stripe_credit_card_modal__card_entry') }}
                </ps-label-header-4>
            </div>
        </template>
        <template #body>
        ...
        <template #footer>
            <div class="flex items-center justify-center mb-4">
                <ps-button class="text-center w-60 mx-auto  " @click="registerCard">{{
                    $t('stripe_credit_card_modal__save') }}</ps-button>
                <ps-button class="text-center w-60 mx-auto ms-4" theme="btn-second" @click="actionClicked('no')"> {{
                    $t('stripe_credit_card_modal__cancel') }} </ps-button>
            </div>
        </template>
