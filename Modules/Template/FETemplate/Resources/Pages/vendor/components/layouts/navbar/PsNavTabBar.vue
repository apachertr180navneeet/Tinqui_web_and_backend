<template>
    <nav
        class="language-bar fixed z-50 w-full h-10 flex items-center bg-feSecondary-900"
        :class="topOfPage ? ' ' : 'hidden'"
    >
        <div
            class="xl:w-feLarge lg:w-large w-full flex justify-between mx-4 sm:mx-6 lg:mx-auto"
        >
      ...
      <ps-label v-else textColor="text-feSecondary-200 dark:text-feAchromatic-50" class="text-sm hidden md:block">
        {{ $t("select_your_location") }} 
            ...
            <div v-if="dataReady" class="flex gap-4">
                <div class="english-active-lang">
                    <ps-dropdown h="h-55" align="left">
                        <template #select>
                            <ps-label class="">
                                <button
                                    type="button"
                                    class="inline-flex items-center content-start justify-between w-full leading-5 font-medium text-sm focus:outline-none transition ease-in-out duration-150 btn-focus text-feAchromatic-50"
                                    :id="
                                        $page.props.languages.filter(
                                            (language) =>
                                                language.symbol ==
                                                activeLanguage
                                        )[0].name
                        ...
                        <template #filter>
                            <div>
                                <ps-input-with-left-icon
                                    class="bg-feSecondary-100 dark:bg-transparent shadow-none w-half"
                                    rounded="rounded-none"
                                    theme="border-none  focus:ring-0 focus:ring-transparent text-feSecondary-500 dark:text-feAchromatic-50"
                                    height="h-10"
                                    placeholderColor="placeholder-feSecondary-300"
                                    @keyup="languageSearch(languageKeys)"
                                    v-model:value="languageKeys"
                                    v-bind:placeholder="$t('search_language')"
                                    style="width: 14.5rem"
                                >
                                    <template #icon>
                                        <ps-icon
                ...
                <div class="flex items-center gap-3">
                    <ps-label textColor="text-feAchromatic-50">{{
                        $t("topbar_dark")
                    }}</ps-label>
                    <ps-icon-toggle
                        toggleOffIcon="day"
...
export default defineComponent({
    ...
    setup(props) {
        ...
        // get township list
        const itemLocationTownshipStore =
            useItemLocationTownshipStoreState("navtab-township");
        // appInfoStore.loadAppInfo(appInfoParameterHolder);
        ...
        function languageSearch(e) {
            this.filteredLanguage = usePage().props.languages.filter(
                (language) =>
        ...
        async function loadLocationData() {
            await itemLocationStore.resetItemLocationList(
                loginUserId,
                paramHolder
            );
            ...
            let isSubLocationOn =
                appInfoStore.appInfo.data?.psAppSetting?.isSubLocation;
            ...
            if (
                ...
                localStorage.locationName === undefined
            ) {
            ...
            if (
                ...
                (localStorage.subLocationName === "" ||
                    localStorage.subLocationName === undefined)
            ) {
            ...
            if (window.location.pathname.includes("item-list")) {
                location.replace(
                    window.location.origin +
                        ...
                        `?item_location_id=${localStorage.locationId}&item_location_township_id=${localStorage.subLocationId}&order_by=added_date&order_type=desc&status=1`
                );
            } else {
                window.location.reload();
            }
