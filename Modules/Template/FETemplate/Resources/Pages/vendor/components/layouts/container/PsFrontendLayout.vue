...
<template>
    <Head>
        <link
            ...
            :href="
                ...
                '/' +
                appInfoStore.appInfo.data?.frontendConfigSetting?.frontendIcon
                    ?.imgPath
            "
        />
    </Head>
    ...
    <div v-if="dataReady" :dir="getDir()" class="bg-feAchromatic-50 h-full">
        <div class="w-full h-full flex flex-col min-h-screen single-product">
            <!-- Body -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const loginUserId = ref(
            props.authUser ? props.authUser.id : PsConst.NO_LOGIN_USER
        );
        ...
        router.on("finish", (event) => {
            if (
                localStorage.loginUserId &&
                localStorage.loginUserId != "" &&
                localStorage.loginUserId != null &&
                localStorage.loginUserId != undefined &&
                localStorage.loginUserId != PsConst.NO_LOGIN_USER
            ) {
                if (firebase.apps.length >= 1) {
                    ...
                    if (route().current() == "fe_chat") {
                        ...
                        userRef
                            .child(localStorage.loginUserId)
                            .set(chat_user_presence);
                        // console.log('online');
                    } else {
        ...
        onMounted(async () => {
            ...
            if ("serviceWorker" in navigator) {
                if (messaging) {
                    ...
                    if (appUrl != null && String(appUrl).endsWith("/")) {
                        url = appUrl + "firebase-messaging-sw.js";
                    }
                    //console.log("fronendlayout", url);
                            ...
                            messaging
                                ...
                                .then(async (currentToken) => {
                                    if (currentToken) {
                                        ...
                                        psValueStore.replacedeviceToken(
                                        ...
                                        if (
                                            ...
                                            regiterHolder.platformName =
                                                PsConst.PLATFORM;
                                            regiterHolder.deviceId =
                                                psValueStore.deviceToken;
                                            regiterHolder.loginUserId =
                                                loginUserId.value;
                                            notiStore.registerNotiToken(
                                                regiterHolder
                                            );
                                        } else {
                                            unRegiterHolder.platformName =
                                                PsConst.PLATFORM;
                                            unRegiterHolder.deviceId =
                                                psValueStore.deviceToken;
                                            unRegiterHolder.userId =
                                                loginUserId.value;
                                            notiStore.unRegisterNotiToken(
                                                unRegiterHolder
                                            );
                                        }
                                        psValueStore.replaceshowProfile(
                                            localStorage.showProfile
                                        );
                                        psValueStore.replaceNotiSetting(
                                            localStorage.notiSetting
                                        );
                                    }
                                })
