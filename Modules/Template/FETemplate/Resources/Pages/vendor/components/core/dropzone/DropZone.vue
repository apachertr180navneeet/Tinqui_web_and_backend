    ...
    if (dropRef.value !== null) {
        ...
        myDropzone.value = new Dropzone(dropRef.value, {
            ...
            init: function (file) {
                ...
                this.on("addedfile", function (file) {
                    all_files.value.push(file);
                    var caption = file.caption == undefined ? "" : file.caption;
                    file._captionLabel = Dropzone.createElement(
                        "<div class='flex mt-6'><p>File Info:</p>"
                    );
                    if (myDropzone.value.getAcceptedFiles().length == 0) {
                        const newElement = document.createElement("div");
                        newElement.innerHTML =
                            '<label for="image" class="image-label text-xxs" id="defaultText">Default</label>';
                        file.previewElement.firstElementChild.appendChild(
                            newElement
                        );
                    ...
                    file.previewElement.querySelector(
                        "[data-dz-name]"
                    ).textContent = file.upload.filename;

                    file._captionBox.name = file.filename;
                    file._captionBox.onkeyup = function () {
                        myMethod();
                    };
                    emit(
                        "fileLength",
                ...
                this.on("removedfile", function (file) {
                    // console.log(file);
                    myDropzone.value.files = myDropzone.value.files.filter(
                        function (el) {
                    ...
                    if (myDropzone.value.files[0] != undefined) {
                        ...
                        defaultlabel.innerHTML =
                            '<label for="image" class="image-label text-xxs" id="defaultText">Default</label>';
                        myDropzone.value.files[0].previewElement.firstElementChild.appendChild(
                            defaultlabel
                        );
        ...
        Sortable.create(document.getElementById("dropzone"), {
            ...
            onEnd: function (evt) {
                ...
                const dropzoneElements = document.querySelectorAll(
                    ".dropzone .dz-preview"
                );
                const dropzoneElementsOrder = document.querySelectorAll(
                    ".dropzone .dz-preview "
                );

                dropzoneElements.forEach(function (el, count) {
                    var name = el.querySelector(
                        ".dz-filename [data-dz-name]"
                    ).innerHTML;
                    ...
                    queue.every(function (file, index) {
                        if (file.name === name) {
                            ...
                            newOrder.push({
                                id: file.upload.uuid,
                                order: file._order.value,
                            });

                            const referenceElement =
                                document.querySelector("#defaultText");
        ...
        if (props.images) {
            const sortedImages = props.images.sort(
            ...
            sortedImages.forEach((element) => {
                ...
                let mockFile = {
                    name: element.imgPath,
                    filename: element.imgPath,
                    caption: element.imgDesc,
                    upload: { filename: element.imgPath, uuid: element.imgId },
                    size: 12345,
                    accepted: true,
                };
