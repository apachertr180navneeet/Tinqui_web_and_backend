    ...
    <div>
        <div class="sm:mt-32 lg:mt-36 mt-28 flex flex-col">
            <div class="content-center mx-auto w-96">
                <ps-label
                    class="content-center bg-feAchromatic-50 text-general-200 px-4 py-4 rounded-md"
                    v-if="userProvider.userResource.message != ''"
                >
            ...
            <div class="w-55 lg:w-80 flex flex-col mx-auto rounded-lg">
                <!-- Register Title -->
                <ps-label
                    class="font-medium text-2xl mt-8 mx-auto lg:text-3xl mb-4"
                >
                    {{ $t("register__register_title") }}
                </ps-label>

                <!-- User Entry -->
                <form class="w-full">
                    <ps-label
                        class="mt-4 lg:mt-5 font-medium text-base lg:text-xl w-full"
                        >{{ $t("register__user_name") }}
                    </ps-label>
                    <ps-input
                        class="lg:mt-2 mt-1 w-full"
                        type="email"
                        v-bind:placeholder="$t('register__user_name')"
                    ...
                    <ps-input
                        class="lg:mt-2 mt-1 w-full"
                        type="email"
                        v-bind:placeholder="$t('register__email')"
                        v-on:keyup.enter="clicked"
                        v-model:value="email"
                        @blur="validateEmail"
                    ></ps-input>
                    ...
                    >
                        {{ $t("login__email_required_error") }}
                    </ps-label>
                    <ps-label
                        class="mt-4 lg:mt-5 font-medium text-base lg:text-xl w-full"
                    >
                        {{ $t("register__password") }}
                    </ps-label>
                    <ps-input
                        class="lg:mt-2 mt-1 w-full"
                        type="password"
                        v-bind:placeholder="$t('register__password')"
                        v-on:keyup.enter="clicked"
                    ...
                    <ps-label
                        class="mt-4 lg:mt-5 font-medium text-base lg:text-xl w-full"
                    >
                        {{ $t("register__confirm_password") }}
                    </ps-label>
                    <ps-input
                        class="lg:mt-2 mt-1 w-full"
                        type="password"
                        v-bind:placeholder="$t('register__confirm_password')"
                        v-on:keyup.enter="clicked"
                ...
                <ps-button
                    ...
                    class="mt-4 lg:mt-5 w-full py-3"
                    v-if="userProvider.loading.value"
                    :disabled="true"
                >
                    {{ $t("register__loading") }}
                </ps-button>
                ...
                <div class="flex flex-auto justify-center mt-3">
                    <ps-label
                        textColor="text-feSecondary-400 dark:text-feAchromatic-500"
                        class="cursor-pointer font-light text-xs lg:text-sm"
                        @click="loginClicked"
                    >
                        {{ $t("register__already_member_login") }}
                    </ps-label>
                </div>

                <div class="flex flex-row justify-center mt-4">
                    <ps-line
                        class="w-full lg:h-3 h-2 border border-s-0 border-e-0 border-feSecondary-400 dark:border-feAchromatic-500"
                    />
                    <ps-label class="px-1 font-medium text-sm lg:text-base">
                        {{ $t("login__or") }}
                    </ps-label>
                    <ps-line
                        class="w-full lg:h-3 h-2 border border-s-0 border-e-0 border-feSecondary-400 dark:border-feAchromatic-500"
                ...
                <ps-button
                    textSize="text-xs lg:text-base"
                    class="mt-3 lg:mt-4 py-3 flex justify-center"
                    @click="phoneloginclicked"
                >
                ...
                <ps-button
                    textSize="text-xs lg:text-base"
                    class="mt-3 lg:mt-4 py-3 flex justify-center"
                    theme="bg-feInfo-600 hover:bg-feInfo-700 text-feAchromatic-50 px-4 py-2"
                ...
                <ps-button
                    textSize="text-xs lg:text-base"
                    class="mt-3 lg:mt-4 py-3 flex justify-center mb-2"
                    theme="bg-feWarning-600 hover:bg-feWarning-700 text-feAchromatic-50 px-4 py-2"
                    id="signinBtn"
                ...
                <ps-button
                    textSize="text-xs lg:text-base"
                    class="mt-3 lg:mt-4 py-3 flex justify-center mb-5"
                    theme="bg-feAchromatic-900 hover:bg-feAchromatic-700 text-feAchromatic-50 px-4 py-2"
                    @click="appleloginclicked"
                >
        ...
        async function clicked() {
            ...
            if (password.value == confirmPassword.value) {
                if (checkedterms.value == true) {
                    await userProvider.signUpWithEmailId(
                        name.value,
                        email.value,
                        password.value,
                        () => {
