        ...
        <template #content>
            <div class="mt-32 mb-10">
                ...
                <div class="flex justify-between mt-6">
                    <ps-label
                        textColor="text-feSecondary-800 text-2xl font-semibold"
                        >{{ $t("core_fe__my_vendors") }}</ps-label
                    >
                    <ps-button @click="addNewVendor()">{{
                ...
                <div class="mt-6">
                    <div
                        class="w-full"
                        v-if="vendorStore.vendorList.data != null"
                    >
                        <div
                        ...
                        >
                            <div
                                class="w-full"
                                v-for="(vendor, index) in vendorStore.vendorList
                                    .data"
                                :key="index"
                            >
                        ...
                        <div class="flex flex-col items-center">
                            <ps-button
                                v-if="vendorStore.loading.value == false"
                                class="font-medium mx-auto mt-6"
                                @click="loadMore"
                                :class="
                                    vendorStore.isNoMoreRecord.value
                                        ? 'hidden'
                                        : ''
                                "
                            >
                                {{ $t("follower_list__load_more") }}
                            </ps-button>
                            <ps-button
                                v-else
                                class="font-medium mx-auto mt-6"
                                ...
                                :disabled="true"
                            >
                                {{ $t("follower_list__loading") }}
                            </ps-button>
                        </div>
                    </div>
                    <ps-no-result
                        v-if="
                            vendorStore.loading.value == false &&
                            vendorStore.vendorList?.data == null
                        "
                        @onClick="loadMore"
                    />
                </div>
