<template>
    <ps-card
        class="lg:rounded-lg bg-feSecondary-50 dark:bg-feSecondary-800 p-4"
    >
        <div class="w-32 h-32 rounded-full mx-auto relative">
            ...
            />
            <div
                v-if="userStore.user.data?.isVerifybluemark == '1'"
        ...
        <ps-label
            textColor="dark:text-feSecondary-200"
            class="text-center text-xl font-semibold mb-2 mt-2"
        >
        ...
        <ps-route-link
            :to="{
                name: 'fe_review_list',
                query: { user_id: userStore.user.data?.userId },
            }"
        >
            <rating
                class="flex items-center content-center justify-center px-4 mb-3 cursor-pointer"
                :rating="
                    userStore.user.data
                        ? Number(userStore.user?.data?.overallRating)
                        : 0
                "
                :maxStars="5"
                size="md"
        ...
        <ps-label
            textColor="text-feSecondary-600 dark:text-feSecondary-400"
            class="text-sm mb-3 text-center"
        >
            {{ userStore.user.data ? userStore.user.data.ratingCount : "" }}
            {{ $t("profile__reviews") }}
        </ps-label>
        ...
        <ps-label
            textColor="text-feSecondary-500 dark:text-feSecondary-400"
            class="text-xs mb-3 text-center"
            >{{ $t("profile__joined_on") }}
            {{
                userStore.user.data
                    ? getDateFormat(userStore.user.data.addedDate)
                    : ""
            }}</ps-label
        >

        <ps-route-link
            class="cursor-pointer mb-2 flex justify-between"
            :to="{
                name: 'fe_follower_list',
                query: { userId: userStore.user.data?.userId },
            }"
        >
            <ps-label class="text-sm"
                >{{ $t("other_profile__followers") }}
            </ps-label>
            <ps-label class="font-semibold text-sm">{{
                userStore.user.data ? userStore.user.data.followerCount : "0"
            }}</ps-label>
        </ps-route-link>
        <ps-route-link
            class="cursor-pointer flex justify-between"
            :to="{
                name: 'fe_following_list',
                query: { userId: userStore.user.data?.userId },
            }"
        >
            <ps-label class="text-sm"
                >{{ $t("other_profile__followings") }}
            </ps-label>
            <ps-label class="font-semibold text-sm">{{
                userStore.user.data ? userStore.user.data.followingCount : "0"
            }}</ps-label>
        </ps-route-link>
    </ps-card>
</template>
