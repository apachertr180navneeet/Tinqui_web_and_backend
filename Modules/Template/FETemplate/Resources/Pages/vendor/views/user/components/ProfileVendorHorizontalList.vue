<template>
    <div v-if="alreadyVendor">
        ...
        <div
            class="w-full mt-6 grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-5 md:gap-6 mb-10"
        >
            <div
                v-for="(vendor, index) in vendorStore.vendorList.data?.slice(
                    0,
                    3
                )"
                :key="index"
    ...
    <vendor-application-modal
        ref="vendor_application_modal"
        storeName="profile-vendor"
    />
</template>
...
export default defineComponent({
    ...
    setup() {
        ...
        onMounted(async () => {
            ...
            alreadyVendor.value =
                vendorStore.vendorList.data.length == 0 ? false : true;
        });
