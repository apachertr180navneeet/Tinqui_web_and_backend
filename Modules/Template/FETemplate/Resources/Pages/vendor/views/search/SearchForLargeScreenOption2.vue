<template>
    <div
        class="w-full lg:rounded-lg sm:rounded-xl rounded-2xl sm:flex mt-4 sm:mt-0 gap-5"
    >
        <div class="flex gap-x-5 flex-col md:flex-row gap-y-2">
            <ps-input-with-left-icon
                theme="dark:text-feSecondary-200"
                placeholderColor=""
                rounded="rounded"
                defaultBorder="bg-feSecondary-50 dark:bg-feSecondary-800 border border-feAchromatic-50 hover:border-feSecondary-400 
                    dark:border-feSecondary-800 hover:dark:border-feSecondary-50 focus:outline-none focus:border-none focus:ring-2 focus:ring-fePrimary-300 ring-fePrimary-300 placeholder-feSecondary-500 dark:placeholder-feSecondary-200"
                padding="h-10"
                v-on:keyup.enter="searchClicked"
                v-model:value="searchKeyword"
                :placeholder="$t('dashboard__enter_keyword')"
            >
                <template #icon>
                    <ps-icon
                        textColor="text-feSecondary-500 dark:text-feSecondary-200"
                        name="search"
            ...
            <div class="md:block hidden">
                <ps-label
                    textColor="text-2xl mt-1 font-medium text-feSecondary-100 dark:text-feSecondary-50"
                    >{{ $t("|") }}</ps-label
                >
            </div>
            ...
            <ps-dropdown
                h="h-64"
                align="left"
                class="w-full md:w-52 h-full lg:rounded-2xl rounded"
                @onClick="loadSubLocation()"
            >
                <template #select>
                    <ps-label class="lg:rounded-2xl rounded-xshadow-sm h-full">
                        <button
                            type="button"
                            class="focus:ring-2 ring-fePrimary-200 inline-flex items-center content-start justify-between w-full h-10 lg:rounded rounded-x px-4"
                            aria-haspopup="true"
                            aria-expanded="true"
                        >
                            <ps-label
                ...
                <template #filter>
                    <div>
                        <ps-input-with-left-icon
                            class="bg-feSecondary-100 dark:bg-transparent shadow-none"
                            rounded="rounded-none"
                            theme="border-none focus:ring-0 focus:ring-transparent text-feSecondary-500 dark:text-feAchromatic-50"
                            height="h-10"
                            @keyup.capture="
                                filtersubLocationUpdate(
                                    categoryStore.filterKeyword
                                )
                            "
                            v-model:value="categoryStore.filterKeyword"
                            v-bind:placeholder="$t('search_categories')"
                        >
                            <template #icon>
                                <ps-icon
                                    textColor="text-fePrimary-500 dark:text-feAchromatic-500"
                                    name="search"
                                    class="cursor-pointer"
                                />
                            </template>
                            ...
                            <template #removeIcon>
                                <div v-if="categoryStore.filterKeyword != ''">
                                    <ps-icon
                                        textColor="text-feAchromatic-500 dark:text-feAchromatic-500"
                                        @click="
                                            [
                                                (categoryStore.filterKeyword =
                                                    ''),
                ...
                <template #list>
                    <div class="pt-2 z-30">
                        <div
                            v-if="categoryStore.loading.value == true"
                            class="mt-4 ms-4 mb-4 flex"
                        >
                            <ps-label-caption>
                                {{ $t("search_for_large_screem__loading") }}
                            </ps-label-caption>
                        </div>
                        <div
                            v-if="categoryStore.itemList.data == null"
                            class="w-56 flex py-4 px-2 items-center"
                        >
                            <ps-label
                                class="ms-2"
                                :class="
                        ...
                        <div
                            v-else
                            class="w-56 flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-feAchromatic-800 cursor-pointer items-center"
                            @click="
                                categoryFilterClicked('', $t('item_list__all'))
                            "
                        >
                            <ps-label
                                class="ms-2"
                ...
                <template #loadmore>
                    <div class="mb-2 w-56">
                        ...
                        <div
                            v-if="
                                categoryStore.itemList.data != null &&
                                categoryStore.isNoMoreRecord.value == false
                            "
                            class=""
                        >
                            <ps-label
                                class="flex mt-4 ms-4 mb-2 underline font-bold cursor-pointer"
                                @click="loadSubLocation(categoryId)"
                            >
                                {{ $t("search_for_large_screem__load_more") }}
                            </ps-label>
                        </div>
                    </div>
                </template>
            ...
            >
                ...
                <template #filter>
                    <div>
                        <ps-input-with-left-icon
                            class="bg-feSecondary-50 dark:bg-transparent shadow-none"
                            rounded="rounded-none"
                            theme="border-none  focus:ring-0 focus:ring-transparent text-feSecondary-500 dark:text-feAchromatic-50"
                            height="h-10"
                            @keyup.capture="
                                filterKeywordUpate(
                                    itemLocationStore.filterKeyword
                                )
                            "
                            v-model:value="itemLocationStore.filterKeyword"
                            v-bind:placeholder="$t('search_location')"
                        >
                            <template #icon>
                                <ps-icon
                                    textColor="text-fePrimary-500 dark:text-feAchromatic-500"
                            ...
                            <template #removeIcon>
                                <div
                                    v-if="itemLocationStore.filterKeyword != ''"
                                >
                                    <ps-icon
                                        textColor="text-feAchromatic-500 dark:text-feAchromatic-500"
                                        @click="
                ...
                <template #list>
                    <div class="pt-2 z-30">
                        <div
                            v-if="itemLocationStore.loading.value == true"
                            class="w-56 flex py-4 px-2 items-center"
                        >
                            <ps-label-caption class="ms-2">
                                {{ $t("search_for_large_screem__loading") }}
                            </ps-label-caption>
                        </div>
                        <div
                            v-if="
                                itemLocationStore.itemLocationList.data == null
                            "
                            class="w-56 flex py-4 px-2 items-center"
                        >
                            <ps-label
                        ...
                        <div
                            v-for="selectData in itemLocationStore
                ...
                <template #loadmore>
                    <div class="w-56">
                        <div
                            v-if="
                                itemLocationStore.itemLocationList.data !=
                                    null &&
                                itemLocationStore.loading.value == true
                            "
                        ...
                        <div
                            v-if="
                                itemLocationStore.itemLocationList.data !=
                                    null &&
                                itemLocationStore.isNoMoreRecord.value == false
                            "
                            class=""
                        >
                            <ps-label
                                class="flex py-4 px-2 underline font-bold cursor-pointer ms-2"
                                @click="loadLocationList"
                            >
                                {{ $t("search_for_large_screem__load_more") }}
                            </ps-label>
                        </div>
            ...
            <div class="flex justify-center">
                <ps-button
                    v-bind:title="$t('search_for_large_screem__search')"
                    class=""
                    rounded="rounded-3xl"
                    theme="bg-fePrimary-500"
                    @click="searchClicked()"
                >
                    <ps-icon
                        textColor="text-feAchromatic-50 dark:text-feSecondary-50"
                        name="search"
                        h="24"
                        w="24"
                        class="cursor-pointer"
...
export default {
    ...
    setup(props, context) {
        ...
        const subparamHolder =
            new CategoryListParameterHolder().CategoryListParameterHolder();
        ...
        function loadLocationList() {
            itemLocationStore.loadItemLocationList(
        ...
        function filterKeywordUpate(value) {
            ...
            timer = window.setTimeout(async () => {
                await itemLocationStore.filterKeywordUpate(
                    value,
                    loginUserId,
                    paramHolder
                );
        ...
        function filtersubLocationUpdate(value) {
            ...
            timer = window.setTimeout(async () => {
                await categoryStore.resetCategoryList(
                    loginUserId,
                    subparamHolder
                );
            }, 400);
