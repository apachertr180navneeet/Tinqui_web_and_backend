    ...
    <div v-if="productStore.item?.data" class="mt-6 px-2 py-6 lg:p-4 bg-feSecondary-50 dark:bg-feSecondary-800 rounded-lg">
        <div class="flex flex-col gap-2 mb-6">
            ...
            <ps-label v-if="appInfoProvider?.appInfo?.data.psItemUploadConfig?.address == PsConst.ONE"
                class="text-feSecondary-800 dark:text-feSecondary-200 font-normal text-sm">{{productStore.item?.data?.itemLocationTownship?.townshipName?productStore.item?.data?.itemLocationTownship?.townshipName+",":""
                }} {{
                productStore.item?.data?.itemLocation.name }} </ps-label>
        </div>
    
        <map-with-pin-picker v-if="appInfoProvider?.appInfo?.data.frontendConfigSetting.googleMap == PsConst.ONE"
            :lat="parseFloat(productStore.item?.data?.lat + '0')" :lng="parseFloat(productStore.item?.data?.lng + '0')"
            :draggable="false" />
    
        <open-street-map :mapContainer="mapContainer" :dir="$page.props.dir"
            v-if="appInfoProvider?.appInfo?.data.frontendConfigSetting.openStreetMap == PsConst.ONE" mapSize="h-68"
            :lat="parseFloat(productStore.item?.data?.lat + '0')" :lng="parseFloat(productStore.item?.data?.lng + '0')"
            :dragValue="false" />
    </div>
</template>
