<template>
    <div
        class="flex flex-col justify-center items-center sm:mt-32 lg:mt-36 mt-28 w-mobile sm:w-median lg:w-large mx-auto">
        <!-- Item entry title -->
        <div class="mb-5">
    ...
    <div class="flex flex-col py-6 ps-5 pe-6 sm:ps-16 md:ps-8 sm:pe-6 md:pe-8 lg:pe-7 lg:ps-8 w-mobile sm:w-median lg:w-large mx-auto border-2 rounded-md mb-6"
        v-if="
            appInfoStore?.isVendorSettingOn() &&
            customFieldStore.customField.data?.vendorList.length > 0
        ">
        <ps-label class="text-base" :textColor="itemId == '0'
                ? 'text-feSecondary-800 dark:text-feSecondary-300'
                : 'text-feSecondary-300'
            ">{{ $t("core_fe__upload_as_vendor") }}</ps-label>
        <div class="mb-4">
            <ps-dropdown align="left" :disabled="itemId != '0'" class="lg:mt-2 mt-1 w-full">
                <template #select>
                    <button :disabled="itemId != '0'"
                        class="w-full cursor-pointer flex flex-row justify-between px-4 py-1.5 items-center rounded-md border border-feSecondary-200">
                        <div v-if="defaultProfileId == ''" class="">
                            <ps-label class="text-sm ms-2" :textColor="itemId == '0'
                                    ? 'text-feSecondary-800 dark:text-feSecondary-300'
                                    : 'text-feSecondary-300'
                                ">
                                {{ $t("core_fe_select_profile") }}
                            </ps-label>
                        </div>
                        <div v-else-if="defaultProfileType == 'user'" class="flex flex-row">
                            <img v-lazy="{
                                src:
                                    ...
                                    '/' +
                                    userStore.user?.data?.userCoverPhoto,
                                loading:
                        ...
                        <div v-else-if="defaultProfileType == 'vendor'" class="flex flex-row">
                            <img v-lazy="{
                                src:
                                    $page.props.thumb1xUrl +
                                    '/' +
                                    customFieldStore.customField.data?.vendorList.filter(
                                        (vendor) =>
                                            vendor.id == defaultProfileId
                                    )[0]?.logo.imgPath,
                                loading:
                                    $page.props.sysImageUrl +
                                    '/loading_gif.gif',
                ...
                <template #list>
                    <div class="rounded-md shadow-xs w-80 lg:w-96">
                        <div class="pt-2 z-30">
                            <div>
                                <div class="w-full flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                    @click="
                                        chooseProfile(
                                            userStore.user?.data?.userId,
                                    ...
                                    <ps-label class="ms-2" :class="userStore.user?.data?.userName ==
                                            paramHolder1.vendorName
                                            ? ' font-bold'
                                            : ''
                                        ">
                                ...
                                <div v-for="vendor in customFieldStore
                                    .customField.data?.vendorList" :key="vendor.id"
                                    ...
                                    <ps-label class="ms-2" :class="vendor.id == paramHolder1.vendorId
                                            ? ' font-bold'
                                            : ''
                                        ">
                                        {{ vendor.name }}
            ...
            <div class="flex flex-row">
                <div class="me-2 flex items-center">
                    <input :disabled="itemId != '0'" type="checkbox" class="rounded border" value="0"
                        v-model="vendorAutoChoose" :class="itemId == '0'
                                ? 'text-fePrimary-500'
                                : 'text-feSecondary-300'
                            " />
                    <ps-label class="ms-2" :textColor="itemId == '0'
                            ? 'text-feSecondary-800 dark:text-feSecondary-300'
                            : 'text-feSecondary-300'
                        ">{{
                            $t("core_fe__always_choose_this_profile")
                        }}</ps-label>
                </div>
            </div>
        </div>
    ...
    <div class="flex flex-col w-mobile sm:w-median lg:w-large mx-auto border-2 rounded-md mb-6"
        v-if="itemId == '0' || dataReady == true">
        <div class="w-full grid grid-cols-12 gap-4 sm:gap-3.5 lg:gap-4">
            <div v-if="itemId == '0'"
                class="col-span-12 mt-8 lg:px-8 md:px-8 md:col-start-1 sm:col-start-2 px-6 w-full">
                <!-- for category dropdown -->
                <div class="w-full me-6 sm:me-2 mb-4 sm:mb-0" @click="showDialog" v-for="(
                        coreField, index
                    ) in customFieldStore.customField.data?.coreList.filter(
                            (coreField) =>
                                coreField.fieldName === 'category_id' &&
                                coreField.isVisible === '1'
                        )" :key="index">
                    <ps-label class="text-base">{{ $t(coreField.labelName)
                        }}<span v-if="(coreField.mandatory = 1)"
                            class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                    <ps-dropdown align="left" class="lg:mt-2 mt-1 w-full" @onClick="loadCategory">
                        <template #select>
                            <ps-dropdown-select :showCenter="true" :selectedValue="$t(paramHolder1.catName)"
            ...
            <div v-else class="col-span-12 mt-8 lg:px-8 md:px-8 md:col-start-1 sm:col-start-2 px-6 w-full">
                <!-- for category dropdown -->
                <div class="w-full me-6 sm:me-2 mb-4 sm:mb-0" v-for="(
                        coreField, index
                    ) in customFieldStore.customField.data?.coreList.filter(
                            (coreField) =>
                                coreField.fieldName === 'category_id' &&
                    ...
                    <ps-dropdown align="left" class="lg:mt-2 mt-1 w-full" :disabled="true">
                        <template #select>
                            <ps-dropdown-select :disabled="true" :showCenter="true"
                                :selectedValue="$t(paramHolder1.catName)" :placeholder="$t(categoryName)" />
                        </template>
                    </ps-dropdown>
                </div>
                <!-- end category -->
        ...
        <div class="w-full grid lg:grid-cols-12 sm:grid-cols-8 grid-cols-4 gap-4 sm:gap-3.5 lg:gap-4 mt-2">
            ...
            <div class="col-span-12 mt-4 lg:px-8 md:px-8 md:col-start-1 sm:col-start-2 px-6 w-full">
                ...
                <div class="flex flex-auto">
                    <ps-label class="font-medium text-sm lg:text-base">
            ...
            <div class="col-span-12 mt-4 lg:px-8 md:px-8 md:col-start-1 sm:col-start-2 px-6 w-full" v-if="galleryLoad">
                <Dropzone :dir="$page.props.dir" @clicked="pushImage" @removeImage="removeImage" @caption="caption"
                    @maxfilesexceeded="maxUpload" ref="dropzone_ref" @queue-complete="queueComplete"
                    @file-length="getFileCount" :images="itemId == 0 ? '' : gallery.data"
                    :edit_mode="itemId == 0 ? 0 : 1" :item_id="itemId == 0 ? 0 : itemId" :max_image_upload="appInfoStore?.appInfo?.data?.psAppSetting
                            ?.maxImgUploadOfItem
            ...
            <!-- Start for video -->
            <div v-if="
                appInfoStore?.appInfo?.data?.mobileSetting
                    .is_show_item_video == '1'
            " class="col-span-12 mt-4 lg:px-8 md:px-8 md:col-start-1 sm:col-start-2 px-6 w-full">
                <div class="flex flex-auto">
                    <ps-label class="text-sm lg:text-base mt-14 sm:mt-0 relative">
                        {{ $t("item_entry__video") }}
                        {{ $t("item_entry__must_be_less_than") }}
                        {{ videoDurationString }})
                    </ps-label>
                </div>
                    ...
                    <div class="mt-2 w-20 h-20 relative cursor-pointer" for="upload-video">
                        ...
                        <div @click="onVideoClick"
                            ...
                            <img v-if="previewVideo.data == ''" alt="Placeholder" width="300px" height="300px"
                                class="rounded-xl w-20 h-20 object-cover" :src="getVideoUrl() == ''
                                        ? $page.props.sysImageUrl +
                                        '/default_photo.png'
                                        : getVideoUrl()
                                    " />
                            <img v-else v-for="video in previewVideo.data" alt="Placeholder" width="300px"
                                height="300px" class="rounded-xl w-20 h-20 object-cover" :key="video" v-lazy="video" />
                        </div>
                        <div @click="videoDelete()" v-if="
                            previewVideo.data != '' || getVideoUrl() != ''
                        "
                            class="cursor-pointer relative mt-14 ms-11 bg-fePrimary-50 rounded-full dark:bg-feAchromatic-800 py-1.5 px-2.5">
                            <ps-icon name="trash-alt"
                                class="text-feSecondary-500 dark:text-feAchromatic-50 lg:text-base text-sm" />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full grid grid-cols-12 gap-4 sm:gap-3.5 lg:gap-4">
            ...
            <div class="col-span-12 mt-4 lg:px-8 md:px-8 md:col-start-1 sm:col-start-2 px-6 w-full">
                <div class="w-full">
                    <!-- title-->
                    <div class="mb-4" v-for="(
                            coreField, index
                        ) in customFieldStore.customField.data?.coreList.filter(
                                (coreField) =>
                                    coreField.fieldName === 'title' &&
                                    coreField.isVisible === '1'
                            )" :key="index">
                        <ps-label class="text-base">{{ $t(coreField.labelName) }}
                            <span v-if="(coreField.mandatory = 1)"
                                class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                        <ps-input v-if="!editable" disabled ref="title" type="text" v-model:value="paramHolder1.title"
                            class="dark:bg-transparent" :placeholder="$t(coreField.placeholder)"
                            @keypress="validateTitle" />
                        <ps-input v-else ref="title" type="text" v-model:value="paramHolder1.title" class="dark:bg-transparent"
                            :placeholder="$t(coreField.placeholder)" @keypress="validateTitle" />
                        <ps-label class="lg:mt-2 mt-1 w-full text-xs" textColor="text-feError-500"
                            v-if="validation.titleStatus">
                            {{ $t("item_entry__title_required_error") }}
                        </ps-label>
                    </div>
                    <!-- end title -->
                    <div class="flex flex-col sm:flex-row mb-4">
                        ...
                        ">
                            <div class="mb-4" v-for="(
                                ...
                                <ps-label class="text-base">{{ $t(coreField.labelName) }}
                                    <!-- <span v-if="coreField.mandatory=1" class="color-red text-feError-800 font-medium ms-1">*</span> -->
                                </ps-label>
                                <ps-dropdown align="left" class="lg:mt-2 mt-1 w-full" @onClick="
                                    resetSubCategory(paramHolder1.catId)
                                    ">
                                    <template #select>
                                        <ps-dropdown-select :showCenter="true" :selectedValue="$t(paramHolder1.subCatName)
                                            " :placeholder="$t(coreField.placeholder)
                                                " />
                                    </template>
                                    <template #filter>
                                        <div class="w-56">
                                            <ps-input-with-right-icon class="rounded-xl flex-1" @keyup.enter="
                                                filterSubCatUpdate(
                                                    subCatKeyword
                                                )
                                                " v-model:value="subCatKeyword" v-bind:placeholder="$t(
                                                    'item_list__search_subcat'
                                                )
                                                    ">
                                                <template #icon>
                                                    <ps-icon textColor="text-feSecondary-400 dark:text-feAchromatic-500"
                                                        name="search" class="cursor-pointer" @click="
                                                            filterSubCatUpdate(
                                                                subCatKeyword
                                                            )
                                                            " />
                                    ...
                                    <template #list>
                                        <div class="rounded-md shadow-xs w-56">
                                            <div class="pt-2 z-30">
                                                ...
                                                ">
                                                    ...
                                                    ">
                                                        <ps-label class="p-2 flex" textColor="text-feError-500">
                                                            {{
                                                                $t(
                                                                    "item_entry__select_category_first"
                                                                )
                                                            }}</ps-label>
                                                    </div>
                                                    <div v-else-if="
                                                ...
                                                <div v-else>
                                                    <div v-for="selectSubcategory in subCategoryStore
                                                        .subCategoryList
                                                        .data" :key="selectSubcategory.id
                                                                "
                                                        ...
                                                        @click="
                                                            ...
                                                            ">
                                                        <ps-label class="ms-2" :class="selectSubcategory.id ==
                                                                paramHolder1.subCatId
                                                                ? ' font-bold'
                                                                : ''
                                                            ">
                                                            {{
                                ...
                                <ps-label class="lg:mt-2 mt-1 w-full text-xs" textColor="text-feError-500"
                                    v-if="validation.subCatStatus">
                                    {{
                                        $t(
                                            "item_entry__sub_category_required_error"
                                        )
                                    }}
                                </ps-label>
                            </div>
                    ...
                    <div class="mb-6" v-if="phoneShow != 0" v-for="(
                            coreField, index
                        ) in customFieldStore.customField.data?.coreList.filter(
                                (coreField) =>
                                    coreField.fieldName === 'phone' &&
                                    coreField.isVisible === '1'
                            )" :key="index">
                        <ps-label class="text-base">{{ $t(coreField.labelName)
                            }}<span v-if="coreField.mandatory == 1"
                                class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                        <div v-if="phoneStatus" class="mb-1 flex flex-col" v-for="(phone, index) in phoneShow"
                            :key="phone">
                            <div class="flex w-full mt-2">
                                <ps-dropdown align="left" class="">
                                    <!-- <template #filter>
                                        <div class="w-56">
                                            <ps-input-with-right-icon class="rounded-xl flex-1" @keyup.enter="
                                                filterPhoneUpdate(
                                                ...
                                                " v-model:value="phoneKeyword" v-bind:placeholder="$t('phone_code_by_country')
                                                    ">
                                                <template #icon>
                                                    <ps-icon textColor="text-feSecondary-400 dark:text-feAchromatic-500"
                                                        name="search" class="cursor-pointer" @click="
                                    ...
                                    <template #select>
                                        <ps-dropdown-select disabled :showCenter="true" :selectedValue="phoneList[index].code
                                            " />
                                    </template>
                                    <!-- <template #list>
                                        <div class="rounded-md shadow-xs w-56">
                                            <div class="pt-2 z-30">
                                                ...
                                                <div v-else>
                                                    <div v-for="selectData in phoneStore
                                                        .phoneList.data" :key="selectData.id"
                                                        class="w-56 flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                                        @click="
                                                            phoneFilterClicked(
                                                                selectData,
                                                            ...
                                                            ">
                                                        <ps-label class="ms-2" :class="selectData.country_code ==
                                                            phoneList[index]
                                                                .value
                                                            ? ' font-bold'
                                                            : ''
                                    ...
                                    <template #loadmore>
                                        <div class="mb-2 w-56">
                                            <div v-if="
                                                phoneStore.phoneList.data !=
                                                null &&
                                                phoneStore.loading.value ==
                                                true
                                            " class="mt-4 ms-4 flex">
                                                <ps-label>
                                                    {{
                                ...
                                <ps-input class="w-full dark:bg-transparent" ref="phone" type="text" maxlength="10"
                                ...
                                <div class="flex flex-row">
                                    <ps-label class="mx-2 text-base cursor-pointer" textColor="text-fePrimary-500"
                                        v-if="!(phone == 1 && phoneShow == 1)" @click="removePhone(index)">{{
                                            "- " + $t("core__fe_remove_field")
                                        }}</ps-label>

                                    <ps-label @click="addPhone()" textColor="text-fePrimary-500" v-if="
                                        phone <
                                        parseInt(
                                    ...
                                    " class="text-base cursor-pointer">{{
                                            "+ " + $t("core__fe_add_new_fields")
                                        }}</ps-label>
                                </div>
                            </div>
                        </div>
                        <ps-label class="lg:mt-2 mt-1 w-full text-xs" textColor="text-feError-500"
                            v-if="validation.contactStatus">
                            {{ $t("item_entry__contact_required_error") }}
                        </ps-label>
                    </div>
                    ...
                    <div v-if="
                        appInfoStore.appInfo.data?.psAppSetting
                            ?.SelectedPriceType != 'NO_PRICE'
                    " class="mb-4">
                        <ps-label v-for="(
                                coreField, index
                            ) in customFieldStore.customField.data?.coreList.filter(
                                    (coreField) =>
                                        coreField.fieldName === 'original_price' &&
                                        coreField.isVisible === '1'
                                )" :key="index" class="text-base">{{ $t(coreField.labelName)
                            }}<span v-if="(coreField.mandatory = 1)"
                                class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>

                        <div v-if="
                            appInfoStore.appInfo.data?.psAppSetting
                                ?.SelectedPriceType == 'NORMAL_PRICE'
                        " class="flex flex-row items-center mt-2">
                            <div class="" v-for="(
                                    coreField, index
                                ) in customFieldStore.customField.data?.coreList.filter(
                                        (coreField) =>
                                            coreField.fieldName === 'currency_id' &&
                                            coreField.isVisible === '1'
                                    )" :key="index">
                                <ps-dropdown v-if="editable" align="left" class="w-full" @onClick="loadCurrency">
                                    <template #select>
                                    ...
                                    <template #list>
                                        <!-- <div class="rounded-md shadow-xs w-56">
                                            <div class="pt-2 z-30">
                                                ...
                                                <div v-else>
                                                    <div
                                                        v-for="itemcurrency in itemCurrencyStore
                                                            .itemCurrencyList
                                                            .data"
                                                        :key="itemcurrency.id"
                                                        class="w-56 flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                ...
                                <ps-dropdown v-else-if="!editable" disabled align="left" class="w-full"
                                    @onClick="warning" @click="warning">
                                    <template #select>
                                        <ps-dropdown-select :showCenter="true" :selectedValue="paramHolder1.currencyShortForm
                                            " />
                                    </template>
                                ...
                                <ps-label class="w-full text-xs" textColor="text-feError-500"
                                    ...
                                    {{
                                        $t(
                                            "item_entry__currency_symbol_required_error"
                                        )
                                    }}
                                </ps-label>
                            </div>

                            <div class="w-full" v-for="(
                                    coreField, index
                                ) in customFieldStore.customField.data?.coreList.filter(
                                        (coreField) =>
                                            coreField.fieldName ===
                                ...
                                <ps-input v-if="editable" ref="price" class="dark:bg-transparent" type="text"
                                    v-model:value="paramHolder1.price" :placeholder="$t(coreField.placeholder)"
                                    @keypress="validatePrice" />
                                <ps-input v-else-if="!editable" ref="price" class="dark:bg-transparent" type="text"
                                    v-model:value="paramHolder1.price" :placeholder="$t(coreField.placeholder)" disabled
                                    @click="warning" @keypress="validatePrice" />
                            </div>
                        </div>

                        <div v-if="
                            appInfoStore.appInfo.data?.psAppSetting
                                ?.SelectedPriceType == 'PRICE_RANGE'
                        " class="flex-row">
                            <div class="" v-for="(
                                    coreField, index
                                ) in customFieldStore.customField.data?.coreList.filter(
                                ...
                                <ps-dropdown class="lg:mt-2 mt-1 w-full">
                                    <template #select>
                                        <ps-dropdown-select :showCenter="true" :selectedValue="paramHolder1.price == ''
                                                ? ''
                                                : price_range.filter(
                                                    (price) =>
                                                        price.id ==
                                        ...
                                        <div class="rounded-md shadow-xs w-56">
                                            <div class="pt-2 z-30">
                                                <div v-for="range in price_range" :key="range.id"
                                                    class="w-56 flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                                    @click="
                                                        priceRangeClicked(range)
                                                        ">
                                                    <ps-label class="ms-2" :class="range.id ==
                                                            paramHolder1.price
                                                            ? ' font-bold'
                                                            : ''
                                                        ">
                        ...
                        <ps-label class="mt-1 w-full text-xs" textColor="text-feError-500"
                            v-if="validation.priceStatus">
                            {{ $t("item_entry__price_required_error") }}
                        </ps-label>
                    </div>
                    ...
                    ">
                        <div class="mb-4" v-for="(
                                coreField, index
                                    ...
                                    (coreField) =>
                                        coreField.fieldName === 'percent' &&
                                        coreField.isVisible === '1'
                                )" :key="index">
                            <ps-label class="text-base">{{ $t('item_horizontal___discount')
                                }}<span v-if="coreField.mandatory == 1"
                                    class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                            <ps-input v-if="!editable" ref="percent" class="dark:bg-transparent" type="text"
                                v-model:value="paramHolder1.percent" :placeholder="$t(coreField.placeholder)"
                                @keypress="validatePercent" disabled @click="warning" />
                            <ps-input v-else ref="percent" class="dark:bg-transparent" type="text"
                                v-model:value="paramHolder1.percent" :placeholder="$t(coreField.placeholder)"
                                @keypress="validatePercent" />
                            <ps-label class="lg:mt-2 mt-1 w-full text-xs" textColor="text-feError-500"
                                v-if="validation.discountStatus">
                                {{ $t("item_entry__discount_required_error") }}
                            </ps-label>
                        </div>
                    </div>
                    ...
                    <div class="mb-4" v-for="(
                            coreField, index
                                ...
                                (coreField) =>
                                    coreField.fieldName === 'description' &&
                                    coreField.isVisible === '1'
                            )" :key="index">
                        <ps-label class="text-base">{{ $t(coreField.labelName) }}
                            <span v-if="coreField.mandatory == 1"
                                class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                        <ps-textarea v-if="editable" rows="4" v-model:value="paramHolder1.description"
                            :placeholder="$t(coreField.placeholder)" @keypress="validateDescription" />
                        <ps-textarea v-else-if="!editable" rows="4" v-model:value="paramHolder1.description"
                            @click="warning" disabled :placeholder="$t(coreField.placeholder)"
                            @keypress="validateDescription" />
                        <ps-label class="lg:mt-2 mt-1 w-full text-xs" textColor="text-feError-500"
                            v-if="validation.descriptionStatus">
                            {{ $t("item_entry__description_required_error") }}
                        </ps-label>
                    </div>
                    <!--for product warranty  -->
                    <div>
                        <div class="mb-4">
                            <ps-label class="text-base">{{ $t("item_product_warranty")
                                }}<span class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                            <select
                                class="w-full cursor-pointer flex flex-row justify-between px-4 py-1.5 items-center rounded-md border border-feSecondary-200 dark:bg-feAchromatic-900 dark:text-feSecondary-100"
                                id="isAuctionItem" v-model="paramHolder1.isWarrantyItem">
                                <option class="rounded-md shadow-xs w-full" v-bind:value="1">
                                    {{ $t("chat__yes") }}
                                </option>
                                <option class="rounded-md shadow-xs w-full" v-bind:value="0">
                                    {{ $t("chat__no") }}
                                </option>
                            </select>
                        </div>
                    ...
                    <div>
                        <div class="mb-4">
                            <ps-label class="text-base">{{ $t("item_payment_method")
                                }}<span class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                            <select
                                class="w-full cursor-pointer flex flex-row justify-between px-4 py-1.5 items-center rounded-md border border-feSecondary-200 dark:bg-feAchromatic-900 dark:text-feSecondary-100"
                                id="isAuctionItem" v-model="paramHolder1.preferredPayment">
                                <option class="rounded-md shadow-xs w-full" v-bind:value="'wallet'">
                                    {{ $t("payment_method_wallet") }}
                                </option>
                                <option class="rounded-md shadow-xs w-full" v-bind:value="'bank_transfer'">
                                    {{ $t("payment_method_bank") }}
                                </option>
                            </select>
                        </div>
                    </div>

                    <div v-for="customFieldHeader in customFieldStore.customField
                        .data?.customList" :key="customFieldHeader.id">
                        <!-- dropdown-->
                        <div class="mb-4" v-if="
                            customFieldHeader.uiType.coreKeysId ===
                            'uit00001' &&
                            customFieldHeader.isVisible === '1' &&
                            customFieldHeader.isDelete === '0'
                        ">
                            <ps-label class="text-base">{{ $t(customFieldHeader.name)
                                }}<span class="color-red text-feError-800 font-medium ms-1"
                                    v-show="customFieldHeader.mandatory == 1">*</span></ps-label>
                            <ps-dropdown v-if="
                                customFieldHeader.name?.toLowerCase() ==
                                'price type' && !editable
                            " align="left" class="lg:mt-2 mt-1 w-full" @onClick="warning" disabled>
                                <template #select>
                                    <ps-dropdown-select disabled :showCenter="true" :selectedValue="$t(
                                        ...
                                        <div class="pt-2 z-30">
                                            <div v-if="
                                                customizeUiStoreList.data.filter(
                                                    ...
                                                    .data == null
                                            ">
                                                <ps-label class="p-2 flex" disabled @click="
                                                    loadCustomField(
                                                        customFieldHeader.coreKeysId
                                                    )
                                                    ">{{
                                            ...
                                            <div v-else>
                                                <div v-for="selectData in customizeUiStoreList.data.filter(
                                                    ...
                                                    ?.customizeUiList.data" :key="selectData.coreKeysId"
                                                    class="w-56 flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                                    @click="
                            ...
                            <ps-dropdown v-else align="left" class="lg:mt-2 mt-1 w-full" @onClick="
                                loadCustomField(
                                    customFieldHeader.coreKeysId
                                )
                                ">
                                <template #select>
                                    <ps-dropdown-select :showCenter="true" :selectedValue="$t(
                                        customizeUiStoreList.data
                                            .filter(
                                                (customizeDetail) =>
                                                    customizeDetail.id ===
                        ...
                        ">
                            ...
                            <ps-input type="text" class="w-full dark:bg-transparent rounded"
                                :placeholder="$t(customFieldHeader.placeholder)" v-model:value="form.product_relation[
                                    customFieldHeader.coreKeysId
                                    ]
                                    " @keypress="
                                    validateCustom(customFieldHeader.coreKeysId)
                                    " />
                            <ps-label textColor="text-feError-500 " class="lg:mt-2 mt-1 w-full text-xs">{{
                                product_relation_errors &&
                                product_relation_errors[
                            ...
                            <div class="flex flex-col">
                                <div class="mb-1" v-for="detail in customizeUiStoreList.data.filter(
                                    (customizeDetail) =>
                                        customizeDetail.id ===
                                        customFieldHeader.coreKeysId
                                )[0]?.provider?.customizeUiList.data" :key="detail.id">
                        ...
                        <div class="mb-4" v-if="
                            ...
                            customFieldHeader.isVisible === '1' &&
                            customFieldHeader.isDelete === '0' &&
                            customFieldHeader.coreKeysId !== 'ps-itm00008'
                        ">
                        ...
                        <div class="mb-4" v-if="
                            ...
                            customFieldHeader.isVisible === '1' &&
                            customFieldHeader.isDelete === '0' &&
                            customFieldHeader.coreKeysId != 'ps-itm00009'
                        ">
                            <ps-label class="text-base">{{ $t(customFieldHeader.name)
                                }}<span class="color-red text-feError-800 font-medium ms-1"
                                    v-show="customFieldHeader.mandatory == '1'">*</span></ps-label>
                            <ps-textarea rows="4" :placeholder="$t(customFieldHeader.placeholder)" v-model:value="form.product_relation[
                                customFieldHeader.coreKeysId
                                ]
                                " @keypress="
                                    validateCustom(customFieldHeader.coreKeysId)
                                ...
                                <CheckBox :oldData="form.product_relation[
                                    ...
                                    " @toParent="handleMultiSelect" :customizeDetails="customizeUiStoreList.data?.filter(
                                        (customizeDetail) =>
                                            customizeDetail?.id ===
                                            customFieldHeader?.coreKeysId
                                    )[0]?.provider?.customizeUiList.data
                    ...
                    <div v-if="
                        appInfoStore.appInfo.data?.mobileSetting
                            ?.is_show_discount == '1' && !isLiveAuction
                    ">
                        <div class="mb-4" v-for="(
                                coreField, index
                            ) in customFieldStore.customField.data?.coreList.filter(
                                    (coreField) =>
                                        coreField.fieldName === 'auction_period' &&
                                        coreField.isVisible === '1'
                                )" :key="index">
                            <ps-label class="text-base">{{ $t(coreField.labelName)
                                }}<span v-if="coreField.mandatory == 1"
                            ...
                            <!-- <ps-input
                                ...
                                :placeholder="$t(coreField.placeholder)"
                            /> -->
                            <select v-if="!editable" disabled @click="warning"
                                class="w-full cursor-pointer flex flex-row justify-between px-4 py-1.5 items-center rounded-md border border-feSecondary-200 dark:bg-feAchromatic-900 dark:text-feSecondary-100"
                                id="isAuctionItem" v-model="paramHolder1.auction_period">
                                <!-- <option class="rounded-md shadow-xs w-full" selected value="">
                                    {{ $t(coreField.placeholder) }}
                                </option> -->
                                <option selected class="rounded-md shadow-xs w-full" v-bind:value="7">
                                    {{ $t("7_day_period") }}
                                </option>
                                <option class="rounded-md shadow-xs w-full" v-bind:value="14">
                                    {{ $t("14_day_period") }}
                                </option>
                                <option class="rounded-md shadow-xs w-full" v-bind:value="30">
                                    {{ $t("30_day_period") }}
                                </option>
                            </select>
                            <select v-else
                                ...
                                <option class="rounded-md shadow-xs w-full" v-bind:value="7" selected>
                                    {{ $t("7_day_period") }}
                                </option>
                                <option class="rounded-md shadow-xs w-full" v-bind:value="14">
                                    {{ $t("14_day_period") }}
                    ...
                    <div class="flex flex-col sm:flex-row mb-4">
                        ...
                        <div class="mb-4 w-full me-6 sm:me-2 mb-4 sm:mb-0" v-for="(
                                coreField, index
                            ) in customFieldStore.customField.data?.coreList.filter(
                                    (coreField) =>
                                        coreField.fieldName ===
                            ...
                            <ps-label class="text-base">{{ $t(coreField.labelName)
                                    ...
                                    class="color-red text-feError-800 font-medium ms-1">*</span></ps-label>
                            <ps-dropdown v-if="editable" align="left" class="lg:mt-2 mt-1 w-full mb-2"
                                @onClick="loadLocation" v-on:keyup.enter="
                                    filterKeywordUpate(locationKeyword)
                                    ">
                                <template #select>
                                ...
                                <template #filter>
                                    <div class="w-56">
                                        <ps-input-with-right-icon class="rounded-full flex-1"
                                            v-model:value="locationKeyword" v-bind:placeholder="$t(
                                                'item_entry__search_location'
                                            )
                                                ">
                                            <template #icon>
                                                <ps-icon name="search" class="cursor-pointer" @click="
                                                    filterKeywordUpate(
                                                        locationKeyword
                                                    )
                                                    " />
                                ...
                                <template #list>
                                    <div class="rounded-md shadow-xs w-56">
                                        <div class="pt-2 z-30">
                                            <div v-if="
                                                itemLocationStore.loading
                                                    .value == true
                                            " class="mt-4 ms-4 mb-4">
                                                <ps-label @click="loadLocation">
                                                    {{
                                                        $t(
                                                            "item_entry__loading"
                                                        )
                                            ...
                                            <div v-for="selectData in itemLocationStore
                                                .itemLocationList.data" :key="selectData.id"
                                                class="w-56 flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                                @click="
                                                    ...
                                                    ">
                                                <ps-label class="ms-2" :class="selectData.id ==
                                                        paramHolder1.itemLocationId
                                                        ? ' font-bold text-fePrimary-700'
                                                        : ''
                                                    ">
                                                    {{ selectData.name }}
                                                </ps-label>
                                ...
                                <template #loadmore>
                                    <div class="mb-2 w-56">
                                        <div v-if="
                                            itemLocationStore
                                                .itemLocationList.data !=
                                            null &&
                                            itemLocationStore.loading
                                                .value == true
                                        " class="mt-4 ms-4 flex">
                                            <ps-label>{{ $t("item_entry__loading") }}
                                            </ps-label>
                                        </div>
                            ...
                            <ps-dropdown v-else-if="!editable" align="left" class="lg:mt-2 mt-1 w-full mb-2"
                                @onClick="warning">
                                <template #select>
                                    <ps-dropdown-select :showCenter="true" :selectedValue="paramHolder1.itemLocationName
                                        " :placeholder="$t(coreField.placeholder)" />
                            ...
                            <ps-label class="lg:mt-2 mt-1 w-full text-xs" textColor="text-feError-500"
                                v-if="validation.itemLocationStatus">
                                {{ $t("item_entry__location_required_error") }}
                            </ps-label>
                        </div>
                        <div class="mb-4 w-full" v-if="
                            appInfoStore.appInfo.data?.psAppSetting
                                ?.isSubLocation == '1'
                        ">
                            <div v-for="(
                                        ...
                                        <div class="w-56">
                                            <ps-input-with-right-icon class="rounded-xl flex-1" @keyup.enter="
                                                ...
                                                " v-model:value="sublocationKeyword
                                                    ...
                                                    'search_for_large_screem__sub_location'
                                                )
                                                    ">
                                                <template #icon>
                                                    <ps-icon textColor="text-feSecondary-400 dark:text-feAchromatic-500"
                                    ...
                                    <template #list>
                                        <div class="rounded-md shadow-xs w-56">
                                            <div class="pt-2 z-30">
                                                ...
                                                ">
                                                    ...
                                                    ">
                                                        <ps-label class="p-2 flex" textColor="text-feError-500">
                                                            {{
                                                                $t(
                                                                    "item_entry__select_location_first"
                                                                )
                                                            }}</ps-label>
                                                    </div>
                                                    <div v-else-if="
                                                ...
                                                <div v-else>
                                                    <div v-for="locationTownship in locationTownshipProvider
                                                        .locationTownshipList
                                                        .data" :key="locationTownship.id
                                                                "
                                                        class="w-56 flex py-4 px-2 hover:bg-fePrimary-50 dark:hover:bg-fePrimary-900 cursor-pointer items-center"
                                                        @click="
                                                            ...
                                                            ">
                                                        <ps-label class="ms-2" :class="locationTownship.id ==
                                                                paramHolder1.locationTownshipId
                                                                ? ' font-bold'
                                                                : ''
                                                            ">
                                                            {{
                                                                locationTownship.townshipName
                                                            }}
                                                        </ps-label>
                                                    </div>
                                                </div>
                                ...
                                <ps-label class="lg:mt-2 mt-1 w-full text-xs" textColor="text-feError-500"
                                    ...
                                    {{
                                        $t(
                                            "item_entry__location_township_required_error"
                                        )
                                    }}
                                </ps-label>
                            </div>
                    ...
                    <!-- <div class="flex flex-col justify-start lg:mt-5 mt-4 font-medium text-base lg:text-lg mb-4">
                        <map-with-pin-picker v-if="
                            appInfoStore?.appInfo?.data
                                ?.frontendConfigSetting.googleMap == '1' &&
                            editable
                        " class="h-68" :onChange="updateCoordinates" disabled />
                        <map-with-pin-picker v-else-if="
                            appInfoStore?.appInfo?.data
                                ?.frontendConfigSetting.googleMap == '1' &&
                            !editable
                        " class="h-68" :onChange="warning" disabled />
                        <open-street-map :dir="$page.props.dir" v-if="
                            isReloadOpenStreetMap &&
                            appInfoStore?.appInfo?.data
                                ?.frontendConfigSetting.googleMap == '0' &&
                            editable
                        " disabled class="h-68" :onChange="updateLeafletCoordinates"
                            :lat="parseFloat(paramHolder1.latitude)" :lng="parseFloat(paramHolder1.longitude)" />

                        <open-street-map :dir="$page.props.dir" v-else-if="
                            isReloadOpenStreetMap &&
                            appInfoStore?.appInfo?.data
                                ?.frontendConfigSetting.googleMap == '0' &&
                            !editable
                        " class="h-68" :onChange="warning" disabled :lat="parseFloat(paramHolder1.latitude)"
                            :lng="parseFloat(paramHolder1.longitude)" />
                    </div> -->

                    <div class="mt-2" v-for="customFieldHeader in customFieldStore.customField
                        .data?.customList" :key="customFieldHeader.id">
                        <div class="mb-4" v-if="
                            customFieldHeader.uiType.coreKeysId ===
                            'uit00006' &&
                        ...
                        ">
                            <ps-label class="text-base">{{ $t(customFieldHeader.name)
                                    ...
                                    v-show="customFieldHeader.mandatory == '1'">*</span></ps-label>
                            <ps-textarea rows="4" v-if="editable" :placeholder="$t(customFieldHeader.placeholder)"
                                v-model:value="form.product_relation[
                                    customFieldHeader.coreKeysId
                                    ]
                                    " @keypress="
                                    validateCustom(customFieldHeader.coreKeysId)
                                    " />
                            <ps-textarea v-else-if="!editable" rows="4" :placeholder="$t(customFieldHeader.placeholder)"
                                v-model:value="form.product_relation[
                                    customFieldHeader.coreKeysId
                                    ]
                                    " disabled @keypress="
                ...
                <div class="w-full">
                    ...
                    <div v-if="itemId == '0'">
                        <ps-button v-if="isLiveAuction" class="lg:mt-5 mt-4 mb-5 text-center w-full"
                            ...
                            @click="submitClicked('go_live')">
                            {{ $t("core__fe_item_entry__go_live") }}
                        </ps-button>
                        <ps-button v-else class="lg:mt-5 mt-4 mb-5 text-center w-full"
                            textSize="lg:text-sm sm:text-xs text-xs"
                            theme="bg-fePrimary-500 dark:bg-feAccent-500 text-feAchromatic-50 dark:text-feAchromatic-900 py-1"
                            @click="submitClicked('submit')">
                            {{ $t("core__fe_item_entry__submit") }}
                        </ps-button>
                    </div>
                    <ps-button v-else class="lg:mt-5 mt-4 mb-5 text-center w-full"
                        textSize="lg:text-sm sm:text-xs text-xs"
                        theme="bg-fePrimary-500 dark:bg-feAccent-500 text-feAchromatic-50 dark:text-feAchromatic-900 py-1"
                        @click="submitClicked('update')">
                        {{ $t("item_entry__update") }}
                    </ps-button>
    ...
    <ps-edit-warn-dialog ref="ps_edit_warn_dialog" />
</template>
...
import PsEditWarnDialog from "../../../components/core/dialog/PsEditWarnDialog.vue";
...
const MapWithPinPicker = defineAsyncComponent(
    () =>
        import("@template1/vendor/components/layouts/map/MapWithPinPicker.vue")
);
const OpenStreetMap = defineAsyncComponent(
    () => import("@template1/vendor/components/layouts/map/OpenStreetMap.vue")
);
...
export default {
    ...
    setup(props) {
        ...
        const loginUserId = props.authUser
            ? props.authUser.id
        ...
        const paramHolder =
            new LocationParameterHolder().getDefaultParameterHolder();

        paramHolder1.value.latitude =
            psValueStore.locationLat == null || psValueStore.locationLat == ""
                ? "0"
                : psValueStore.locationLat;
        paramHolder1.value.longitude =
            psValueStore.locationLng == null || psValueStore.locationLng == ""
                ? "0"
                : psValueStore.locationLng;
        ...
        const locationTownshipProvider =
            useItemLocationTownshipStoreState("entry");
        ...
        onMounted(async () => {
            ...
            phoneStoredefault.loadPhoneCountryCode(
                loginUserId,
                itemProvider.phoneparamHolder
            );
            // let phonedata = phoneStore.phoneList.data.filter((phone) => {
            //     return phone.country_code == "+41"
            // })
            // phoneStore.phoneList.data = phonedata
            ...
            if (!locationPermission.value) {
                ...
                paramHolder1.value.latitude =
                    appInfoStore?.appInfo?.data?.psAppSetting?.latitude.toString();
                paramHolder1.value.longitude =
                    appInfoStore?.appInfo?.data?.psAppSetting?.longitude.toString();
            ...
            if (
                appInfoStore?.appInfo?.data?.psAppSetting?.isPaidApp ==
                PsConst.ONE &&
                itemId == "0" &&
                userStore.user?.data?.postCount == PsConst.LIMITED &&
                userStore.user?.data?.remainingPost == "0"
            ) {
            ...
            videoDurationString.value = PsUtils.secondToDuration(
                parseInt(
                    appInfoStore?.appInfo?.data.mobileSetting?.video_duration
                ) / 1000
            );
            ...
            if (
                parseInt(
                    appInfoStore.appInfo.data?.mobileSetting?.phone_list_count
                ) < 1
            ) {
                phoneShow.value = 0;
            } else {
            ...
            await customFieldStore.loadCustomFieldList(
                loginUserId,
                props.categoryId
            );
            await defaultitemCurrencyStore.loadItemCurrencyList(loginUserId);

            paramHolder1.value.currencyShortForm =
                defaultitemCurrencyStore.itemCurrencyList.data[0].currencySymbol;
            paramHolder1.value.itemCurrencyId =
                defaultitemCurrencyStore.itemCurrencyList.data[0].id;
            ...
            if (
                ...
                localStorage.defaultProfileType == "user"
            ) {
                ...
                appInfoStore?.isVendorSettingOn() &&
                localStorage.defaultProfileType &&
                localStorage.defaultProfileType == "vendor" &&
                customFieldStore.customField.data?.vendorList.length > 0
            ) {
                let hasaccess = false;

                customFieldStore.customField.data?.vendorList.forEach(function (
                    vendor
                ) {
                    if (
                        localStorage.defaultProfileId &&
                        localStorage.defaultProfileId == vendor.id
                    ) {
                        vendorAutoChoose.value = true;
            ...
            if (itemId != "0") {
                ...
                phoneList[0]["code"] =
                    appInfoStore?.appInfo?.data?.defaultPhoneCountryCode
                        ?.country_code ?? "+41";
                phoneCountryCide[0]["value"] =
                    appInfoStore?.appInfo?.data?.defaultPhoneCountryCode
                        ?.country_code ?? "+41";
            }
        });
        ...
        onUnmounted(() => {
            ...
            for (const customField of customFieldStore.customField?.data
                ...
                if (
                    ...
                    customizeUiStoreList.data.filter(
                        (customizeDetail) =>
                            customizeDetail.id === customField.coreKeysId
                    )[0].provider.$reset;
                }
                if (
                    customField.isVisible == "1" &&
                ...
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        Axios.get(
                            ...
                            .then(async (response) => {
                                paramHolder1.value.latitude =
                                    await response.data.lat.toString();
                                paramHolder1.value.longitude =
                                    await response.data.lon.toString();
                                form.product_relation[itemAddressCoreKeysId] =
        ...
        async function loadItemForEdit() {
            ...
            if (item.status == PsStatus.SUCCESS) {
                ...
                paramHolder1.value.currencyShortForm =
                    item.data.itemCurrency.currencySymbol;

                paramHolder1.value.price = checkPriceFormat(
                ...
                paramHolder1.value.is_warranty_item = item.data.isWarrantyItem;
                paramHolder1.value.preferred_payment =
                    item.data.preferredPayment;
                ...
                paramHolder1.value.itemLocationName =
                    item.data.itemLocation.name;
                paramHolder1.value.locationTownshipId =
                    item.data.itemLocationTownshipId;
                paramHolder1.value.locationTownshipName =
                    item.data.itemLocationTownship.townshipName;
                paramHolder1.value.auction_period = item?.data?.auctionPeriod;
                ...
                gallery.value = await galleryProvider.loadGalleryList(
                    appInfoStore.appInfo.data.mobileSetting.is_show_item_video,
                    loginUserId,
                    itemId,
                    PsConst.ITEM_TYPE,
                    null
                ...
                if (gallery.value.data != null) {
                    ...
                    for (let i = 0; i < imageCount.value; i++) {
                        let id = imgList.filter(
                            (image) => image.ordering == i + 1
                        )[0]?.imgId;
                        if (id && id != "") {
                            imgId[i] = id;
                        }
                ...
                myArray.forEach((phone, i) => {
                    ...
                    if (tempPhone.length > 1) {
                        ...
                        phoneList.push({
                            id: i,
                            value: tempPhone[0],
                            code: phoneStoredefault.phoneList.data[0]
                                .country_code,
                        });
                    }
        ...
        function filterKeywordUpate(value) {
            itemLocationStore.filterKeywordUpate(
                value,
                loginUserId,
                paramHolder
            );
        ...
        function filtersubLocationUpdate(value) {
            ...
            locationTownshipProvider.filtersubLocationUpdate(
                loginUserId,
                subparamHolder
            );
        }
        ...
        function loadSubCategory(catId) {
            if (catId != "") {
                ...
                // await subCategoryStore.subCategoryList.data.map(subcategory => {
                //     subcategory.name = trans(subcategory.name);
                //     return subcategory;
                // })
                // await subCategoryStore.subCategoryList.data.sort((a, b) => {
        ...
        function loadLocationTownship(locationId) {
            if (locationId != "") {
                locationTownshipProvider.loadItemLocationTownshipList(
                    locationId
                );
            }
        }
        ...
        function resetLocationTownship(locationId) {
            if (locationId != "") {
                locationTownshipProvider.resetItemLocationTownshipList(
                    locationId
                );
            }
        }
        ...
        async function submitItem(type) {
            //console.log("submit click type----------->>", type);
            if (itemId == "0") {
            ...
            if (
                ...
                if (isLiveAuction.value == true) {
                    // console.log("live auction ", isLiveAuction, paramHolder1.value.auction_period);
                    paramHolder1.value.auction_period = "";
                } else if (

                    paramHolder1.value.auction_period.toString() !== "7" &&
                    paramHolder1.value.auction_period.toString() !== "14" &&
                    paramHolder1.value.auction_period.toString() !== "30"
                ) {
                    // console.log("paramHolder1.value.auction_period",paramHolder1.value.auction_period)
                    paramHolder1.value.auction_period = "7";
                }
                ps_loading_dialog.value.openModal();
                ps_loading_dialog.value.message = trans(
                    "item_entry__uploading_item"
                );
                let returnData;
                try {
                    // //console.log("paramHolder1", paramHolder1);
                    // return false;
                    const currentDate = new Date().getTime();
                    const startDateTimestampStr =
                        PsUtils.getTimeStampDividedByOneThousand(currentDate);
                    paramHolder1.value.product_creation_date =
                        startDateTimestampStr;
                    returnData = await itemProvider.submitItemEntry(
                        paramHolder1.value,
                        loginUserId
                    );

                    newItemId.value = returnData.data.id;
                } catch (e) {
                    PsUtils.log(e);
                }
                ...
                if (
                    ...
                    returnData.status == PsStatus.SUCCESS
                ) {
                    if (returnData.data.id != "") {
                        ...
                        if (
                            ...
                            ps_loading_dialog.value.setMessage(
                                trans("item_entry__uploading_video")
                            );
                            await itemProvider.postItemVideoUpload(
                                "",
                                itemProvider.item.data.id,
                                selectedFileVideo,
                                loginUserId
                            );
                        }
                        ...
                        if (
                            selectedFileVideoThumb != undefined &&
                            selectedFileVideoThumb != ""
                        ) {
                            ps_loading_dialog.value.setMessage(
                                trans("item_entry__uploading_video_thumb")
                            );
                            await itemProvider.postItemVideoThumbUpload(
                                itemProvider.item.data.id,
                                selectedFileVideoThumb,
                                loginUserId
                            );
                        ...
                        ps_loading_dialog.value.setMessage(
                            trans("item_entry__uploading_image"),
                            1
                        );
                        await dropzone_ref.value.autoProcessQueuestart(
                            itemProvider.item.data.id
                        );
                        ...
                        if (queueFinish.value == true) {
                            ...
                            if (props.itemId&&props.itemId != "0") {
                                //console.log("item id is not equal to 0");
                                ps_success_dialog.value.openModal(
                                    ...
                                    () => {
                                        if (type === "go_live") {
                                            router.get(
                                                route(
                                                    `go-live?item_id=${returnData.data.id}&host=${paramHolder1.value.addedUserId}`
                                                )
                                            );
                                        } else {
                                            router.get(route("dashboard"));
                                ...
                                //console.log("item id is equal to 0");
                                ps_success_dialog.value.openModal(
                                    trans("ps_success_dialog__success"),
                                    trans("item_upload__success_upload"),
                                    trans("ps_confirm_dialog__yes"),
                                    () => {
                                        if (type === "go_live") {
                                            router.get(
        ...
        function getImageUrl(index) {
            if (
                ...
                const imageName = galleryProvider.galleryList.data.filter(
                    (image) => image.ordering == index
                )[0]?.imgPath;
                ...
                previewImages.data != null &&
                previewImages.data.length >= index
            ) {
                if (previewImages.data[index - 1]?.file == "") {
                    if (previewImages.data[index - 1]?.imgUrl) {
                        return (
                            props.thumb2xUrl +
                            "/" +
                            previewImages.data[index - 1]?.imgUrl
                        );
                    } else {
                        return "";
                    }
        ...
        function getImageId(index) {
            if (
                ...
                return galleryProvider.galleryList.data?.filter(
                    (image) => image.ordering == index
                )[0]?.imgId;
            } else {
                return "";
        ...
        function getVideoUrl() {
            if (
                ...
                if (
                    itemProvider.item.data.videoThumbnail.imgPath &&
                    itemProvider.item.data.videoThumbnail.imgPath != ""
                ) {
        ...
        watch(queueFinish, (value) => {
            if (dropzone_loading.value == true && queueFinish.value == true) {
                ...
                ps_success_dialog.value.openModal(
                    trans("ps_success_dialog__success"),
                    trans(props.itemId&&props.itemId!='0'? "item_upload__success_update": "item_upload__success_upload"),
                    trans("ps_confirm_dialog__yes"),
                    () => {
                        if (paramHolder1.value.is_auction_item == 1) {
                            router.get(
                                route(`fe_host_auction`, {
                        ...
                        video.oncanplay = function () {
                            if (
                                ...
                                ps_warning_dialog.value.openModal(
                                    "Video Upload!",
                                    "Video must be less than" +
                                    videoDurationString.value
                                );
                                return false;
                            } else {
        ...
        async function submitClicked(type) {
            ...
            if (
                paramHolder1.value.phone != "" &&
                paramHolder1.value.phone.endsWith("#")
            ) {
                paramHolder1.value.phone = paramHolder1.value.phone.slice(
                    0,
                    -1
                );
            }
            ...
            if (paramHolder1.value.lat == "" && paramHolder1.value.lng == "") {
                paramHolder1.value.lat =
                    appInfoStore?.appInfo?.data.psAppSetting?.lat;
                paramHolder1.value.lng =
                    appInfoStore?.appInfo?.data.psAppSetting?.lng;
            }
            ...
            customFieldStore.customField.data?.coreList.forEach((coreField) => {
                if (
                    coreField.fieldName === "title" &&
                    (paramHolder1.value.title == "" ||
                        paramHolder1.value.title == undefined) &&
                    coreField.isVisible == "1" &&
                ...
                if (
                    coreField.fieldName === "category_id" &&
                    (paramHolder1.value.catId == "" ||
                        paramHolder1.value.catId == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                ...
                if (
                    coreField.fieldName === "subcategory_id" &&
                    (paramHolder1.value.subCatId == "" ||
                        paramHolder1.value.subCatId == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                ...
                if (
                    coreField.fieldName === "original_price" &&
                    (paramHolder1.value.price == "" ||
                        paramHolder1.value.price == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                    validation.value.priceStatus = true;
                ...
                if (
                    coreField.fieldName === "percent" &&
                    (paramHolder1.value.percent == "" ||
                        paramHolder1.value.percent == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                    validation.value.discountStatus = true;
                ...
                if (
                    coreField.fieldName === "location_city_id" &&
                    (paramHolder1.value.itemLocationId == "" ||
                        paramHolder1.value.itemLocationId == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                ...
                if (
                    coreField.fieldName === "location_township_id" &&
                    (paramHolder1.value.locationTownshipId == "" ||
                        paramHolder1.value.locationTownshipId == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                ...
                if (
                    coreField.fieldName === "currency_id" &&
                    (paramHolder1.value.itemCurrencyId == "" ||
                        paramHolder1.value.itemCurrencyId == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                    // alert("here");
                ...
                if (
                    coreField.fieldName === "phone" &&
                    (paramHolder1.value.phone == "" ||
                        paramHolder1.value.phone == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                    validation.value.contactStatus = true;
                ...
                if (
                    coreField.fieldName === "description" &&
                    (paramHolder1.value.description == "" ||
                        paramHolder1.value.description == undefined) &&
                    coreField.isVisible == "1" &&
                    coreField.mandatory == 1
                ) {
                    validation.value.descriptionStatus = true;
            ...
            customFieldStore.customField.data?.customList.forEach(
                (customField) => {
                    if (
                        ...
                        product_relation_errors[customField.coreKeysId] =
                            customField.name +
            ...
            if (
                ...
                if (!(parseFloat(paramHolder1.value.price) > 0)) {
                    ps_error_dialog.value.openModal(
                        "",
                        trans("core__fe_enter_valid_price")
                    );
                    return;
                }
            }
            if (type == "submit" || type == "go_live") {
                if (paramHolder1.value.auction_period.toString() == "14") {
                    paramHolder1.value.price =
                        Number(paramHolder1.value.price) + 3.9;
                } else if (
                    paramHolder1.value.auction_period.toString() == "30"
                ) {
                    paramHolder1.value.price =
                        Number(paramHolder1.value.price) + 6.9;
                }
            }
            if (type == "submit" || type == "update") {
                paramHolder1.value.is_auction_item = "0";
                submitItem("submit");
            } else if (type == "go_live") {
                paramHolder1.value.is_auction_item = "1";
                submitItem("go_live");
            }
        }
        ...
        async function doDeleteImage(value) {
            ...
            const status = await galleryProvider.deleteImage(
                value,
                loginUserId
            );
            PsUtils.log(status);
        ...
        async function doDeleteVideo(video, icon) {
            ps_loading_dialog.value.openModal();
            const status = await galleryProvider.deleteVideo(
                video,
                loginUserId
            );
        ...
        function selectCustomDropdown(field, data) {
            form.product_relation[field.coreKeysId] = data.id;
            console.log("price type", field,data)
            if (field.coreKeysId?.toString() == "ps-itm00003" ) {
                if (
                    data.name?.toLowerCase() == "live auction" ||
                    data.name?.toLowerCase() == "negotiable"||data.id.toString() == '104'
                ) {
                    isLiveAuction.value = true;
                } else {
                    isLiveAuction.value = false;
                }
            }
            validateCustom(field.coreKeysId);
        }
        ...
        function addPhone() {
            ...
            phoneList.push({
                id: phoneShow.value,
                value: "",
                code: phoneStoredefault.phoneList.data[0].country_code,
            });
        ...
        /********************auction item entry *************/
        paramHolder1.value.is_auction_item = is_auction_item.value;

        /**********************April 02 ********************/

        function warning() {
            ps_edit_warn_dialog.value.openModal();
        }
