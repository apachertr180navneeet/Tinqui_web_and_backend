    ...
    <div class="xl:w-feLarge lg:w-large md:w-full md:px-6 lg:px-0 mx-auto">
      <div v-if="itemList && itemList.length>0"
        ...
        <div class="flex flex-col mt-6 sm:mb-0">
          <ps-label-header-5 class="font-semibold" textColor="">{{ $t("home__fe_fixed_price")
            }}</ps-label-header-5>
        </div>
        <!-- <div class="sm:mt-0">
        ...
        <div class="homepage-slider w-full">
          <auction-item-horizontal-swiper :itemList="itemList" :isLoading="isLoading" storeName="dashboard_fixed_price" />
        </div>
      </div>
    </div>
  </template>
