<template>
    <ps-content-container
        v-if="categoryStore.hasData() || categoryStore.loading.value"
        class="mt-10"
    >
        <template #content>
            <div class="flex justify-between items-center mb-6 test-category">
                <!-- <ul class="custom-cate">
                <li v-for="(category, index) in categoryStore.itemList.data.slice(0, 4)" :key="category.catId">
                ...
                <ul class="custom-cate">
                    <li
                        v-for="category in categoryStore.itemList.data.slice(
                            0,
                            4
                        )"
                        :key="category.catId"
                ...
                <!-- <ps-route-link :to="{ name: 'fe_category.index' }">
                    <ps-button
                        class="btn_view_all_category flex flex-row"
                        padding="p-2 sm:px-4 sm:py-2"
                        shadow="shadow-sm"
                        rounded="rounded"
                        hover=""
                        focus=""
                    ...
                    >
                        ...
                        <ps-icon
                            class="sm:ms-2 block rtl:hidden"
                        ...
                        />
                        <ps-icon
                            class="sm:ms-2 hidden rtl:block"
                            textColor="dark:text-feSecondary-200"
                            name="leftChervon"
                            h="24"
                            w="24"
            ...
            <!-- <div
                class="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-9 lg:grid-cols-12 xl:gap-6 gap-4 sm:gap-6 show-category-list"
                v-if="categoryStore.hasData()"
            >
                <div
                    v-for="category in categoryStore.itemList.data.slice(0, 8)"
                    :key="category.catId"
            ...
            <!-- <div
                    v-for="category in categoryStore.itemList.data.slice(0, 4)"
                    :key="category.catId"
...
// import DashboardSidebar from "@template1/vendor/components/modules/category/DashboardSidebar.vue";
