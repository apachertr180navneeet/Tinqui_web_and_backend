    ...
    <ps-content-container>
        <template #content>
            <div class="sm:mt-32 lg:mt-36 mt-28">
                <div
                    class="flex flex-col sm:flex-row items-center justify-between mt-24 mb-5 sm:mb-6 lg:mb-8"
                >
                    <ps-breadcrumb-2 :items="breadcrumb" class="" />

                    <div
                        class="w-full flex content-center items-center mt-7 sm:mt-0"
                    >
                        <!-- keyword search -->
                        <div
                            class="flex-grow lg:w-72 xl:w-76 h-10 w-full sm:w-56 md:w-60"
                        >
                            <ps-input-with-right-icon
                                v-on:keyup.enter="searchClicked"
                                v-model:value="
                                    subCategoryStore.paramHolder.keyword
                                "
                                class=""
                                v-bind:placeholder="
                                    $t('sub_category_list__search')
                                "
                            >
                                <template #icon>
                                    <ps-icon
                                        v-if="
                                            subCategoryStore.paramHolder
                                                .keyword == ''
                                        "
                                        name="search"
                        ...
                        >
                            <template #select>
                                <ps-dropdown-select
                                    ...
                                    border="border border-1 border-feSecondary-200 dark:border-feSecondary-400"
                                    text="text-sm font-medium text-feAchromatic-800 dark:text-feAchromatic-100 hidden lg:inline"
                                    iconTheme="text-feAchromatic-800 ms-2 hidden lg:inline"
                                    leftIcon="filter"
                                    leftIconTheme="inline lg:hidden"
                        ...
                        <!-- <div
                            v-if="
                                appInfoStore?.appInfo?.data?.psAppSetting
                                    ?.isSubcatSubscription == PsConst.ONE
                            "
                            class="ml-4"
                        >
                            <ps-button
                ...
                <div class="flex flex-col">
                    <div class="flex flex-row mb-8">
                        <div class="w-full flex flex-col">
                            <div
                                v-if="
                                    ...
                                    subCategoryStore.subCategoryList.data !=
                                        null
                                "
                                class="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-9 lg:grid-cols-12 md:gap-6 gap-4"
                            >
                                <!-- Column -->
                                <div
                                    class="relative w-full col-span-4 sm:col-span-2 md:col-span-3 lg:col-span-4 xl:col-span-3"
                                ...
                                >
                                    ...
                                    >
                                        <div
                                            ...
                                            :class="
                                                scribe &&
                                                subscribedSubCatList.filter(
                                                    (subcat) =>
                                                        subcat ==
                                                        subCategory.id + '_FE'
                                                ).length == 1
                                                    ? 'border-2 border-fePrimary-500'
                                                    : ''
                                            "
                                        >
                                            <div
                                                class="h-44 relative flex justify-center"
                                            >
                                                <img
                                                ...
                                                <div
                                                    class="w-14 h-14 absolute -bottom-7 bg-feAchromatic-50 p-3 items-center rounded-full border-2 border-white dark:bg-feSecondary-800 shadow-md"
                                                >
                                                    <img
                                                        alt="Placeholder"
                                            ...
                                            <div class="h-20">
                                                <ps-label
                                                    class="peer-checked/draft:text-sky-500 pt-9 lg:text-base sm:text-sm text-base font-medium"
                                                    textColor=""
                                                >
                                                    {{
                                                        $t(
                                    ...
                                    >
                                        <custom-checkbox
                                            :isScribe="subCategory.isSubScribe"
                                            :checked="subCategory.id + '_FE'"
                                            :value="subCategory.id + '_FE'"
                            ...
                            <ps-no-result
                                v-if="
                                    subCategoryStore.loading.value == false &&
                                    subCategoryStore.subCategoryList?.data ==
                                        null &&
                                    initial == false
                                "
                            ...
                            <ps-button
                                v-if="
                                    ...
                                    initial == false
                                "
                                class="w-60 mx-auto mt-8"
                                @click="loadMore"
                                :class="
                                    subCategoryStore.isNoMoreRecord.value
                                        ? 'hidden'
                                        : ''
                                "
                            >
                                {{ $t("sub_category_list__load_more") }}
                            </ps-button>
                            <ps-button
                                v-else-if="initial == false"
                                class="w-60 mx-auto mt-8"
                                @click="loadMore"
                                :disabled="true"
                            >
                                {{ $t("sub_category_list__loading") }}
                            </ps-button>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    setup(props) {
        ...
        subCategoryStore.limit =
            props.mobileSetting?.default_loading_limit ?? 12;
        ...
        async function submitScribe() {
            ...
            const status = await subCategoryScribeProvider.subCategoryScription(
                loginUserId,
                holder
            );
            if (status.status == PsStatus.SUCCESS) {
                await subCategoryStore.resetSearchSubCategoryList(
                    loginUserId,
                    subCategoryStore.paramHolder
                );
        ...
        async function loadDataList() {
            ...
            const status = await subCategoryScribeProvider.isUserSubscribed(
                "1",
                loginUserId
            );
        ...
        function loadMore() {
            subCategoryStore.resetSearchSubCategoryList(
                loginUserId,
                subCategoryStore.paramHolder
            );
        }
        ...
        onMounted(() => {
            if (
                initial.value == true &&
                subCategoryStore.subCategoryList?.data == null
            ) {
                ps_loading_dialog.value.openModal();
            }
        });
