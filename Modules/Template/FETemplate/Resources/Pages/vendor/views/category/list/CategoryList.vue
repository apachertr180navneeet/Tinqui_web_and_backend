        ...
        <template #content>
            <div class="sm:mt-32 lg:mt-36 mt-28">
                ...
                >
                    ...
                    <div
                        class="flex justify-end content-center items-center w-full"
                    >
                        <ps-input-with-right-icon
                            v-on:keyup.enter="searchClicked"
                            v-model:value="categoryStore.paramHolder.keyword"
                            class="sm:w-80 w-full"
                            padding="py-2 px-4 h-10"
                            v-bind:placeholder="$t('category__fe_search')"
                        >
                            <template #icon>
                                <ps-icon
                                    v-if="
                ...
                <div class="flex flex-row mb-8 mt-8">
                    <div class="w-full">
                        ...
                        >
                            <!-- Column -->
                            <div
                                class="w-full col-span-4 sm:col-span-2 md:col-span-3 lg:col-span-4 xl:col-span-3"
                                v-for="category in categoryStore.itemList.data"
                                :key="category.catId"
                            >
                                <ps-route-link
                                    :to="{
                                        name: appInfoStore?.isShowSubCategory()
                                            ? 'fe_sub_category'
                                            : 'fe_item_list',
                                        query: {
                                    ...
                                    @click="
                                        updateCategoryTouchCount(category.catId)
                                    "
                                >
                        ...
                        <ps-no-result
                            v-if="
                                categoryStore.loading.value == false &&
                        ...
                        <ps-button
                            v-if="
                                ...
                                initial == false
                            "
                            class="font-medium mx-auto mt-6"
                            @click="loadMore"
                            :class="
                                categoryStore.isNoMoreRecord.value
                                    ? 'hidden'
                                    : ''
                            "
                        >
                            {{ $t("category_list__load_more") }}
                        </ps-button>
                        <ps-button
                            v-else-if="initial == false"
                            class="font-medium mx-auto mt-8"
                            @click="loadMore"
                            :disabled="true"
                        >
                            {{ $t("category_list__loading") }}
                        </ps-button>
                        <!-- <div v-if="loading">
                            refreshing...
                        </div> -->
