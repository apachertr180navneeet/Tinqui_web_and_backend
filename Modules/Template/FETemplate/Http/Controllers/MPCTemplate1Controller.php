...
namespace Modules\Template\FETemplate\Http\Controllers;
...
class MPCTemplate1Controller extends Controller
{
    ...
    const contactInfoPage = self::parentPath . "contact/ContactUsInfo";
     const internalwalletPage = self::parentPath . "wallet/internalWallet";
    const adTypesPage = self::parentPath . "ad_types/adTypes";
    const feescommissionsPage = self::parentPath . "fee_commissions/commissions";
    ...
    const userWalletPage = self::parentPath . "user/Wallet";
    const stripeConnecttPage = self::parentPath . "user/StripeConnect";

    const userPurchasesHistoryPage = self::parentPath . "sales_purchase_history/SalesPurchaseHistory";
    const userDisputesPage = self::parentPath . "user/Disputes";
    const createUserDisputePage = self::parentPath . "user/CreateDisputes";
    const userDisputeDetails = self::parentPath . "user/DisputeDetails";
    const userWalletWithdrawalPage = self::parentPath . "user/WalletWithdrawal";
    const userBankWithdrawalPage = self::parentPath . "user/BankWithdrawal";
    const userWalletPaymentDonePage = self::parentPath . "user/WalletPaymentDone";
    const userWalletPaymentFailedPage = self::parentPath . "user/WalletPaymentFailed";
    ...
    const joinAuctionPage = self::parentPath . "auction/GoLive";
    const hostAuctionPage = self::parentPath . "auction/AuctionHost";
    const checkoutPage = self::parentPath . "checkout/Checkout";
    const userTrackOrderPage = self::parentPath . "checkout/UserTrackOrder";
    const userbidsPage = self::parentPath . "buyer_bidding/BuyerBids";
    const userViewBidsPage = self::parentPath . "viewBids/view";
    const auctionViewPage = self::parentPath . "auction/AuctionView";
    ...
    public function __construct(AboutService $aboutService, BlogService $blogService, PushNotificationMessageService $pushnotificationMessageService, LandingPageService $landingPageService, ItemService $itemService, ImageService $imageService, CategoryService $categoryService, UserService $userService)
    {
        ...
        $this->metaTitle = $this->frontendSettingService->frontend_meta_title ?? 'test';
        $this->metaDescription = $this->frontendSettingService->frontend_meta_description ?? 'test';
    }
    ...
    //April 29  feContactUsInfo
     public function feContactUsInfo()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::contactInfoPage);
    }

    public function feInternalWallet()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::internalwalletPage);
    }
    public function feAdTypes()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::adTypesPage);
    }
     public function feFeesCommissions()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::feescommissionsPage);
    }
    ...
    /**************user wallet Feb 09 *******/
    public function feUserWallet()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userWalletPage);
    }
     public function feStripeConnect()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::stripeConnecttPage);
    }

    public function feUserDisputes()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userDisputesPage);
    }

    public function feSalePurchaseHistory()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userPurchasesHistoryPage);
    }

    public function feUserBids()
    {
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userbidsPage);
    }

    public function feUserViewBids(Request $request)
    {
        $dataArr = [
            'query' => json_decode(json_encode($request->all()))
        ];
        ...
        return renderView(self::userViewBidsPage, $dataArr);
    }

    public function feTrackOrder(Request $request)
    {
        $dataArr = [
            'query' => json_decode(json_encode($request->all()))
        ];
        ...
        return renderView(self::userTrackOrderPage, $dataArr);
    }

    public function feCreateUserDisputes()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::createUserDisputePage);
    }


    public function feDisputeDetails(Request $request)
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);
        ...
        return renderView(self::userDisputeDetails, $dataArr);


       // return renderView(self::userDisputeDetails);
    }

    /*********************March26 */
    public function feUserWalletWithdrawal()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userWalletWithdrawalPage);
    }

    /*********************payment urls *****************/
    public function fePaymentDone()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userWalletPaymentDonePage);
    }
    public function fePaymentFailed()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userWalletPaymentFailedPage);
    }
    /*********************payment urls ends*****************/

    public function feUserBankDetails()
    {
        //for meta
        $this->setMeta($this->metaTitle ?? __('site_name'), $this->metaDescription ?? null, null);

        return renderView(self::userBankWithdrawalPage);
    }
    ...
    public function feJoinAuction(Request $request)
    {
        $dataArr = [
            'query' => json_decode(json_encode($request->all()))
        ];
        ...
        return renderView(self::joinAuctionPage, $dataArr);
    }

    public function feHostAuction(Request $request)
    {
        $dataArr = [
            'query' => json_decode(json_encode($request->all()))
        ];
        ...
        return renderView(self::hostAuctionPage, $dataArr);
    }

    public function feCheckout(Request $request)
    {
        $dataArr = [
            'query' => json_decode(json_encode($request->all()))
        ];
        ...
        return renderView(self::checkoutPage, $dataArr);
    }

    public function feAuctionView(Request $request)
    {
        $dataArr = [
            'query' => json_decode(json_encode($request->all()))
        ];
        ...
        return renderView(self::auctionViewPage, $dataArr);
    }
